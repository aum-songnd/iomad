...
/**
 * Back-end code for handling data about thquizzes and the current user's attempt.
 *
 * There are classes for loading all the information about a thquiz and attempts,
...
/**
 * Class for thquiz exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_thquiz_exception extends moodle_exception {
    /**
     * Constructor.
     *
     * @param thquiz $thquizobj the thquiz the error relates to.
     ...
     */
    public function __construct($thquizobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $thquizobj->view_url();
        }
        parent::__construct($errorcode, 'thquiz', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a thquiz and the questions it contains, and making the
...
class thquiz {
    ...
    protected $cm;
    /** @var stdClass the thquiz settings from the database. */
    protected $thquiz;
    ...
    /** @var stdClass[] of thquiz_section rows. */
    protected $sections = null;
    /** @var thquiz_access_manager the access manager for this thquiz. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/thquiz:preview. */
     ...
     *
     * @param object $thquiz the row from the thquiz table.
     * @param object $cm the course_module object for this thquiz.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($thquiz, $cm, $course, $getcontext = true) {
        $this->thquiz = $thquiz;
        $this->cm = $cm;
        $this->thquiz->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new thquiz object for a specific user.
     *
     * @param int $thquizid the the thquiz id.
     * @param int|null $userid the the userid (optional). If passed, relevant overrides are applied.
     * @return thquiz the new thquiz object.
     */
    public static function create($thquizid, $userid = null) {
        global $DB;

        $thquiz = thquiz_access_manager::load_thquiz_and_settings($thquizid);
        $course = $DB->get_record('course', array('id' => $thquiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('thquiz', $thquiz->id, $course->id, false, MUST_EXIST);

        // Update thquiz with override information.
        if ($userid) {
            $thquiz = thquiz_update_effective_access($thquiz, $userid);
        }

        return new thquiz($thquiz, $cm, $course);
    }

    /**
     * Create a {@link thquiz_attempt} for an attempt at this thquiz.
     *
     * @param object $attemptdata row from the thquiz_attempts table.
     * @return thquiz_attempt the new thquiz_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new thquiz_attempt($attemptdata, $this->thquiz, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this thquiz.
     */
    public function preload_questions() {
        $slots = qbank_helper::get_question_structure($this->thquiz->id, $this->context);
        $this->questions = [];
    ...
    /**
     * Fully load some or all of the questions for this thquiz. You must call
    ...
    public function load_questions($deprecated = null) {
        if ($deprecated !== null) {
            debugging('The argument to thquiz::load_questions is no longer supported. ' .
    ...
    /**
     * Get an instance of the {@link \mod_thquiz\structure} class for this thquiz.
     * @return \mod_thquiz\structure describes the questions in the thquiz.
     */
    public function get_structure() {
        return \mod_thquiz\structure::create_for_thquiz($this);
    }

    // Simple getters ==========================================================
    /**
     * Get the id of the course this thquiz belongs to.
     *
    ...
    /**
     * Get the course settings object that this thquiz belongs to.
     *
    ...
    /**
     * Get this thquiz's id (in the thquiz table).
     *
     * @return int the thquiz id.
     */
    public function get_thquizid() {
        return $this->thquiz->id;
    }

    /**
     * Get the thquiz settings object.
     *
     * @return stdClass the row of the thquiz table.
     */
     ...
     * Get the thquiz name.
     *
     * @return string the name of this thquiz.
     */
    public function get_thquiz_name() {
        return $this->thquiz->name;
    }

    /**
     * Get the navigation method in use.
     *
     * @return int THQUIZ_NAVMETHOD_FREE or THQUIZ_NAVMETHOD_SEQ.
     */
    public function get_navigation_method() {
        return $this->thquiz->navmethod;
    }

    /** @return int the number of attempts allowed at this thquiz (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->thquiz->attempts;
    }

    /**
     * Get the course-module id for this thquiz.
     *
    ...
    /**
     * Get the course-module object for this thquiz.
     *
    ...
    /**
     * Get the thquiz context.
     *
     * @return context_module the module context for this thquiz.
     */
    ...
    /**
     * @return bool whether the current user is someone who previews the thquiz,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/thquiz:preview', $this->context);
        }
    ...
    public function is_participant($userid) {
        return is_enrolled($this->get_context(), $userid, 'mod/thquiz:attempt', $this->show_only_active_users());
    }
    ...
    /**
     * @return bool whether any questions have been added to this thquiz.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'thquiz', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this thquiz.
     *
     * @return array 0, 1, 2, ... => thquiz_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('thquiz_sections',
                    array('thquizid' => $this->get_thquizid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return thquiz_access_manager and instance of the thquiz_access_manager class
     * for this thquiz at this time.
     *
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new thquiz_access_manager($this, $timenow,
                    has_capability('mod/thquiz:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the thquiz context.
     *
     * @param string $capability the name of the capability to check. For example mod/thquiz:view.
    ...
    /**
     * Wrapper round the require_capability function that automatically passes in the thquiz context.
     *
     * @param string $capability the name of the capability to check. For example mod/thquiz:view.
     * @param int|null $userid A user id. By default (null) checks the permissions of the current user.
     * @param bool $doanything If false, ignore effect of admin role assignment.
    ...
    /**
     * @return string the URL of this thquiz's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/thquiz/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this thquiz's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/thquiz/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/thquiz/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
     ...
     * @return moodle_url the URL of this thquiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/thquiz/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/thquiz/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/thquiz/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by thquiz access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by thquiz access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_thquiz_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_thquiz_display_options::DURING ||
                $when == mod_thquiz_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_thquiz_display_options::LATER_WHILE_OPEN && $this->thquiz->timeclose &&
                $this->thquiz->reviewattempt & mod_thquiz_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'thquiz',
                    userdate($this->thquiz->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'thquiz');
        }
    }
     ...
     *
     * @param string $title the name of this particular thquiz page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_thquiz_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this thquiz.
     *
     * @param  boolean $includepotential if the thquiz include random questions,
...
/**
 * This class extends the thquiz class to hold data about the state of a particular attempt,
 * in addition to the data about the thquiz.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class thquiz_attempt {
    ...
    /** @var int maximum number of slots in the thquiz for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var thquiz object containing the thquiz settings. */
    protected $thquizobj;

    /** @var stdClass the thquiz_attempts row. */
    protected $attempt;
    ...
    /** @var array of thquiz_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_thquiz_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the thquiz_attempts table.
     * @param object $thquiz the thquiz object for this attempt and user.
     * @param object $cm the course_module object for this thquiz.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $thquiz, $cm, $course, $loadquestions = true) {
        $this->attempt = $attempt;
        $this->thquizobj = new thquiz($thquiz, $cm, $course);
     ...
     *
     * @param array $conditions passed to $DB->get_record('thquiz_attempts', $conditions).
     * @return thquiz_attempt the desired instance of this class.
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('thquiz_attempts', $conditions, '*', MUST_EXIST);
        $thquiz = thquiz_access_manager::load_thquiz_and_settings($attempt->thquiz);
        $course = $DB->get_record('course', array('id' => $thquiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('thquiz', $thquiz->id, $course->id, false, MUST_EXIST);
        ...
        return new thquiz_attempt($attempt, $thquiz, $cm, $course);
    }

    /**
     * Static function to create a new thquiz_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new thquiz_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return thquiz_attempt_state_name($state);
    }
    ...
    public function load_questions() {
        ...
        if (isset($this->quba)) {
            throw new coding_exception('This thquiz attempt has already had the questions loaded.');
        }

        $this->quba = question_engine::load_questions_usage_by_activity($this->attempt->uniqueid);
        $this->slots = $DB->get_records('thquiz_slots',
                array('thquizid' => $this->get_thquizid()), 'slot', 'slot, id, requireprevious');
        $this->sections = array_values($DB->get_records('thquiz_sections',
                array('thquizid' => $this->get_thquizid()), 'firstslot'));
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'thquiz');
                }
    ...
    // Simple getters ==========================================================
    public function get_thquiz() {
        return $this->thquizobj->get_thquiz();
    }

    public function get_thquizobj() {
        return $this->thquizobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->thquizobj->get_courseid();
    }
     ...
     */
    public function get_course() {
        return $this->thquizobj->get_course();
    }

    /** @return int the thquiz id. */
    public function get_thquizid() {
        return $this->thquizobj->get_thquizid();
    }

    /** @return string the name of this thquiz. */
    public function get_thquiz_name() {
        return $this->thquizobj->get_thquiz_name();
    }

    /** @return int the thquiz navigation method. */
    public function get_navigation_method() {
        return $this->thquizobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->thquizobj->get_cm();
    }
     ...
     */
    public function get_cmid() {
        return $this->thquizobj->get_cmid();
    }
     ...
     */
    public function is_preview_user() {
        return $this->thquizobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this thquiz (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->thquizobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this thquiz. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->thquizobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the thquiz_attempts table. */
    ...
    /** @return string one of the thquiz_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/thquiz:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the thquiz in a non-trivial way?
     *
     ...
     * To see how this works, try setting a thquiz to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the thquiz
     ...
     *
     * @param question_display_options $options the display options for this thquiz attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return thquiz_feedback_for_grade($grade, $this->get_thquiz(),
                $this->thquizobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->thquizobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        $this->thquizobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_thquiz_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/thquiz:attempt';
        } else {
            $capability = 'mod/thquiz:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a thquiz
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the thquiz as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/thquiz:viewreports') ||
                $this->has_capability('mod/thquiz:preview')) {
    ...
    public function can_navigate_to($slot) {
        ...
        switch ($this->get_navigation_method()) {
            case THQUIZ_NAVMETHOD_FREE:
                return true;
                break;
            case THQUIZ_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return thquiz_attempt_state($this->get_thquiz(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_thquiz_display_options for this thquiz at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = thquiz_get_review_options($this->get_thquiz(),
                        $this->attempt, $this->thquizobj->get_context());
            ...
            $options = mod_thquiz_display_options::make_from_thquiz($this->get_thquiz(),
                    mod_thquiz_display_options::DURING);
            $options->flags = thquiz_get_flag_option($this->attempt, $this->thquizobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the thquiz.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the thquiz, or for the
     * whole thquiz.
     *
    ...
    public function get_question_usage() {
        if (!(PHPUNIT_TEST || defined('BEHAT_TEST'))) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the thquiz_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
            ...
            $this->get_navigation_method() != THQUIZ_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_thquiz()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the thquiz where this question appears.
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the thquiz this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the thquiz.
     */
    public function get_question_mark($slot) {
        return thquiz_format_question_grade($this->get_thquiz(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->thquizobj->get_thquiz()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->thquizobj->get_thquiz()->timelimit;
        }
        if ($this->thquizobj->get_thquiz()->timeclose) {
            $deadlines[] = $this->thquizobj->get_thquiz()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->thquizobj->get_thquiz()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string thquiz view url.
     */
    public function view_url() {
        return $this->thquizobj->view_url();
    }
     ...
     * @return string the URL of this thquiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->thquizobj->start_attempt_url($page);
    }
    ...
    public function attempt_page_title(int $page) : string {
        if ($this->get_num_pages() > 1) {
            $a = new stdClass();
            $a->name = $this->get_thquiz_name();
            $a->currentpage = $page + 1;
            $a->totalpages = $this->get_num_pages();
            $title = get_string('attempttitlepaged', 'thquiz', $a);
        } else {
            $title = get_string('attempttitle', 'thquiz', $this->get_thquiz_name());
        }
    ...
    public function summary_page_title() : string {
        return get_string('attemptsummarytitle', 'thquiz', $this->get_thquiz_name());
    }

    /**
     * @return moodle_url the URL of this thquiz's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/thquiz/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/thquiz/processattempt.php');
    }

    public function processattempt_url_cluster($cm_id_cluster) {
        return new moodle_url('/mod/thquiz/processattempt.php', array('cm_id_cluster' => $cm_id_cluster));
    }
    ...
    public function review_page_title(int $page, bool $showall = false) : string {
        if (!$showall && $this->get_num_pages() > 1) {
            ...
            $a->currentpage = $page + 1;
            $a->totalpages = $this->get_num_pages();
            $title = get_string('attemptreviewtitlepaged', 'thquiz', $a);
        } else {
            $title = get_string('attemptreviewtitle', 'thquiz', $this->get_thquiz_name());
        }
    ...
    public function cannot_review_message($short = false) {
        return $this->thquizobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'thquiz'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_thquiz_renderer $renderer the thquiz renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_thquiz_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_thquiz-blocked_question_warning');
        }
    ...
    protected function render_question_helper($slot, $reviewing, $thispageurl,
            mod_thquiz_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'thquiz', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'thquiz');
        ...
        $placeholderqa->start($this->get_thquiz()->preferredbehaviour, 1);
     ...
     *
     * @param int $slot the slot number of a question in this thquiz attempt.
    ...
    public function render_question_at_step($slot, $seq, $reviewing,
            mod_thquiz_renderer $renderer, $thispageurl = null) {
     ...
     *
     * @param int $slot the slot of a question in this thquiz attempt.
     * @param bool $reviewing is the being printed on an attempt or a review page.
     ...
     *
     * @param mod_thquiz_renderer $output the thquiz renderer to use to output things.
     * @param string $panelclass The type of panel, thquiz_attempt_nav_panel or thquiz_review_nav_panel
     * @param int $page the current page number.
     * @param bool $showall whether we are showing the whole thquiz on one page. (Used by review.php.)
     * @return block_contents the requested object.
     */
    public function get_navigation_panel(mod_thquiz_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);
        $attemptobj = $panel->get_attemptobj();
		$thquiz = $attemptobj->get_thquiz();

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_thquiz_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->title = $thquiz->name;
        $bc->content = $output->navigation_panel_cluster($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this thquiz.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_thquiz_links_to_other_attempts} object returned contains an
     ...
     * @return mod_thquiz_links_to_other_attempts|bool containing array int => null|moodle_url.
     *      False if none.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = thquiz_get_user_attempts($this->get_thquiz()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_thquiz_links_to_other_attempts();
     ...
     * @return mod_thquiz_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->thquizobj->get_thquiz()->graceperiod;
        ...
        // Otherwise, we were in thquiz_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->thquizobj->get_thquiz()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('thquiz_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            thquiz_save_best_grade($this->get_thquiz(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_thquiz\question\qubaids_for_users_attempts(
                $this->get_thquizid(), $this->get_userid(), 'all', true);

        $transaction = $DB->start_delegated_transaction();

        // Add the question to the usage. It is important we do this before we choose a variant.
        $newquestionid = qbank_helper::choose_question_for_redo($this->get_thquizid(),
                    $this->get_thquizobj()->get_context(), $this->slots[$slot]->id, $qubaids);
        $newquestion = question_bank::load_question($newquestionid, $this->get_thquiz()->shuffleanswers);
     ...
     * Submit the attempt.
     *
     * The separate $timefinish argument should be used when the thquiz attempt
    ...
    public function process_finish($timestamp, $processsubmitted, $timefinish = null, $studentisonline = false) {
        ...
        if (!$this->requires_manual_grading() ||
                !has_capability('mod/thquiz:emailnotifyattemptgraded', $this->get_thquizobj()->get_context(),
                        $this->get_userid())) {
        ...
        if (!$this->is_preview()) {
            thquiz_save_best_grade($this->get_thquiz(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_thquiz\event\attempt_submitted', $timestamp, $studentisonline);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('thquiz_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_thquiz\event\attempt_becameoverdue', $timestamp, $studentisonline);

        $transaction->allow_commit();

        thquiz_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_thquiz\event\attempt_abandoned', $timestamp, $studentisonline);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp, $studentisonline) {
        global $USER;
        $thquizrecord = $this->get_thquiz();
        $params = array(
            'context' => $this->get_thquizobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'thquizid' => $thquizrecord->id,
                'studentisonline' => $studentisonline
            )
        );
        $event = $eventclass::create($params);
        $event->add_record_snapshot('thquiz', $this->get_thquiz());
        $event->add_record_snapshot('thquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the thquiz.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/thquiz/$script.php.
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/thquiz/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a thquiz.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        $toolate = false;
        if ($timeclose !== false) {
            if ($timenow > $timeclose - THQUIZ_MIN_TIME_TO_CONTINUE) {
                // If there is only a very small amount of time left, there is no point trying
                // to show the student another page of the thquiz. Just finish now.
                $timeup = true;
        ...
        if ($timeup) {
            if ($this->get_thquiz()->overduehandling === 'graceperiod') {
                if ($timenow > $timeclose + $this->get_thquiz()->graceperiod + $graceperiodmin) {
        ...
        // Update the thquiz attempt record.
        try {
            if ($becomingabandoned) {
                $this->process_abandon($timenow, true);
            } else {
                if (!$toolate || $this->get_thquiz()->overduehandling === 'graceperiod') {
    ...
    public function check_page_access(int $page, bool $allownext = true): bool {
        if ($this->get_navigation_method() != THQUIZ_NAVMETHOD_SEQ) {
            return true;
        }
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('thquiz_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
 ...
 * @since      Moodle 2.9
 */
class thquiz_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class thquiz_nav_question_button implements renderable {
    ...
    public $url;
    /** @var int THQUIZ_NAVMETHOD_FREE or THQUIZ_NAVMETHOD_SEQ. */
    public $navmethod;
}
 ...
 * @since      Moodle 2.0
 */
abstract class thquiz_nav_panel_base {
    /** @var thquiz_attempt */
    ...
    public function __construct(thquiz_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the thquiz navigation block.
     *
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            ...
            if (!is_null($heading)) {
                $sections = $this->attemptobj->get_thquizobj()->get_sections();
                if (!(empty($heading) && count($sections) == 1)) {
                    $buttons[] = new thquiz_nav_section_heading(format_string($heading));
                }
            }
            ...
            $button = new thquiz_nav_question_button();
            $button->id          = 'thquiznavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'thquiz');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'thquiz');
        } else {
            return get_string('viewed', 'thquiz');
        }
    }
     ...
     *
     * @param mod_thquiz_renderer $output the thquiz renderer to use.
     * @return string HTML to output.
     */
    public function render_before_button_bits(mod_thquiz_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_thquiz_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_thquiz()->showuserpicture == THQUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_thquiz()->showuserpicture == THQUIZ_SHOWIMAGE_LARGE) {
    ...
    public function get_button_container_class() {
        // Thquiz navigation is set on 'Show all questions on one page'.
        if ($this->showall) {
            return 'allquestionsononepage';
        }
        // Thquiz navigation is set on 'Show one page at a time'.
        return 'multipages';
    }
}
...
/**
 * Specialisation of {@link thquiz_nav_panel_base} for the attempt thquiz page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
// class thquiz_attempt_nav_panel extends thquiz_nav_panel_base {
...
//     public function render_before_button_bits(mod_thquiz_renderer $output) {
//         return html_writer::tag('div', get_string('navnojswarning', 'thquiz'),
//                 array('id' => 'thquiznojswarning'));
//     }

//     public function render_end_bits(mod_thquiz_renderer $output) {
//         if ($this->page == -1) {
//             // Don't link from the summary page to itself.
//             return '';
//         }
//         return html_writer::link($this->attemptobj->summary_url(),
//                 get_string('endtest', 'thquiz'), array('class' => 'endtestlink aalink')) .
...
/**
 * Specialisation of {@link thquiz_nav_panel_base} for the review thquiz page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
// class thquiz_review_nav_panel extends thquiz_nav_panel_base {
//     public function get_question_url($slot) {
