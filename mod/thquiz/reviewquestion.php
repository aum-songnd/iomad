...
$baseurl = new moodle_url('/mod/thquiz/reviewquestion.php',
...
$PAGE->set_title(get_string('reviewofquestion', 'thquiz', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'thquiz' => format_string($attemptobj->get_thquiz_name()), 'user' => fullname($student))));
...
if ($attemptobj->is_own_attempt()) {
    if (!$attemptobj->is_finished()) {
        echo $output->review_question_not_allowed($attemptobj, get_string('cannotreviewopen', 'thquiz'));
        die();
...
// Thquiz name.
$summarydata['thquizname'] = array(
    'title'   => get_string('modulename', 'thquiz'),
    'content' => format_string($attemptobj->get_thquiz_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'thquiz'),
    'content' => $attemptobj->get_question_name($slot),
);

// Other attempts at the thquiz.
