...
/**
 * This page handles listing of thquiz overrides
 *
 * @package    mod_thquiz
...
$thquiz = $DB->get_record('thquiz', ['id' => $cm->instance], '*', MUST_EXIST);
...
$canedit = has_capability('mod/thquiz:manageoverrides', $context);
if (!$canedit) {
    require_capability('mod/thquiz:viewoverrides', $context);
}

$thquizgroupmode = groups_get_activity_groupmode($cm);
$showallgroups = ($thquizgroupmode == NOGROUPS) || has_capability('moodle/site:accessallgroups', $context);
...
$url = new moodle_url('/mod/thquiz/overrides.php', ['cmid' => $cm->id, 'mode' => $mode]);

$title = get_string('overridesforthquiz', 'thquiz',
        format_string($thquiz->name, true, ['context' => $context]));
$PAGE->set_url($url);
...
$PAGE->set_secondary_active_tab("mod_thquiz_useroverrides");
...
if (!empty($orphaned)) {
    $DB->delete_records_list('thquiz_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    if ($groups) {
        ...
        $sql = "SELECT o.*, g.name
                 ...
                 WHERE o.thquiz = :thquizid AND g.id $insql
              ORDER BY g.name";
...
$table->head[] = get_string('overrides', 'thquiz');
...
$overridedeleteurl = new moodle_url('/mod/thquiz/overridedelete.php');
$overrideediturl = new moodle_url('/mod/thquiz/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    $active = true;
    if (!$groupmode) {
        if (!has_capability('mod/thquiz:attempt', $context, $override->userid)) {
            // User not allowed to take the thquiz.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('thquizopens', 'thquiz');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'thquiz');
    }
    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('thquizcloses', 'thquiz');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'thquiz');
    }
    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'thquiz');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'thquiz');
    }
    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'thquiz');
        $values[] = $override->attempts > 0 ?
                $override->attempts : get_string('unlimited');
    }
    // Format password.
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'thquiz');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'thquiz') : get_string('none', 'thquiz');
    }
...
if ($canedit) {
    if ($groupmode) {
        if (empty($groups)) {
            // There are no groups.
            $warningmessage = get_string('groupsnone', 'thquiz');
            $addenabled = false;
        }
    } else {
        // See if there are any students in the thquiz.
        if ($showallgroups) {
            $users = get_users_by_capability($context, 'mod/thquiz:attempt', 'u.id');
            $nousermessage = get_string('usersnone', 'thquiz');
        } else if ($groups) {
            $users = get_users_by_capability($context, 'mod/thquiz:attempt', 'u.id', '', '', '', array_keys($groups));
            $nousermessage = get_string('usersnone', 'thquiz');
        } else {
            $users = [];
            $nousermessage = get_string('groupsnone', 'thquiz');
        }
...
$tertiarynav = new \mod_thquiz\output\overrides_actions($cmid, $mode, $canedit, $addenabled);
echo $renderer->render($tertiarynav);

if ($mode === 'user') {
    echo $OUTPUT->heading(get_string('useroverrides', 'thquiz'));
} else {
    echo $OUTPUT->heading(get_string('groupoverrides', 'thquiz'));
}

// Output the table and button.
echo html_writer::start_tag('div', ['id' => 'thquizoverrides']);
if (count($table->data)) {
    echo html_writer::table($table);
} else {
    if ($groupmode) {
        echo $OUTPUT->notification(get_string('overridesnoneforgroups', 'thquiz'), 'info', false);
    } else {
        echo $OUTPUT->notification(get_string('overridesnoneforusers', 'thquiz'), 'info', false);
    }
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'thquiz'), 'info', false);
}
