...
if ($page == -1) {
    $nexturl = $attemptobj->summary_url_cluster($cm_cluster_id);
} else {
    $nexturl = $attemptobj->attempt_cluster_url($cm_cluster_id, null, $page);
    if ($scrollpos !== '') {
...
if ($attemptobj->is_finished()) {
    throw new moodle_quiz_exception($attemptobj->get_quizobj(),
            'attemptalreadyclosed', null, $attemptobj->review_cluster_url($cm_cluster_id));
}
