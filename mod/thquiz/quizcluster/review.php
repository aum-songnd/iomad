...
$cm_cluster_id = optional_param('cm_cluster_id', null, PARAM_INT);
...
$attemptobj = quiz_cluster_create_attempt_handling_errors($attemptid, $cmid);
...
$PAGE->set_url($attemptobj->review_cluster_url($cm_cluster_id,null, $page, $showall));


// Check login.
require_login_cluster($attemptobj->get_course(), false, $attemptobj->get_cm());
...
if ($attemptobj->is_own_attempt()) {
    if (!$attemptobj->is_finished()) {
        redirect($attemptobj->attempt_cluster_url($cm_cluster_id, null, $page));
        ...
        redirect($quizobj->view_cluster_url($cm_cluster_id), $attemptobj->cannot_review_message());
...
// $navbc = $attemptobj->get_navigation_panel_cluster1($output, $cm_cluster_id, 'thquiz_review_nav_panel', $page, $showall);
$navbc = $attemptobj->get_navigation_panel_cluster($output, $cm_cluster_id, 'thquiz_review_nav_panel', $page, $showall);
...
echo $output->review_cluster_page($cm_cluster_id, $attemptobj, $slots, $page, $showall, $lastpage, $options, $summarydata);
