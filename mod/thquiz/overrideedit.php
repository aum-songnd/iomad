...
/**
 * This page handles editing and creation of thquiz overrides
 *
 * @package   mod_thquiz
...
$url = new moodle_url('/mod/thquiz/overrideedit.php');
if ($action) {
...
// Merge thquiz defaults with data.
...
$overridelisturl = new moodle_url('/mod/thquiz/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new thquiz_override_form($url, $cm, $thquiz, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->thquiz = $thquiz->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $thquiz->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('thquiz_overrides', $conditions)) {
            ...
            $thquiz->cmid = $cm->id;
            thquiz_delete_override($thquiz, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('thquiz_overrides', $fromform);
        $cachekey = $groupmode ? "{$fromform->thquiz}_g_{$fromform->groupid}" : "{$fromform->thquiz}_u_{$fromform->userid}";
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('thquiz_overrides', $fromform);
        $cachekey = $groupmode ? "{$fromform->thquiz}_g_{$fromform->groupid}" : "{$fromform->thquiz}_u_{$fromform->userid}";
        cache::make('mod_thquiz', 'overrides')->delete($cachekey);
    ...
    if ($groupmode) {
        ...
        thquiz_update_events($thquiz, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'thquiz');
...
$PAGE->activityheader->set_attrs([
    "title" => format_string($thquiz->name, true, array('context' => $context)),
    "description" => "",
    "hidecompletion" => true
]);
echo $OUTPUT->header();
