...
// Get the thquiz slot.
$slot = $DB->get_record('thquiz_slots', ['id' => $slotid]);
if (!$slot) {
    new moodle_exception('invalidrandomslot', 'mod_thquiz');
}

if (!$thquiz = $DB->get_record('thquiz', ['id' => $slot->thquizid])) {
    new moodle_exception('invalidthquizid', 'thquiz');
}

$cm = get_coursemodule_from_instance('thquiz', $slot->thquizid, $thquiz->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/thquiz/edit.php', ['cmid' => $cm->id]);
}

$url = new moodle_url('/mod/thquiz/editrandom.php', ['slotid' => $slotid]);
$PAGE->set_url($url);
...
$mform = new mod_thquiz\form\randomquestion_form(new moodle_url('/mod/thquiz/editrandom.php'), ['contexts' => $contexts]);
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_thquiz');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_thquiz');
