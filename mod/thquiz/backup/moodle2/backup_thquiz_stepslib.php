...
/**
 * Define all the backup steps that will be used by the backup_thquiz_activity_task.
 *
 * @package    mod_thquiz
 ...
 */
class backup_thquiz_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $thquiz = new backup_nested_element('thquiz', ['id'], [
        ...
        $this->add_subplugin_structure('thquizaccess', $thquiz, true);

        $qinstances = new backup_nested_element('question_instances');

        $qinstance = new backup_nested_element('question_instance', ['id'],
                ['thquizid', 'slot', 'page', 'requireprevious', 'maxmark']);

        $this->add_question_references($qinstance, 'mod_thquiz', 'slot');

        $this->add_question_set_references($qinstance, 'mod_thquiz', 'slot');
        ...
        $this->add_subplugin_structure('thquizaccess', $attempt, true);

        // Build the tree.
        $thquiz->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $thquiz->add_child($sections);
        $sections->add_child($section);

        $thquiz->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $thquiz->add_child($overrides);
        $overrides->add_child($override);

        $thquiz->add_child($grades);
        $grades->add_child($grade);

        $thquiz->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $thquiz->set_source_table('thquiz', ['id' => backup::VAR_ACTIVITYID]);

        $qinstance->set_source_table('thquiz_slots', ['thquizid' => backup::VAR_PARENTID]);

        $section->set_source_table('thquiz_sections', ['thquizid' => backup::VAR_PARENTID]);

        $feedback->set_source_table('thquiz_feedback', ['thquizid' => backup::VAR_PARENTID]);

        // Thquiz overrides to backup are different depending of user info.
        $overrideparams = ['thquiz' => backup::VAR_PARENTID];
        ...
        $override->set_source_table('thquiz_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('thquiz_grades', ['thquiz' => backup::VAR_PARENTID]);
            $attempt->set_source_sql('
                    SELECT *
                    FROM {thquiz_attempts}
                    WHERE thquiz = :thquiz AND preview = 0', ['thquiz' => backup::VAR_PARENTID]);
        }

        // Define source alias.
        $thquiz->set_source_alias('attempts', 'attempts_number');
        ...
        $thquiz->annotate_files('mod_thquiz', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_thquiz', 'feedback', 'id');

        // Return the root element (thquiz), wrapped into standard activity structure.
        return $this->prepare_activity_structure($thquiz);
    }
}
