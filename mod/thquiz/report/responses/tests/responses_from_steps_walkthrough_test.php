 ...
 */
class responses_from_steps_walkthrough_test extends \mod_thquiz\attempt_walkthrough_from_csv_test {
     ...
     *
     * @param array $thquizsettings settings to override default settings for thquiz created by generator. Taken from thquizzes.csv.
    ...
    public function test_walkthrough_from_csv($thquizsettings, $csvdata) {
        ...
        $thquizattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        foreach ($csvdata['responses'] as $responsesfromcsv) {
            ...
            if (!isset($thquizattemptids[$responses['thquizattempt']])) {
                throw new \coding_exception("There is no thquizattempt {$responses['thquizattempt']}!");
            }
            $this->assert_response_test($thquizattemptids[$responses['thquizattempt']], $responses);
        }
    }

    protected function assert_response_test($thquizattemptid, $responses) {
        $thquizattempt = thquiz_attempt::create($thquizattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $thquizattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new \coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in thquizattempt {$responses['thquizattempt']}!");
            }
            foreach (array('responsesummary', 'fraction', 'state') as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  thquizattempt {$responses['thquizattempt']} in $column, slot $slot, ".
