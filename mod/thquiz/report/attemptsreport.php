...
/**
 * Base class for thquiz reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class thquiz_attempts_report extends thquiz_default_report {
    ...
    /** @var context_module the thquiz context. */
    protected $context;

    /** @var mod_thquiz_attempts_report_form The settings form to use. */
    protected $form;
     ...
     */
    public function init($mode, $formclass, $thquiz, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = thquiz_report_qm_filter_select($thquiz);

        $this->form = new $formclass($this->get_base_url(),
                array('thquiz' => $thquiz, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                array('mod/thquiz:attempt', 'mod/thquiz:reviewmyattempts'), $currentgroup);

        return array($currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins);
    }

    /**
     * Outputs the things you commonly want at the top of a thquiz report.
     *
     ...
     * @param mod_thquiz_attempts_report_options $options the current report settings.
     * @param int $currentgroup the current group.
     * @param bool $hasquestions whether there are any questions in the thquiz.
     * @param bool $hasstudents whether there are any relevant students.
     */
    protected function print_standard_header_and_messages($cm, $course, $thquiz,
            $options, $currentgroup, $hasquestions, $hasstudents) {
        global $OUTPUT;

        $this->print_header_and_tabs($cm, $course, $thquiz, $this->mode);
        ...
        if ($strattemptnum = thquiz_num_attempt_summary($thquiz, $cm, true, $currentgroup)) {
            echo '<div class="thquizattemptcounts">' . $strattemptnum . '</div>';
        }
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'thquiz');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'thquiz');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'thquiz');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'thquiz');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this thquiz.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($thquiz, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'thquiz') . '/' .
                    thquiz_format_grade($thquiz, $thquiz->grade);
        }

        if ($includefeedback && thquiz_has_feedback($thquiz)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'thquiz');
        }
    }
     ...
     * @param mod_thquiz_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_thquiz_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/thquiz:deleteattempts', $this->context);
                    $this->delete_selected_attempts($thquiz, $cm, $attemptids, $allowedjoins);
    ...
    /**
     * Delete the thquiz attempts
     * @param object $thquiz the thquiz settings. Attempts that don't belong to
     * this thquiz are not deleted.
     ...
     */
    protected function delete_selected_attempts($thquiz, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        global $DB;
        ...
        foreach ($attemptids as $attemptid) {
            if (empty($allowedjoins->joins)) {
                $sql = "SELECT thquiza.*
                         ...
                         WHERE thquiza.id = :attemptid";
            } else {
                $sql = "SELECT thquiza.*
                          FROM {thquiz_attempts} thquiza
                          JOIN {user} u ON u.id = thquiza.userid
                        {$allowedjoins->joins}
                         WHERE {$allowedjoins->wheres} AND thquiza.id = :attemptid";
            }
            $params = $allowedjoins->params + array('attemptid' => $attemptid);
            $attempt = $DB->get_record_sql($sql, $params, IGNORE_MULTIPLE);
            if (!$attempt || $attempt->thquiz != $thquiz->id || $attempt->preview != 0) {
                // Ensure the attempt exists, belongs to this thquiz and belongs to
                // a student included in the report. If not skip.
                continue;
            }

            // Set the course module id before calling thquiz_delete_attempt().
            $thquiz->cmid = $cm->id;
