...
/**
 * Base class for the table used by a {@link thquiz_attempts_report}.
 *
 * @package   mod_thquiz
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class thquiz_attempts_report_table extends table_sql {
    ...
    /** @var object the thquiz settings for the thquiz we are reporting on. */
    protected $thquiz;
    ...
    /** @var object mod_thquiz_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this thquiz.. */
    protected $questions;
    ...
    protected $togglegroup = 'thquiz-attempts';
     ...
     * @param mod_thquiz_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $thquiz, $context, $qmsubselect,
            mod_thquiz_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_checkbox($attempt) {
        ...
        if ($attempt->attempt) {
            $checkbox = new \core\output\checkbox_toggleall($this->togglegroup, false, [
                ...
                'label' => get_string('selectattempt', 'thquiz'),
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/thquiz/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'thquiz'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return thquiz_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = thquiz_report_feedback_for_grade(
                thquiz_rescale_grade($attempt->sumgrades, $this->thquiz, false),
                $this->thquiz->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/thquiz/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'thquiz')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as thquiza.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(thquiza.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                thquiza.uniqueid AS usageid,
                thquiza.id AS attempt,
                u.id AS userid,
                u.idnumber,
                u.picture,
                u.imagealt,
                ...
                thquiza.timefinish,
                thquiza.timestart,
                CASE WHEN thquiza.timefinish = 0 THEN null
                     WHEN thquiza.timefinish > thquiza.timestart THEN thquiza.timefinish - thquiza.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and thquiz_attempts tables.
        $from = " {user} u";
        $from .= "\n{$userfields->joins}";
        $from .= "\nLEFT JOIN {thquiz_attempts} thquiza ON
                                    thquiza.userid = u.id AND thquiza.thquiz = :thquizid";
        $params = array_merge($userfields->params, ['thquizid' => $this->thquiz->id]);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (thquiza.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = thquiz_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case thquiz_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'thquiza.id IS NOT NULL AND thquiza.preview = 0';
                break;
            case thquiz_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "thquiza.preview = 0 AND thquiza.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case thquiz_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "thquiza.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case thquiz_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(thquiza.preview = 0 OR thquiza.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (thquiza.state $statesql OR thquiza.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{thquiz_attempts} {$alias}thquiza", "{$alias}thquiza.uniqueid",
                "{$alias}thquiza.thquiz = :{$alias}thquizid", array("{$alias}thquizid" => $this->sql->params['thquizid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = thquiza.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT thquiza.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) thquizasubquery", 'thquizasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['thquiza.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/thquiz:deleteattempts', $this->context)) {
            $deletebuttonparams = [
                'type'  => 'submit',
                'class' => 'btn btn-secondary mr-1',
                'id'    => 'deleteattemptsbutton',
                'name'  => 'delete',
                'value' => get_string('deleteselected', 'thquiz_overview'),
                'data-action' => 'toggle',
            ...
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'thquiz')));
        }
    }
     ...
     *
     * It returns the HTML for a master \core\output\checkbox_toggleall component that selects/deselects all thquiz attempts.
     *
    ...
    public function checkbox_col_header(string $columnname) {
        ...
        $selectalltext = get_string('selectall', 'thquiz');
        $deselectalltext = get_string('selectnone', 'thquiz');
