...
global $CFG;
require_once($CFG->dirroot . '/mod/thquiz/tests/attempt_walkthrough_from_csv_test.php');
 ...
 *
 * The thquiz stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-thquiz-tools/tree/master/statsspreadsheet
 *
 ...
 */
class stats_from_steps_walkthrough_test extends \mod_thquiz\attempt_walkthrough_from_csv_test {

    /**
     * @var thquiz_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a thquiz add questions to it, walk through thquiz attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($thquizsettings, $csvdata) {
        ...
        list($questions, $thquizstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($thquizsettings['testnumber'] === '00') {
            $this->check_variants_count_for_thquiz_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_thquiz_stats_for_thquiz_00($thquizstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_thquiz_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $thquizstats
     */
    protected function check_thquiz_stats_for_thquiz_00($thquizstats) {
        $thquizstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($thquizstatsexpected as $statname => $statvalue) {
            $this->assertEqualsWithDelta($statvalue, $thquizstats->$statname, abs($statvalue) * 1.5e-5, $thquizstats->$statname);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $thquizstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new thquiz_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->thquiz);
        list($thquizstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->thquiz,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = thquiz_statistics_qubaids_condition($this->thquiz->id, $groupstudentsjoins, $whichattempts);

        // We will create some thquiz and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $thquizcalc = new calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($thquizcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $thquizstats, $questionstats, $qubaids);
        }
        return array($questions, $thquizstats, $questionstats, $qubaids);
    }
