...
/**
 * Thquiz statistics report, table for showing statistics of each question in the thquiz.
 *
...
/**
 * This table has one row for each question in the thquiz, with sub-rows when
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_statistics_table extends flexible_table {
    /** @var object the thquiz settings. */
    protected $thquiz;

    /** @var integer the thquiz course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-thquiz-report-statistics-report');
    }
     ...
     * @param int $cmid the thquiz course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($thquiz, $cmid, $reporturl, $s) {
        $this->thquiz = $thquiz;
        $this->cmid = $cmid;
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'thquiz_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'thquiz_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'thquiz');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'thquiz_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'thquiz_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'thquiz_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'thquiz_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'thquiz_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'thquiz_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'thquiz_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'thquiz_statistics');
    ...
    protected function col_actions($questionstat) {
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            return thquiz_question_action_icons($this->thquiz, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'thquiz_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'thquiz_statistics')));
                ...
                if ($this->is_calculated_question_summary($questionstat)) {
                    ...
                    // in the UI.
                    $name = html_writer::div(get_string('rangeofvalues', 'thquiz_statistics'));
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'thquiz_statistics')));
                }
            }
        }
        ...
        if ($this->is_calculated_question_summary($questionstat)) {
            $name .= html_writer::link($url, get_string('viewanalysis', 'thquiz_statistics'));
        } else if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'thquiz_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the thquiz. That is, the indended influence this question
    ...
    protected function col_intended_weight($questionstat) {
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            if (is_null($min) && is_null($max)) {
                ...
                $min = thquiz_report_scale_summarks_as_percentage($min, $this->thquiz);
                $max = thquiz_report_scale_summarks_as_percentage($max, $this->thquiz);
                return $this->format_range($min, $max);
            }
        } else {
            return thquiz_report_scale_summarks_as_percentage($questionstat->maxmark, $this->thquiz);
        }
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            if (is_null($min) && is_null($max)) {
                ...
                if ($negcovar) {
                    $min = get_string('negcovar', 'thquiz_statistics');
                }
            ...
            $negcovar = get_string('negcovar', 'thquiz_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'thquiz_statistics'),
     ...
     * other questions in this thquiz.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    ...
    protected function format_range(string $min = null, string $max = null) {
        if (is_null($min) && is_null($max)) {
            ...
            return get_string('rangebetween', 'thquiz_statistics', $a);
        }
    }
