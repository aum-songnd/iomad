...
class calculated {
    ...
    /* Following stats all described here : http://docs.moodle.org/dev/Thquiz_statistics_calculations#Test_statistics  */
     ...
     */
    public function get_formatted_thquiz_info_data($course, $cm, $thquiz) {
        ...
        // General information about the thquiz.
        $thquizinfo = array();
        $thquizinfo[get_string('thquizname', 'thquiz_statistics')] = format_string($thquiz->name);
        $thquizinfo[get_string('coursename', 'thquiz_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $thquizinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($thquiz->timeopen) {
            $thquizinfo[get_string('thquizopen', 'thquiz')] = userdate($thquiz->timeopen);
        }
        if ($thquiz->timeclose) {
            $thquizinfo[get_string('thquizclose', 'thquiz')] = userdate($thquiz->timeclose);
        }
        if ($thquiz->timeopen && $thquiz->timeclose) {
            $thquizinfo[get_string('duration', 'thquiz_statistics')] =
                format_time($thquiz->timeclose - $thquiz->timeopen);
        }
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = thquiz_report_scale_summarks_as_percentage($value, $thquiz);
                    break;
                case 'number_format_percent':
                    $formattedvalue = thquiz_format_grade($thquiz, $value) . '%';
                    break;
                case 'number_format':
                    // 2 extra decimal places, since not a percentage,
                    // and we want the same number of sig figs.
                    $formattedvalue = format_float($value, $thquiz->decimalpoints + 2);
                    break;
                case 'number':
            ...
            $thquizinfo[get_string($property, 'thquiz_statistics',
    ...
    public function cache($qubaids) {
        ...
        $DB->delete_records('thquiz_statistics', ['hashcode' => $qubaids->get_hash_code()]);

        // Store the data.
        $DB->insert_record('thquiz_statistics', $toinsert);
        $transaction->allow_commit();
    }

    /**
     * Given a record from 'thquiz_statistics' table load the data into the properties of this class.
     *
