...
/**
 * Class to calculate and also manage caching of thquiz statistics.
 *
 * These thquiz statistics calculations are described here :
 *
 * http://docs.moodle.org/dev/Thquiz_statistics_calculations#Test_statistics
 *
...
class calculator {
     ...
     * @return calculated $thquizstats The statistics for overall attempt scores.
     */
    public function calculate($thquizid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $thquizstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($thquizid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $thquizstats->{$propertyname} = $value;
        }
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                thquiz_statistics_attempts_sql($thquizid, $groupstudentsjoins, $whichattempts);

            $thquizstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($thquizstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $thquizstats->standarddeviation = sqrt($powers->power2 / ($s - 1));

                // Skewness.
                if ($s > 2) {
                    // See http://docs.moodle.org/dev/Thquiz_item_analysis_calculations_in_practise#Skewness_and_Kurtosis.
                    $m2 = $powers->power2 / $s;
                    $m3 = $powers->power3 / $s;
                    $m4 = $powers->power4 / $s;
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $thquizstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $thquizstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $thquizstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $thquizstats->errorratio = 100 * sqrt(1 - ($thquizstats->cic / 100));
                            $thquizstats->standarderror = $thquizstats->errorratio *
                                $thquizstats->standarddeviation / 100;
                        }
                    }
            ...
            $thquizstats->cache(thquiz_statistics_qubaids_condition($thquizid, $groupstudentsjoins, $whichattempts));
        }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record('thquiz_statistics', ['hashcode' => $qubaids->get_hash_code(),
    ...
    public function get_last_calculated_time($qubaids) {
        global $DB;
        $lastcalculatedtime = $DB->get_field('thquiz_statistics', 'COALESCE(MAX(timemodified), 0)',
    ...
    /**
     * Given a particular thquiz grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'thquiz_statistics');
    }

    /**
     * Given a particular thquiz grading method return a string id for use as a field name prefix in mdl_thquiz_statistics or to
     ...
     *
     * See : http://docs.moodle.org/dev/Thquiz_item_analysis_calculations_in_practise
     ...
     */
    protected function attempt_counts_and_averages($thquizid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(thquiz_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = thquiz_statistics_attempts_sql($thquizid, $groupstudentsjoins, $which);
     ...
     * Median mark.
     *
     * http://docs.moodle.org/dev/Thquiz_statistics_calculations#Median_Score
     *
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT thquiza.id, thquiza.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
     ...
     *
     * Explanation here : http://docs.moodle.org/dev/Thquiz_item_analysis_calculations_in_practise
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((thquiza.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((thquiza.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((thquiza.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
