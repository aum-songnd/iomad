...
/**
 * Thquiz statistics report class.
 *
...
require_once($CFG->dirroot . '/mod/thquiz/report/statistics/statistics_form.php');
require_once($CFG->dirroot . '/mod/thquiz/report/statistics/statistics_table.php');
require_once($CFG->dirroot . '/mod/thquiz/report/statistics/statistics_question_table.php');
require_once($CFG->dirroot . '/mod/thquiz/report/statistics/statisticslib.php');

/**
 * The thquiz statistics report provides summary information about each question in
 * a thquiz, compared to the whole thquiz. It also provides a drill-down to more
 * detailed information about each question.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_statistics_report extends thquiz_default_report {

    /** @var context_module context of this thquiz.*/
    protected $context;

    /** @var thquiz_statistics_table instance of table class used for main questions stats table. */
    protected $table;
     ...
     * Display the report.
     */
    public function display($thquiz, $cm, $course) {
        global $OUTPUT, $DB;
        ...
        if (!thquiz_has_questions($thquiz->id)) {
            $this->print_header_and_tabs($cm, $course, $thquiz, 'statistics');
            echo thquiz_no_questions_message($thquiz, $cm, $this->context);
            return true;
        }
        ...
        $whichattempts = optional_param('whichattempts', $thquiz->grademethod, PARAM_INT);
        ...
        $reporturl = new moodle_url('/mod/thquiz/report.php', $pageoptions);

        $mform = new thquiz_statistics_settings_form($reporturl, compact('thquiz'));

        $mform->set_data(array('whichattempts' => $whichattempts, 'whichtries' => $whichtries));

        if ($whichattempts != $thquiz->grademethod) {
        ...
        if (empty($currentgroup)) {
            ...
            // All users who can attempt thquizzes and who are in the currently selected group.
            $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                    array('mod/thquiz:reviewmyattempts', 'mod/thquiz:attempt'), $currentgroup);
        ...
        $this->table = new thquiz_statistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'thquiz_statistics');
        } else {
            $report = get_string('questionstatsfilename', 'thquiz_statistics');
        }
        $courseshortname = format_string($course->shortname, true,
                array('context' => context_course::instance($course->id)));
        $filename = thquiz_report_download_filename($report, $courseshortname, $thquiz->name);
        $this->table->is_downloading($download, $filename,
                get_string('thquizstructureanalysis', 'thquiz_statistics'));
        $questions = $this->load_and_initialise_questions_for_calculations($thquiz);
        ...
        if (!$nostudentsingroup) {
            ...
            list($thquizstats, $questionstats) =
                $this->get_all_stats_and_analysis($thquiz, $whichattempts, $whichtries, $groupstudentsjoins, $questions, $progress);
            if (is_null($thquizstats)) {
                echo $OUTPUT->notification(get_string('nostats', 'thquiz_statistics'), 'error');
                return true;
            }
        } else {
            // Or create empty stats containers.
            $thquizstats = new \thquiz_statistics\calculated($whichattempts);
            $questionstats = new \core_question\statistics\questions\all_calculated_for_qubaid_condition();
        }

        // Set up the table.
        $this->table->statistics_setup($thquiz, $cm->id, $reporturl, $thquizstats->s());
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && $nostudentsingroup) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'thquiz_statistics'));
                }
            }

            if (!$this->table->is_downloading() && $thquizstats->s() == 0) {
                echo $OUTPUT->notification(get_string('nogradedattempts', 'thquiz_statistics'));
            }
        ...
        if ($everything) { // Implies is downloading.
            ...
            $thquizinfo = $thquizstats->get_formatted_thquiz_info_data($course, $cm, $thquiz);
            $this->download_thquiz_info_table($thquizinfo);

            if ($thquizstats->s()) {
                $this->output_thquiz_structure_analysis_table($questionstats);

                if ($this->table->is_downloading() == 'html' && $thquizstats->s() != 0) {
                    $this->output_statistics_graph($thquiz->id, $qubaids);
                }
            ...
            $this->output_individual_question_data($thquiz, $questionstats->for_subq($qid, $variantno));
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                              get_string('backtothquizreport', 'thquiz_statistics') . '</a>',
            ...
            if ($variantno === null &&
                ...
                if (!$this->table->is_downloading()) {
                    ...
                    echo $OUTPUT->heading(get_string('slotstructureanalysis', 'thquiz_statistics', $number), 3);
                }
                ...
                $this->output_individual_question_data($thquiz, $questionstats->for_slot($slot, $variantno));
            ...
            echo $OUTPUT->heading(get_string('thquizinformation', 'thquiz_statistics'), 3);
            echo $this->output_caching_info($thquizstats->timemodified, $thquiz->id, $groupstudentsjoins, $whichattempts, $reporturl);
            echo $this->everything_download_options($reporturl);
            $thquizinfo = $thquizstats->get_formatted_thquiz_info_data($course, $cm, $thquiz);
            echo $this->output_thquiz_info_table($thquizinfo);
            if ($thquizstats->s()) {
                echo $OUTPUT->heading(get_string('thquizstructureanalysis', 'thquiz_statistics'), 3);
                $this->output_thquiz_structure_analysis_table($questionstats);
                $this->output_statistics_graph($thquiz, $qubaids);
            }
        }
     ...
     */
    protected function output_individual_question_data($thquiz, $questionstat) {
        global $OUTPUT;

        // On-screen display. Show a summary of the question's place in the thquiz,
        ...
        $questioninfotable->data[] = array(get_string('modulename', 'thquiz'), $thquiz->name);
        $questioninfotable->data[] = array(get_string('questionname', 'thquiz_statistics'),
                $questionstat->question->name.'&nbsp;'.$datumfromtable['actions']);

        if ($questionstat->variant !== null) {
            $questioninfotable->data[] = array(get_string('variant', 'thquiz_statistics'), $questionstat->variant);

        }
        $questioninfotable->data[] = array(get_string('questiontype', 'thquiz_statistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($questionstat->question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'thquiz_statistics'),
        ...
        $labels = array(
            's' => get_string('attempts', 'thquiz_statistics'),
            'facility' => get_string('facility', 'thquiz_statistics'),
            'sd' => get_string('standarddeviationq', 'thquiz_statistics'),
            'random_guess_score' => get_string('random_guess_score', 'thquiz_statistics'),
            'intended_weight' => get_string('intended_weight', 'thquiz_statistics'),
            'effective_weight' => get_string('effective_weight', 'thquiz_statistics'),
            'discrimination_index' => get_string('discrimination_index', 'thquiz_statistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'thquiz_statistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'thquiz_statistics'), 3);
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($questionstat->question);
        echo $OUTPUT->heading(get_string('questionstatistics', 'thquiz_statistics'), 3);
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_question_preview_urls($question->questiontext, $question->id,
                ...
                $this->context->id, 'thquiz_statistics');
    ...
    protected function output_individual_question_response_analysis($question, $variantno, $s, $reporturl, $qubaids,
        ...
        $qtable = new thquiz_statistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'thquiz_statistics'), 3);
            ...
            if (!empty($question->number) && !is_null($variantno)) {
                $questiontabletitle = get_string('analysisnovariant', 'thquiz_statistics', $a);
            } else if (!empty($question->number)) {
                $questiontabletitle = get_string('analysisno', 'thquiz_statistics', $a);
            } else if (!is_null($variantno)) {
                $questiontabletitle = get_string('analysisvariant', 'thquiz_statistics', $a);
            } else {
                $questiontabletitle = get_string('analysisnameonly', 'thquiz_statistics', $a);
            }

            if ($this->table->is_downloading() == 'html') {
                $questiontabletitle = get_string('analysisofresponsesfor', 'thquiz_statistics', $questiontabletitle);
            }
    ...
    /**
     * Output the table that lists all the questions in the thquiz with their statistics.
     *
     ...
     *                                                                                               the thquiz including subqs and
     *                                                                                               variants.
     */
    protected function output_thquiz_structure_analysis_table($questionstats) {
            ...
            if (is_null($structureanalysis)) {
                ...
                foreach ($structureanalysis as $row) {
                    ...
                    if ($row instanceof \core_question\statistics\questions\calculated_question_summary) {
                        ...
                        $bgcssclass = 'thquiz_statistics-summaryrow';
    ...
    /**
     * Return HTML for table of overall thquiz statistics.
     *
     * @param array $thquizinfo as returned by {@link get_formatted_thquiz_info_data()}.
     * @return string the HTML.
     */
    protected function output_thquiz_info_table($thquizinfo) {

        $thquizinfotable = new html_table();
        $thquizinfotable->align = array('center', 'center');
        $thquizinfotable->width = '60%';
        $thquizinfotable->attributes['class'] = 'generaltable titlesleft';
        $thquizinfotable->data = array();

        foreach ($thquizinfo as $heading => $value) {
             $thquizinfotable->data[] = array($heading, $value);
        }

        return html_writer::table($thquizinfotable);
    }

    /**
     * Download the table of overall thquiz statistics.
     *
     * @param array $thquizinfo as returned by {@link get_formatted_thquiz_info_data()}.
     */
    protected function download_thquiz_info_table($thquizinfo) {
        global $OUTPUT;
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('thquizinformation', 'thquiz_statistics'));
     ...
     *
     * @param int|object $thquizorid The thquiz, or its ID.
     ...
     */
    protected function output_statistics_graph($thquizorid, $qubaids) {
        global $DB, $PAGE;

        $thquiz = $thquizorid;
        if (!is_object($thquiz)) {
            $thquiz = $DB->get_record('thquiz', array('id' => $thquizorid), '*', MUST_EXIST);
        }

        // Load the rest of the required data.
        $questions = thquiz_report_get_significant_questions($thquiz);
        ...
        $fieldstoplot = [
            'facility' => get_string('facility', 'thquiz_statistics'),
            'discriminativeefficiency' => get_string('discriminative_efficiency', 'thquiz_statistics')
        ];
        ...
        $chart->get_xaxis(0, true)->set_label(get_string('position', 'thquiz_statistics'));
        ...
        $graphname = get_string('statisticsreportgraph', 'thquiz_statistics');
        echo $output->chart($chart, $graphname);
    }

    /**
     * Get the thquiz and question statistics, either by loading the cached results,
     ...
     *                                   for each question.
     * @return array with 2 elements:    - $thquizstats The statistics for overall attempt scores.
    ...
    public function get_all_stats_and_analysis(
            $thquiz, $whichattempts, $whichtries, \core\dml\sql_join $groupstudentsjoins,
        ...
        $thquizcalc = new \thquiz_statistics\calculator($progress);
        ...
        // concurrently and causing database deadlocks. We use a long timeout here as a big thquiz with lots of attempts may
        ...
        if (!$lock) {
            ...
            $locktimeout = get_config('thquiz_statistics', 'getstatslocktimeout');
            $lock = \core\lock\lock_utils::wait_for_lock_with_progress(
                $lockfactory,
                $qubaids->get_hash_code(),
                $progress,
                $locktimeout,
                get_string('getstatslockprogress', 'thquiz_statistics'),
            );
            if (!$lock) {
        ...
        try {
            if ($thquizcalc->get_last_calculated_time($qubaids) === false) {
                if (!$calculateifrequired) {
                    $progress->progress(4);
                    $progress->end_progress();
                    $lock->release();
                ...
                $thquizstats = $thquizcalc->calculate(
                    $thquiz->id,
                    $whichattempts,
                    $groupstudentsjoins,
                    count($questions),
                ...
                $thquizstats = $thquizcalc->get_cached($qubaids);
                $progress->progress(2);
                $questionstats = $qcalc->get_cached($qubaids);
                $progress->progress(3);
            }

            if ($thquizstats->s() && $performanalysis) {
        ...
        return array($thquizstats, $questionstats);
    }
    ...
    protected function get_progress_trace_instance() {
        if ($this->progress === null) {
            if (!$this->table->is_downloading()) {
                $this->progress = new \core\progress\display_if_slow(get_string('calculatingallstats', 'thquiz_statistics'));
    ...
    /**
     * Analyse responses for all questions and sub questions in this thquiz.
     *
    ...
    /**
     * Return a little form for the user to request to download the full report, including thquiz stats and response analysis for
    ...
    protected function everything_download_options(moodle_url $reporturl) {
        global $OUTPUT;
        return $OUTPUT->download_dataformat_selector(get_string('downloadeverything', 'thquiz_statistics'),
     ...
     * @return string HTML.
     */
    protected function output_caching_info($lastcachetime, $thquizid, $groupstudentsjoins, $whichattempts, $reporturl) {
        global $DB, $OUTPUT;
        ...
        list($fromqa, $whereqa, $qaparams) = thquiz_statistics_attempts_sql($thquizid, $groupstudentsjoins, $whichattempts, true);
        $count = $DB->count_records_sql("
                SELECT COUNT(1)
                FROM $fromqa
                WHERE $whereqa
                AND thquiza.timefinish > {$lastcachetime}", $qaparams);
        ...
        $output .= get_string('lastcalculated', 'thquiz_statistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'thquiz_statistics'));
    ...
    public function clear_cached_data($qubaids) {
        global $DB;
        $DB->delete_records('thquiz_statistics', array('hashcode' => $qubaids->get_hash_code()));
    ...
    /**
     * Load the questions in this thquiz and add some properties to the objects needed in the reports.
     *
     * @param object $thquiz the thquiz.
     * @return array of questions for this thquiz.
     */
    public function load_and_initialise_questions_for_calculations($thquiz) {
        // Load the questions.
        $questions = thquiz_report_get_significant_questions($thquiz);
        $questiondata = [];
        foreach ($questions as $qs => $question) {
            if ($question->qtype === 'random') {
                $question->id = 0;
                $question->name = get_string('random', 'thquiz');
                $question->questiontext = get_string('random', 'thquiz');
    ...
    /**
     * Load question stats for a thquiz
     *
     * @param int $thquizid question usage
     * @param bool $calculateifrequired if true (the default) the stats will be calculated if not already stored.
    ...
    public function calculate_questions_stats_for_question_bank(
        ...
        global $DB;
        $thquiz = $DB->get_record('thquiz', ['id' => $thquizid], '*', MUST_EXIST);
        $questions = $this->load_and_initialise_questions_for_calculations($thquiz);

        [, $questionstats] = $this->get_all_stats_and_analysis($thquiz,
            $thquiz->grademethod, question_attempt::ALL_TRIES, new \core\dml\sql_join(),
