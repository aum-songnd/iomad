...
/**
 * Base class for the options that control what is visible in an {@link thquiz_attempts_report}.
 *
 * @package   mod_thquiz
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_thquiz_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the thquiz being reported on. */
    public $thquiz;

    /** @var object the course module objects for the thquiz being reported on. */
    public $cm;

    /** @var object the course settings for the course the thquiz is in. */
    public $course;

    /**
     * @var array form field name => corresponding thquiz_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => thquiz_attempt::IN_PROGRESS,
        'stateoverdue'    => thquiz_attempt::OVERDUE,
        'statefinished'   => thquiz_attempt::FINISHED,
        'stateabandoned'  => thquiz_attempt::ABANDONED,
    );

    /**
     * @var string thquiz_attempts_report::ALL_WITH or thquiz_attempts_report::ENROLLED_WITH
     *      thquiz_attempts_report::ENROLLED_WITHOUT or thquiz_attempts_report::ENROLLED_ALL
     */
    public $attempts = thquiz_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of thquiz_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(thquiz_attempt::IN_PROGRESS, thquiz_attempt::OVERDUE,
            thquiz_attempt::FINISHED, thquiz_attempt::ABANDONED);
    ...
    public $pagesize = thquiz_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $thquiz the settings for the thquiz being reported on.
     * @param object $cm the course module objects for the thquiz being reported on.
     * @param object $coures the course settings for the coures this thquiz is in.
     */
    public function __construct($mode, $thquiz, $cm, $course) {
        $this->mode   = $mode;
        $this->thquiz   = $thquiz;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = thquiz_report_should_show_grades($thquiz, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/thquiz/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('thquiz_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('thquiz_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == thquiz_attempts_report::ALL_WITH) {
                $this->attempts = thquiz_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = thquiz_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(thquiz_attempts_report::ALL_WITH, thquiz_attempts_report::ENROLLED_WITH,
                thquiz_attempts_report::ENROLLED_WITHOUT, thquiz_attempts_report::ENROLLED_ALL))) {
            $this->attempts = thquiz_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!thquiz_report_can_filter_only_graded($this->thquiz)) {
        ...
        if ($this->attempts == thquiz_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = thquiz_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(thquiz_attempt::FINISHED, $this->states);
    }
}
