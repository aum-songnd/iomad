...
/**
 * This file defines the setting form for the thquiz grading report.
 *
...
/**
 * Thquiz grading report settings form.
 *
 * @copyright 2010 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_grading_settings_form extends moodleform {
    ...
    /** @var stdClass $context the thquiz context. */
    protected $context;

    /**
     * thquiz_grading_settings_form constructor.
     *
    ...
    public function __construct(array $hidden, stdClass $counts, bool $shownames, bool $showcustomfields, stdClass $context) {
        ...
        parent::__construct($CFG->wwwroot . '/mod/thquiz/report.php');
    }

    protected function definition() {
        $mform = $this->_form;

        $mform->addElement('header', 'options', get_string('options', 'thquiz_grading'));
        ...
        foreach (array('needsgrading', 'manuallygraded', 'autograded', 'all') as $type) {
            ...
            $gradeoptions[$type] = get_string('gradeattempts' . $type, 'thquiz_grading',
                    $this->counts->$type);
        }
        $mform->addElement('select', 'grade', get_string('attemptstograde', 'thquiz_grading'),
                $gradeoptions);

        $mform->addElement('text', 'pagesize', get_string('questionsperpage', 'thquiz_grading'),
                array('size' => 3));
        $mform->addRule('pagesize', null, 'positiveint', null, 'client');
        $mform->setType('pagesize', PARAM_INT);

        $orderoptions = [
            'random' => get_string('random', 'thquiz_grading'),
        ...
        $mform->addElement('select', 'order', get_string('orderattemptsby', 'thquiz_grading'),
                $orderoptions);
        ...
        $mform->addElement('submit', 'submitbutton', get_string('changeoptions', 'thquiz_grading'));
    }
}
