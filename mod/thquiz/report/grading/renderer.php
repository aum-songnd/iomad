...
/**
 * Defines the renderer for the thquiz_grading module.
 *
...
/**
 * The renderer for the thquiz_grading module.
 *
 * @copyright  2018 Huong Nguyen <huongnv13@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_grading_renderer extends plugin_renderer_base {

    /**
     * Render no question notification.
     *
     * @param object $thquiz The thquiz settings.
     * @param object $cm The course-module for this thquiz.
     * @param object $context The thquiz context.
     * @return string The HTML for the no questions message.
     */
    public function render_thquiz_no_question_notification($thquiz, $cm, $context) {
        return thquiz_no_questions_message($thquiz, $cm, $context);
    }
     ...
     */
    public function render_thquiz_no_grade_question_notification() {
        return $this->notification(get_string('nothingfound', 'thquiz_grading'));
    }
    ...
    public function render_display_index_heading($linktext, $listquestionurl) {
        ...
        $output .= $this->heading(get_string('questionsthatneedgrading', 'thquiz_grading'), 3);
    ...
    public function render_questions_table($includeauto, $data, $header) {
        if (empty($data)) {
            return $this->render_thquiz_no_grade_question_notification();
        }
        $output = '';
    ...
    public function render_grade_link($counts, $type, $gradestring, $gradequestionurl) {
        ...
        if ($counts->$type > 0) {
            $output .= ' ' . html_writer::link(
                            $gradequestionurl,
                            get_string($gradestring, 'thquiz_grading'),
     ...
     * @param thquiz_grading_settings_form $filterform Question filter form.
    ...
    public function render_grading_interface($questioninfo, $listquestionsurl, $filterform, $paginginfo, $pagingbar, $formaction,
        ...
        $output .= $this->heading(get_string('gradingquestionx', 'thquiz_grading', $questioninfo), 3);

        $output .= html_writer::tag('p', html_writer::link($listquestionsurl,
                get_string('backtothelistofquestions', 'thquiz_grading')),
                ['class' => 'mdl-align']);

        $output .= $filterform->render();

        $output .= $this->heading(get_string('gradingattemptsxtoyofz', 'thquiz_grading', $paginginfo), 3);
        ...
        $output .= html_writer::tag('div', html_writer::empty_tag('input', [
                ...
                'value' => get_string('saveandnext', 'thquiz_grading')
        ]), ['class' => 'mdl-align']);
