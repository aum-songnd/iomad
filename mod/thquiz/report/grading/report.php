...
/**
 * This file defines the thquiz manual grading report class.
 *
 * @package   thquiz_grading
...
require_once($CFG->dirroot . '/mod/thquiz/report/grading/gradingsettings_form.php');


/**
 * Thquiz report to help teachers manually grade questions that need it.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_grading_report extends thquiz_default_report {
    ...
    /** @var array from thquiz_report_get_significant_questions. */
    protected $questions;
    ...
    /** @var stdClass the thquiz settings. */
    protected $thquiz;

    /** @var context the thquiz context. */
    protected $context;

    /** @var thquiz_grading_renderer Renderer of Thquiz Grading. */
    protected $renderer;
    ...
    public function display($thquiz, $cm, $course) {
        ...
        $pagesize = optional_param('pagesize',
                get_user_preferences('thquiz_grading_pagesize', self::DEFAULT_PAGE_SIZE),
                PARAM_INT);
        $page = optional_param('page', 0, PARAM_INT);
        $order = optional_param('order',
                get_user_preferences('thquiz_grading_order', self::DEFAULT_ORDER),
                PARAM_ALPHAEXT);
        ...
        require_capability('mod/thquiz:grade', $this->context);
        $shownames = has_capability('thquiz/grading:viewstudentnames', $this->context);
        // Whether the current user can see custom user fields.
        $showcustomfields = has_capability('thquiz/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this thquiz.
        $this->questions = thquiz_report_get_significant_questions($thquiz);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'thquiz_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/thquiz:reviewmyattempts', 'mod/thquiz:attempt'), $this->currentgroup);
        }

        $hasquestions = thquiz_has_questions($this->thquiz->id);
        if (!$hasquestions) {
            $this->print_header_and_tabs($cm, $course, $thquiz, 'grading');
            echo $this->renderer->render_thquiz_no_question_notification($thquiz, $cm, $this->context);
            return true;
        }
        ...
        if (!$counts || $counts->$grade == 0) {
            redirect($this->list_questions_url(), get_string('alldoneredirecting', 'thquiz_grading'));
        }
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "thquiza.thquiz = :mangrthquizid AND
                thquiza.preview = 0 AND
                thquiza.state = :statefinished";
        $params = array('mangrthquizid' => $this->cm->instance, 'statefinished' => thquiz_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND thquiza.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON thquiza.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{thquiz_attempts} thquiza $usersjoin ", 'thquiza.uniqueid', $where, $params);
    }

    /**
     * Load the thquiz_attempts rows corresponding to a list of question_usage ids.
     *
     * @param int[] $qubaids the question_usage ids of the thquiz_attempts to load.
     * @return array thquiz attempts, with added user name fields.
     */
    protected function load_attempts_by_usage_ids($qubaids) {
        global $DB;

        list($asql, $params) = $DB->get_in_or_equal($qubaids);
        $params[] = thquiz_attempt::FINISHED;
        $params[] = $this->thquiz->id;
        ...
        $attemptsbyid = $DB->get_records_sql("
                ...
                {$userfieldssql->joins}
                WHERE thquiza.uniqueid $asql AND thquiza.state = ? AND thquiza.thquiz = ?",
                $params);
    ...
    protected function display_index($includeauto) {
        ...
        $this->print_header_and_tabs($this->cm, $this->course, $this->thquiz, 'grading');
        ...
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'thquiz_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'thquiz_grading');
        }
        echo $this->renderer->render_display_index_heading($linktext, $this->list_questions_url(!$includeauto));
        $data = [];
        $header = [];

        $header[] = get_string('qno', 'thquiz_grading');
        $header[] = get_string('qtypeveryshort', 'question');
        $header[] = get_string('questionname', 'thquiz_grading');
        $header[] = get_string('tograde', 'thquiz_grading');
        $header[] = get_string('alreadygraded', 'thquiz_grading');
        if ($includeauto) {
            $header[] = get_string('automaticallygraded', 'thquiz_grading');
        }
        $header[] = get_string('total', 'thquiz_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new thquiz_grading_settings_form($hidden, $counts, $shownames, $showcustomfields, $this->context);
        ...
        if ($mform->is_submitted()) {
            if ($mform->is_validated()) {
                ...
                set_user_preference('thquiz_grading_pagesize', $pagesize);
                set_user_preference('thquiz_grading_order', $order);
                ...
                $pagesize = get_user_preferences('thquiz_grading_pagesize', self::DEFAULT_PAGE_SIZE);
            }
        }
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = thquiz_get_review_options($this->thquiz, $attempt, $this->context);
    ...
    protected function process_submitted_data(): bool {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new thquiz_attempt($attempt, $this->thquiz, $this->cm, $this->course);
            ...
            $events[] = \mod_thquiz\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby === 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON thquiza.userid = u.id {$userfieldssql->joins}";
    ...
    public function print_header_and_tabs($cm, $course, $thquiz, $reportmode = 'overview') {
        global $PAGE;
        $this->renderer = $PAGE->get_renderer('thquiz_grading');
        parent::print_header_and_tabs($cm, $course, $thquiz, $reportmode);
    }

    /**
     * Get question heading.
     *
     * @param stdClass $attempt An instance of thquiz_attempt.
    ...
    protected function get_question_heading(stdClass $attempt, bool $shownames, bool $showcustomfields): string {
        ...
        if ($shownames && $showcustomfields) {
            return get_string('gradingattemptwithcustomfields', 'thquiz_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'thquiz_grading', $a);
