...
/**
 * This file defines the thquiz overview report class.
 *
...
require_once($CFG->dirroot . '/mod/thquiz/report/attemptsreport.php');
require_once($CFG->dirroot . '/mod/thquiz/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/thquiz/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/thquiz/report/overview/overview_table.php');


/**
 * Thquiz report subclass for the overview (grades) report.
 *
 * @copyright 1999 onwards Martin Dougiamas and others {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_overview_report extends thquiz_attempts_report {
    ...
    public function display($thquiz, $cm, $course) {
        ...
        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'overview', 'thquiz_overview_settings_form', $thquiz, $cm, $course);

        $options = new thquiz_overview_options('overview', $thquiz, $cm, $course);
        ...
        $table = new thquiz_overview_table($thquiz, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
        $filename = thquiz_report_download_filename(get_string('overviewfilename', 'thquiz_overview'),
                $courseshortname, $thquiz->name);
        $table->is_downloading($options->download, $filename,
                $courseshortname . ' ' . format_string($thquiz->name, true));
        ...
        if ($options->attempts == self::ALL_WITH) {
            ...
            // groups mode, so setting allowed to empty (which means all thquiz attempts
            // are accessible, is not a security porblem.
            $allowedjoins = new \core\dml\sql_join();
        }

        $this->course = $course; // Hack to make this available in process_actions.
        $this->process_actions($thquiz, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $options->get_url());

        $hasquestions = thquiz_has_questions($thquiz->id);

        // Start output.
        if (!$table->is_downloading()) {
            // Only print headers if not asked to download data.
            $this->print_standard_header_and_messages($cm, $course, $thquiz,
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/thquiz:regrade', $this->context)) {
                    $regradesneeded = $this->count_question_attempts_needing_regrade(
                            $thquiz, $groupstudentsjoins);
                    if ($currentgroup) {
                        $a= new stdClass();
                        ...
                        $regradealldrydolabel =
                                get_string('regradealldrydogroup', 'thquiz_overview', $a);
                        $regradealldrylabel =
                                get_string('regradealldrygroup', 'thquiz_overview', $a);
                        $regradealllabel =
                                get_string('regradeallgroup', 'thquiz_overview', $a);
                    } else {
                        $regradealldrydolabel =
                                get_string('regradealldrydo', 'thquiz_overview', $regradesneeded);
                        $regradealldrylabel =
                                get_string('regradealldry', 'thquiz_overview');
                        $regradealllabel =
                                get_string('regradeall', 'thquiz_overview');
                    }
                ...
                if ($strattempthighlight = thquiz_report_highlighting_grading_method(
                        $thquiz, $this->qmsubselect, $options->onlygraded)) {
                    echo '<div class="thquizattemptcounts">' . $strattempthighlight . '</div>';
                }
            }
            ...
            $this->add_grade_columns($thquiz, $options->usercanseegrades, $columns, $headers, false);

            if (!$table->is_downloading() && has_capability('mod/thquiz:regrade', $this->context) &&
                    $this->has_regraded_questions($table->sql->from, $table->sql->where, $table->sql->params)) {
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'thquiz_overview');
            }

            if ($options->slotmarks) {
                foreach ($questions as $slot => $question) {
                    $columns[] = 'qsgrade' . $slot;
                    $header = get_string('qbrief', 'thquiz', $question->number);
                    if (!$table->is_downloading()) {
                        $header .= '<br />';
                    } else {
                        $header .= ' ';
                    }
                    $header .= '/' . thquiz_rescale_grade($question->maxmark, $thquiz, 'question');
                    $headers[] = $header;
                }
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            list($bands, $bandwidth) = self::get_bands_count_and_width($thquiz);
            $labels = self::get_bands_labels($bands, $bandwidth, $thquiz);
            ...
            if ($currentgroup && $this->hasgroupstudents) {
                $sql = "SELECT qg.id
                        ...
                        {$groupstudentsjoins->joins}
                          WHERE qg.thquiz = $thquiz->id AND {$groupstudentsjoins->wheres}";
                if ($DB->record_exists_sql($sql, $groupstudentsjoins->params)) {
                    $data = thquiz_report_grade_bands($bandwidth, $bands, $thquiz->id, $groupstudentsjoins);
                    $chart = self::get_chart($labels, $data);
                    $groupname = format_string(groups_get_group_name($currentgroup), true, [
                        'context' => $this->context,
                    ]);
                    $graphname = get_string('overviewreportgraphgroup', 'thquiz_overview', $groupname);
                    // Numerical range data should display in LTR even for RTL languages.
                    echo $output->chart($chart, $graphname, ['dir' => 'ltr']);
                }
            }

            if ($DB->record_exists('thquiz_grades', array('thquiz'=> $thquiz->id))) {
                $data = thquiz_report_grade_bands($bandwidth, $bands, $thquiz->id, new \core\dml\sql_join());
                $chart = self::get_chart($labels, $data);
                $graphname = get_string('overviewreportgraph', 'thquiz_overview');
                // Numerical range data should display in LTR even for RTL languages.
                echo $output->chart($chart, $graphname, ['dir' => 'ltr']);
            }
        }
     ...
     */
    protected function process_actions($thquiz, $cm, $currentgroup, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $allowedjoins, $redirecturl) {
        parent::process_actions($thquiz, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $redirecturl);

        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('regrade', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    $this->start_regrade($thquiz, $cm);
                    $this->regrade_attempts($thquiz, false, $groupstudentsjoins, $attemptids);
        ...
        if (optional_param('regradeall', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($thquiz, $cm);
            $this->regrade_attempts($thquiz, false, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldry', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($thquiz, $cm);
            $this->regrade_attempts($thquiz, true, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldrydo', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($thquiz, $cm);
            $this->regrade_attempts_needing_it($thquiz, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);
        }
    }
     ...
     * @param object $cm the cm object for the thquiz.
     */
    protected function start_regrade($thquiz, $cm) {
        require_capability('mod/thquiz:regrade', $this->context);
        $this->print_header_and_tabs($cm, $this->course, $thquiz, $this->mode);
    }
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'thquiz_overview'));
    ...
    /**
     * Regrade a particular thquiz attempt. Either for real ($dryrun = false), or
     * as a pretend regrade to see which fractions would change. The outcome is
     * stored in the thquiz_overview_regrades table.
     *
     ...
     *
     * @param object $attempt the thquiz attempt to regrade.
    ...
    public function regrade_attempt($attempt, $dryrun = false, $slots = null): array {
        global $DB;
        // Need more time for a thquiz with many questions.
        ...
        $messages = [];
        $finished = $attempt->state == thquiz_attempt::FINISHED;
        foreach ($slots as $slot) {
            ...
            if (abs($qqr->oldfraction - $qqr->newfraction) > 1e-7) {
                ...
                $DB->insert_record('thquiz_overview_regrades', $qqr, false);
            }
        }
    ...
    /**
     * Regrade attempts for this thquiz, exactly which attempts are regraded is
     ...
     */
    protected function regrade_attempts($thquiz, $dryrun = false,
            \core\dml\sql_join$groupstudentsjoins = null, $attemptids = array()) {
        global $DB;
        $this->unlock_session();

        $userfieldsapi = \core_user\fields::for_name();
        $sql = "SELECT thquiza.*, " . $userfieldsapi->get_sql('u', false, '', '', false)->selects . "
                  FROM {thquiz_attempts} thquiza
                  JOIN {user} u ON u.id = thquiza.userid";
        $where = "thquiz = :qid AND preview = 0";
        $params = array('qid' => $thquiz->id);
        ...
        if ($attemptids) {
            ...
            $where .= " AND thquiza.id $attemptidcondition";
        ...
        $this->regrade_batch_of_attempts($thquiz, $attempts, $dryrun, $groupstudentsjoins);
    }

    /**
     * Regrade those questions in those attempts that are marked as needing regrading
     * in the thquiz_overview_regrades table.
     * @param object $thquiz the thquiz settings.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     * for these users.
     */
    protected function regrade_attempts_needing_it($thquiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        $this->unlock_session();

        $join = '{thquiz_overview_regrades} qqr ON qqr.questionusageid = thquiza.uniqueid';
        $where = "thquiza.thquiz = :qid AND thquiza.preview = 0 AND qqr.regraded = 0";
        $params = array('qid' => $thquiz->id);
        ...
        $toregrade = $DB->get_recordset_sql("
                SELECT thquiza.uniqueid, qqr.slot
                  FROM {thquiz_attempts} thquiza
                  JOIN $join
                 WHERE $where", $params);
        ...
        $attempts = $DB->get_records_sql("
                SELECT thquiza.*, " . $userfieldsapi->get_sql('u', false, '', '', false)->selects . "
                  FROM {thquiz_attempts} thquiza
                  JOIN {user} u ON u.id = thquiza.userid
                 WHERE thquiza.uniqueid $uniqueidcondition
                ", $params);
        ...
        $this->regrade_batch_of_attempts($thquiz, $attempts, false, $groupstudentsjoins);
    }
     ...
     * Each object in the attempts array must be a row from the thquiz_attempts
     ...
     *
     * @param object $thquiz the thquiz settings.
     * @param array $attempts of data from the thquiz_attempts table, with extra data as above.
     * @param bool $dryrun if true, do a pretend regrade, otherwise do it for real.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     */
    protected function regrade_batch_of_attempts($thquiz, array $attempts,
            bool $dryrun, \core\dml\sql_join $groupstudentsjoins) {
        global $OUTPUT;
        $this->clear_regrade_table($thquiz, $groupstudentsjoins);

        $progressbar = new progress_bar('thquiz_overview_regrade', 500, true);
        $a = array(
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofywithdetails', 'thquiz_overview', $a));
            $messages = $this->regrade_attempt($attempt, $dryrun, $attempt->regradeonlyslots);
            if ($messages) {
                $items = [];
                foreach ($messages as $slot => $message) {
                    $items[] = get_string('regradingattemptissue', 'thquiz_overview',
                            ['slot' => $slot, 'reason' => $message]);
                }
                echo $OUTPUT->notification(
                        html_writer::tag('p', get_string('regradingattemptxofyproblem', 'thquiz_overview', $a)) .
                        html_writer::alist($items), \core\output\notification::NOTIFY_WARNING);
            }
        }
        $progressbar->update($a['done'], $a['count'],
                get_string('regradedsuccessfullyxofy', 'thquiz_overview', $a));

        if (!$dryrun) {
            $this->update_overall_grades($thquiz);
        }
    }
     ...
     */
    protected function count_question_attempts_needing_regrade($thquiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        if ($this->hasgroupstudents) {
            $userjoin = "JOIN {user} u ON u.id = thquiza.userid
                    {$groupstudentsjoins->joins}";
            $usertest = "{$groupstudentsjoins->wheres} AND u.id = thquiza.userid AND ";
            $params = $groupstudentsjoins->params;
        }

        $params['cthquiz'] = $thquiz->id;
        $sql = "SELECT COUNT(DISTINCT thquiza.id)
                  FROM {thquiz_attempts} thquiza
                  JOIN {thquiz_overview_regrades} qqr ON thquiza.uniqueid = qqr.questionusageid
                $userjoin
                 WHERE
                      $usertest
                      thquiza.thquiz = :cthquiz AND
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_sql("
                SELECT 1
                  FROM {$from}
                  JOIN {thquiz_overview_regrades} qor ON qor.questionusageid = thquiza.uniqueid
                 WHERE {$where}", $params);
    }
     ...
     * to these users is cleared.
     */
    protected function clear_regrade_table($thquiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        $select = "questionusageid IN (
                    SELECT uniqueid
                      FROM {thquiz_attempts} thquiza";
        $where = "WHERE thquiza.thquiz = :qid";
        $params = array('qid' => $thquiz->id);
        if ($this->hasgroupstudents && !empty($groupstudentsjoins->joins)) {
            $select .= "\nJOIN {user} u ON u.id = thquiza.userid
                    {$groupstudentsjoins->joins}";
        ...
        $DB->delete_records_select('thquiz_overview_regrades', $select, $params);
    }
     ...
     */
    protected function update_overall_grades($thquiz) {
        thquiz_update_all_attempt_sumgrades($thquiz);
        thquiz_update_all_final_grades($thquiz);
        thquiz_update_grades($thquiz);
    }

    /**
     * Get the bands configuration for the thquiz.
     *
     * This returns the configuration for having between 11 and 20 bars in
     * a chart based on the maximum grade to be given on a thquiz. The width of
     ...
     */
    public static function get_bands_count_and_width($thquiz) {
        $bands = $thquiz->grade;
        ...
        return [$bands, $thquiz->grade / $bands];
    }
     ...
     */
    public static function get_bands_labels($bands, $bandwidth, $thquiz) {
        $bandlabels = [];
        for ($i = 1; $i <= $bands; $i++) {
            $bandlabels[] = thquiz_format_grade($thquiz, ($i - 1) * $bandwidth) . ' - ' . thquiz_format_grade($thquiz, $i * $bandwidth);
        }
        return $bandlabels;
    }
