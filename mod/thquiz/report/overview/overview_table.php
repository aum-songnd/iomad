...
/**
 * This file defines the thquiz grades table.
 *
...
require_once($CFG->dirroot . '/mod/thquiz/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the thquiz grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_overview_table extends thquiz_attempts_report_table {
     ...
     * @param thquiz_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($thquiz, $context, $qmsubselect,
            thquiz_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-thquiz-report-overview-report', $thquiz , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the thquiz.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(thquizaouter.sumgrades) AS grade, COUNT(thquizaouter.sumgrades) AS numaveraged
                  FROM {thquiz_attempts} thquizaouter
                  JOIN (
                       SELECT DISTINCT thquiza.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON thquizaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = thquiz_rescale_grade($record->grade, $this->thquiz, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext' => strip_tags(thquiz_report_feedback_for_grade(
                                         $record->grade, $this->thquiz->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{thquiz_attempts} thquizaouter
                  JOIN (
                       SELECT DISTINCT thquiza.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON thquizaouter.id = relevant_attempt_ids.id",
                    'thquizaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = thquiz_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->thquiz, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = thquiz_format_question_grade($this->thquiz, $record->grade);
        } else {
            $average = thquiz_format_grade($this->thquiz, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/thquiz:regrade', $this->context)) {
            $regradebuttonparams = [
                ...
                'name'  => 'regrade',
                'value' => get_string('regradeselected', 'thquiz_overview'),
                'data-action' => 'toggle',
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = thquiz_rescale_grade($attempt->sumgrades, $this->thquiz);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = thquiz_rescale_grade($newsumgrade, $this->thquiz);
            $oldsumgrade = thquiz_rescale_grade($oldsumgrade, $this->thquiz);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/thquiz/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'thquiz')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/thquiz/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = thquiz_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->thquiz, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = thquiz_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->thquiz, 'question');
            $oldgrade = thquiz_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'thquiz_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'thquiz_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {thquiz_overview_regrades} qqr
                                 WHERE qqr.questionusageid = thquiza.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/thquiz:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('thquiz_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return thquiz_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
