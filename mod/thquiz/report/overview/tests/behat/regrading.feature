@mod @mod_thquiz @thquiz @thquiz_overview @javascript
Feature: Regrading thquiz attempts using the Grades report
    In order to be able to correct mistakes I made setting up my thquiz
    As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | thquiz     | Thquiz for testing regrading | C1     | thquiz1    |
    And the following "questions" exist:
      | questioncategory | qtype       | name |
      | Test questions   | truefalse   | TF   |
      | Test questions   | shortanswer | SA   |
    And thquiz "Thquiz for testing regrading" contains the following questions:
      | question | page | maxmark |
      | TF       | 1    | 5.0     |
      | SA       | 1    | 5.0     |
    And user "student1" has attempted "Thquiz for testing regrading" with responses:
      | slot | response |
      | 1    | True     |
      | 2    | frog     |
    And user "student2" has attempted "Thquiz for testing regrading" with responses:
      | slot | response |
      | 1    | True     |
      | 2    | toad     |

  Scenario: Regrade all attempts
    Given I am on the "Thquiz for testing regrading" "thquiz activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    When I press "Regrade all"

    # Note, the order is not defined, so we can only check part of the message.
    # Also, nothing has changed in the thquiz, so the regrade won't alter any scores,
    # but this is still a useful test that the regrade process completes without errors.
    Then I should see "Thquiz for testing regrading"
    ...
    And I should see "Thquiz for testing regrading"
  ...
  Scenario: Dry-run a full regrade, then regrade the attempts that will need it.
    Given I am on the "Thquiz for testing regrading" "mod_thquiz > edit" page logged in as teacher
  ...
  Scenario: Regrade all attempts works against thquiz selected question version
    Given I am on the "Thquiz for testing regrading" "thquiz activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    ...
    And I should see "Overall number of students achieving grade ranges"
    And "Student One" row "Regrade" column of "attempts" table should not contain "Needed"
    And I am on the "Thquiz for testing regrading" "mod_thquiz > question bank" page
    And I choose "Edit question" action for "TF" in the question bank
    And I set the field "Correct answer" to "False"
    And I press "id_submitbutton"
    And I am on the "Thquiz for testing regrading" "mod_thquiz > edit" page
  ...
  Scenario: Regrade all attempts works against thquiz selected latest question version
    Given I am on the "Thquiz for testing regrading" "thquiz activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    And I click on "mod-thquiz-report-overview-report-selectall-attempts" "checkbox"
    ...
    And I set the field "version" in the "TF" "list_item" to "v1"
    And user "student3" has attempted "Thquiz for testing regrading" with responses:
      | slot | response |
      | 1    | True     |
      | 2    | toad     |
    And I am on the "Thquiz for testing regrading" "mod_thquiz > edit" page
  ...
  Scenario: Regrade attempts should always regrade against latest random question version
    ...
    And I am on the "Thquiz for testing regrading" "thquiz activity" page logged in as student3
    And I click on "Attempt thquiz" "button"
    ...
    And I am on the "Thquiz for testing regrading" "mod_thquiz > question bank" page logged in as teacher
    And I choose "Edit question" action for "TF" in the question bank
    And I set the field "Correct answer" to "False"
    And I press "id_submitbutton"
    And I navigate to "Results" in current page administration
