...
/**
 * Classes to enforce the various access rules that can apply to a thquiz.
 *
 * @package   mod_thquiz
 ...
 * thquiz, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class thquiz_access_manager {
    /** @var thquiz the thquiz settings object. */
    protected $thquizobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of thquiz_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular thquiz.
     * @param object $thquizobj An instance of the class thquiz from attemptlib.php.
     *      The thquiz we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/thquiz:ignoretimelimits', ...)).
     */
    public function __construct($thquizobj, $timenow, $canignoretimelimits) {
        $this->thquizobj = $thquizobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($thquizobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular thquiz.
     ...
     * @return array of {@link thquiz_access_rule_base}s.
     */
    protected function make_rules($thquizobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($thquizobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['thquizaccess_' . $superceededrule]);
        }

        return $rules;
    }

    public function is_rule_seb() {
        if(isset($this->rules['thquizaccess_seb'])){
            return true;
        } else {
            return false;
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('thquizaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the thquiz table.
     *
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($thquizform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'thquiz'));
        foreach (self::get_rule_classes() as $rule) {
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $thquizform);
        }
    ...
    /**
     * Save any submitted settings when the thquiz settings form is submitted.
     *
    ...
    public static function save_settings($thquiz) {
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($thquiz);
        }
    }

    /**
     * Delete any rule-specific settings when the thquiz is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the thquiz table.
     *
     ...
     */
    protected static function get_load_sql($thquizid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{thquiz} thquiz';
        $allparams = array('thquizid' => $thquizid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($thquizid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE thquiz.id = :thquizid", $allparams);
    }
     ...
     */
    public static function load_settings($thquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($thquizid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($thquizid);
        }
    ...
    /**
     * Load the thquiz settings and any settings required by the access rules.
     ...
     *
     * @param int $thquizid the thquiz id.
     * @return object mdl_thquiz row with extra fields.
     */
    public static function load_thquiz_and_settings($thquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($thquizid, $rules, 'thquiz.*');
        $thquiz = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($thquizid) as $name => $value) {
                $thquiz->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this thquiz, such
     * as is shown at the top of the thquiz view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this thquiz now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_thquiz_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_thquiz_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_thquiz_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'thquizobj' => $this->thquizobj,
     ...
     * thquiz. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant thquiz_attempts row.
    ...
    /**
     * @return bolean if this thquiz should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the thquiz view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_thquiz_renderer $output the thquiz renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->thquizobj->view_url(), $message);
            die();
        } else {
            redirect($this->thquizobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the thquiz, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(thquiz_attempt::IN_PROGRESS, thquiz_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = thquiz_attempt_state($this->thquizobj->get_thquiz(), $attempt);
        $reviewoptions = mod_thquiz_display_options::make_from_thquiz(
                $this->thquizobj->get_thquiz(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->thquizobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->thquizobj->review_url($attempt->id),
