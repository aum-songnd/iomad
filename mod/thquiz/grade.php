...
$reportlist = thquiz_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see thquiz reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/thquiz/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = thquiz_get_user_attempts($thquiz->id, $userid, 'finished');
    $attempt = null;
...
redirect(new moodle_url('/mod/thquiz/report.php', array(
