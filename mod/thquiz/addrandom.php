...
/**
 * Fallback page of /mod/thquiz/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $thquiz, $pagevars) =
        question_edit_setup('editq', '/mod/thquiz/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/thquiz/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new thquiz_add_random_form(new moodle_url('/mod/thquiz/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    thquiz_add_random_questions($thquiz, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditingthquiz = get_string('editinga', 'moodle', get_string('modulename', 'thquiz'));
$PAGE->navbar->add($streditingthquiz);
$PAGE->set_title($streditingthquiz);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$thquizname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            throw new \moodle_exception('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontothquiz', 'thquiz', $thquizname), 2);
$mform->display();
echo $OUTPUT->footer();
