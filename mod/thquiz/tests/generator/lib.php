...
/**
 * Thquiz module test data generator class
 *
 * @package mod_thquiz
 * @copyright 2012 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_thquiz_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        $defaultthquizsettings = array(
            ...
            'showuserpicture'        => 0,
            'showblocks'             => 0,
            'navmethod'              => THQUIZ_NAVMETHOD_FREE,
        );

        foreach ($defaultthquizsettings as $name => $value) {
    ...
    /**
     * Create a thquiz attempt for a particular user at a particular course.
     *
     * @param int $thquizid the thquiz id (from the mdl_quit table, not cmid).
     ...
     */
    public function create_attempt($thquizid, $userid, array $forcedrandomquestions = [],
            array $forcedvariants = []) {
        // Build thquiz object and load questions.
        $thquizobj = thquiz::create($thquizid, $userid);

        $attemptnumber = 1;
        $attempt = null;

        if ($attempts = thquiz_get_user_attempts($thquizid, $userid, 'all', true)) {
        ...
        return thquiz_prepare_and_start_new_attempt($thquizobj, $attemptnumber, $attempt, false,
                $forcedrandomquestions, $forcedvariants);
    }

    /**
     * Submit responses to a thquiz attempt.
     *
    ...
    /**
     * Create a thquiz override (either user or group).
     *
     * @param array $data must specify thquizid, and one of userid or groupid.
     */
    public function create_override(array $data): void {
        global $DB;

        // Validate.
        if (!isset($data['thquiz'])) {
            throw new coding_exception('Must specify thquiz (id) when creating a thquiz override.');
        }

        if (!isset($data['userid']) && !isset($data['groupid'])) {
            throw new coding_exception('Must specify one of userid or groupid when creating a thquiz override.');
        }

        if (isset($data['userid']) && isset($data['groupid'])) {
            throw new coding_exception('Cannot specify both userid and groupid when creating a thquiz override.');
        }

        // Create the override.
        $DB->insert_record('thquiz_overrides', (object) $data);

        // Update any associated calendar events, if necessary.
        thquiz_update_events($DB->get_record('thquiz', ['id' => $data['thquiz']], '*', MUST_EXIST));
    }
}
