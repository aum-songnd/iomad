...
class structure_test extends \advanced_testcase {
    ...
    /**
     * Create a course with an empty thquiz.
     * @return array with three elements thquiz, cm and course.
     */
    protected function prepare_thquiz_data() {
     ...
     * @return thquiz the created thquiz.
     */
    protected function create_test_thquiz($layout) {
        list($thquiz, $cm, $course) = $this->prepare_thquiz_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                thquiz_add_thquiz_question($q->id, $thquiz, $page);
                $lastpage = $page;
            }
        }
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_thquiz()}.
     * @param structure $structure the structure to test.
     */
    protected function assert_thquiz_layout($expectedlayout, structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first thquiz section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The thquiz contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the thquiz.');
            }
        }
    }
     ...
     * used by create_test_thquiz as assert_thquiz_layout.
    ...
    public function test_get_thquiz_slots() {
        $thquizobj = $this->create_test_thquiz(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_thquiz_has_one_section_by_default() {
        $thquizobj = $this->create_test_thquiz(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = structure::create_for_thquiz($thquizobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = structure::create_for_thquiz($thquizobj);
        $this->assert_thquiz_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_thquiz_remove_slot() {
        $thquizobj = $this->create_test_thquiz(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_thquiz_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        thquiz_add_random_questions($thquizobj->get_thquiz(), 1, $cat->id, 1, false);
        $structure = structure::create_for_thquiz($thquizobj);
        $sql = 'SELECT qsr.*
                 FROM {question_set_references} qsr
                 JOIN {thquiz_slots} qs ON qs.id = qsr.itemid
                 WHERE qs.thquizid = ?
                   AND qsr.component = ?
                   AND qsr.questionarea = ?';
        $randomq = $DB->get_record_sql($sql, [$thquizobj->get_thquizid(), 'mod_thquiz', 'slot']);
        ...
        $this->assertFalse($DB->record_exists('question_set_references',
            array('id' => $randomq->id, 'component' => 'mod_thquiz', 'questionarea' => 'slot')));
    }
    ...
    public function test_can_remove_last_question_in_a_thquiz() {
        $thquizobj = $this->create_test_thquiz(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        thquiz_add_thquiz_question($q->id, $thquizobj->get_thquiz(), 0);
        $structure = structure::create_for_thquiz($thquizobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        thquiz_add_thquiz_question($q->id, $thquizobj->get_thquiz(), 1);
    ...
    public function test_can_add_random_questions() {
        ...
        $thquiz = $this->create_test_thquiz([]);
        $course = $thquiz->get_course();
        ...
        $structure = structure::create_for_thquiz($thquiz);
    ...
    public function test_get_version_choices_for_slot() {
        ...
        $thquizobj = $this->create_test_thquiz([]);

        // Create a question with two versions.
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category(['contextid' => $thquizobj->get_context()->id]);
        ...
        thquiz_add_thquiz_question($q->id, $thquizobj->get_thquiz());
