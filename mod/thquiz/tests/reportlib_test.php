...
class reportlib_test extends \advanced_testcase {
    public function test_thquiz_report_index_by_keys() {
        $datum = array();
        $object = new \stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = thquiz_report_index_by_keys($datum, array('aid', 'qid'));
        ...
        $indexed = thquiz_report_index_by_keys($datum, array('aid', 'qid'), false);
    ...
    public function test_thquiz_report_scale_summarks_as_percentage() {
        $thquiz = new \stdClass();
        $thquiz->sumgrades = 10;
        $thquiz->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            thquiz_report_scale_summarks_as_percentage(1.234567, $thquiz, false));
        $this->assertEquals('12.35%',
            thquiz_report_scale_summarks_as_percentage(1.234567, $thquiz, true));
        $this->assertEquals('-',
            thquiz_report_scale_summarks_as_percentage('-', $thquiz, true));
    }

    public function test_thquiz_report_qm_filter_select_only_one_attempt_allowed() {
        $thquiz = new \stdClass();
        $thquiz->attempts = 1;
        $this->assertSame('', thquiz_report_qm_filter_select($thquiz));
    }

    public function test_thquiz_report_qm_filter_select_average() {
        $thquiz = new \stdClass();
        $thquiz->attempts = 10;
        $thquiz->grademethod = THQUIZ_GRADEAVERAGE;
        $this->assertSame('', thquiz_report_qm_filter_select($thquiz));
    }

    public function test_thquiz_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new \stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->thquiz = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = thquiz_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | thquizid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('thquiz_attempts', $fakeattempt);
        ...
        $fakeattempt->state = thquiz_attempt::IN_PROGRESS;
        $DB->insert_record('thquiz_attempts', $fakeattempt);
        ...
        $thquiz->grademethod = THQUIZ_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {thquiz_attempts} thquiza WHERE userid = ? AND thquiz = ? AND "
                        . thquiz_report_qm_filter_select($thquiz), array(123, 456));
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $thquiz->grademethod = THQUIZ_ATTEMPTLAST;
        ...
        $thquiz->attempts = 0;
        $thquiz->grademethod = THQUIZ_GRADEHIGHEST;
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {thquiz_attempts} qa_alias WHERE userid = ? AND thquiz = ? AND "
                . thquiz_report_qm_filter_select($thquiz, 'qa_alias'), array(123, 456));
        $this->assertEquals(1, count($bestattempt));
        $bestattempt = reset($bestattempt);
        $this->assertEquals(2, $bestattempt->attempt);
    }

    public function test_thquiz_results_never_below_zero() {
        global $DB;
        $this->resetAfterTest();

        $thquizid = 7;
        $fakegrade = new \stdClass();
        $fakegrade->thquiz = $thquizid;

        // Have 5 test grades.
        $fakegrade->userid = 10;
        $fakegrade->grade = 6.66667;
        $DB->insert_record('thquiz_grades', $fakegrade);
        ...
        $data = thquiz_report_grade_bands(5, 20, $thquizid);
