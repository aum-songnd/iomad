...
use grade_item;
use mod_thquiz\completion\custom_completion;
use question_engine;
use thquiz;
use thquiz_attempt;
...
/**
 * Class for unit testing mod_thquiz/custom_completion.
 *
 * @package   mod_thquiz
 * @copyright 2021 Shamim Rezaie <shamim@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \mod_thquiz\completion\custom_completion
 */
class custom_completion_test extends advanced_testcase {
     ...
     * @return array [$students, $thquiz, $cm, $litecm]
     */
    private function setup_thquiz_for_testing_completion(array $completionoptions): array {
        global $CFG, $DB;
        ...
        $litecm = get_coursemodule_from_id('thquiz', $thquiz->cmid);
     ...
     *
     * @param array $attemptoptions ['thquiz'] => object, ['student'] => object, ['tosubmit'] => array, ['attemptnumber'] => int
     */
    private function do_attempt_thquiz(array $attemptoptions) {
    ...
    /**
     * Test checking the completion state of a thquiz base on core's completionpassgrade criteria.
     * The thquiz requires a passing grade to be completed.
     */
    public function test_completionpass() {
        list($students, $thquiz, $cm) = $this->setup_thquiz_for_testing_completion([
            'nbstudents' => 2,
            'qtype' => 'numerical',
            'thquizoptions' => [
    ...
    public function test_update_moduleinfo() {
        ...
        list($students, $thquiz, $cm, $litecm) = $this->setup_thquiz_for_testing_completion([
            'nbstudents' => 1,
            'qtype' => 'numerical',
            'nbquestions' => 2,
            'sumgrades' => 100,
        ...
        // Update thquiz with passgrade = 20 and use highest grade to calculate.
        $moduleinfo = $this->prepare_module_info($cm, $thquiz, $course, 20, THQUIZ_GRADEHIGHEST);
        ...
        // Update thquiz with gradepass = 80 and use highest grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $thquiz, $course, 80, THQUIZ_GRADEHIGHEST);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
        ...
        // Update thquiz with gradepass = 80 and use average grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $thquiz, $course, 80, THQUIZ_GRADEAVERAGE);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
        ...
        // Update thquiz with gradepass = 50 and use average grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $thquiz, $course, 50, THQUIZ_GRADEAVERAGE);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
        ...
        // Update thquiz with gradepass = 50 and use first attempt grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $thquiz, $course, 50, THQUIZ_ATTEMPTFIRST);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
        ...
        // Update thquiz with gradepass = 50 and use last attempt grade to calculate completion.
        $moduleinfo = $this->prepare_module_info($cm, $thquiz, $course, 50, THQUIZ_ATTEMPTLAST);
        update_moduleinfo($litecm, $moduleinfo, $course, null);
    ...
    /**
     * Set up moduleinfo object sample data for thquiz instance.
     *
     * @param object $cm course-module instance
     * @param object $thquiz thquiz instance data.
     ...
     * @return \stdClass
     */
    private function prepare_module_info(object $cm, object $thquiz, object $course, int $gradepass, string $grademethod): \stdClass {
        ...
        $moduleinfo->modulename = 'thquiz';
        $moduleinfo->thquizpassword = '';
        ...
        $moduleinfo->name = $thquiz->name;
        $moduleinfo->timeopen = $thquiz->timeopen;
        $moduleinfo->timeclose = $thquiz->timeclose;
        $moduleinfo->timelimit = $thquiz->timelimit;
        $moduleinfo->graceperiod = $thquiz->graceperiod;
        $moduleinfo->decimalpoints = $thquiz->decimalpoints;
        $moduleinfo->questiondecimalpoints = $thquiz->questiondecimalpoints;
