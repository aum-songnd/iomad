...
class restore_date_test extends \restore_date_testcase {
    ...
    public function test_restore_dates() {
        ...
        list($course, $thquiz) = $this->create_course_and_module('thquiz', $record);
        ...
        $attempt = thquiz_create_attempt($thquizobj, 1, false, $timestamp, false);
        ...
        thquiz_start_new_attempt($thquizobj, $quba, $attempt, 1, $timestamp);
        ...
        $grade->id = $DB->insert_record('thquiz_grades', $grade);
        ...
        $DB->insert_record('thquiz_overrides', $override);

        // Set time fields to a constant for easy validation.
        $DB->set_field('thquiz_attempts', 'timefinish', $timestamp);

        // Do backup and restore.
        $newcourseid = $this->backup_and_restore($course);
        $newthquiz = $DB->get_record('thquiz', ['course' => $newcourseid]);

        $this->assertFieldsNotRolledForward($thquiz, $newthquiz, ['timecreated', 'timemodified']);
        $props = ['timeclose', 'timeopen'];
        $this->assertFieldsRolledForward($thquiz, $newthquiz, $props);

        $newattempt = $DB->get_record('thquiz_attempts', ['thquiz' => $newthquiz->id]);
        $newoverride = $DB->get_record('thquiz_overrides', ['thquiz' => $newthquiz->id]);
        $newgrade = $DB->get_record('thquiz_grades', ['thquiz' => $newthquiz->id]);
