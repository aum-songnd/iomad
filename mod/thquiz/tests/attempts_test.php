...
use mod_thquiz_overdue_attempt_updater;
use question_engine;
use thquiz;
...
/**
 * Unit tests for thquiz attempt overdue handling
 *
 * @package    mod_thquiz
 * @category   test
...
class attempts_test extends \advanced_testcase {
    ...
    /**
     * Test the functions thquiz_update_open_attempts(), get_list_of_overdue_attempts() and
     * update_overdue_attempts().
     */
    public function test_bulk_update_functions() {
        global $DB,$CFG;

        require_once($CFG->dirroot.'/mod/thquiz/cronlib.php');
        ...
        // Basic thquiz settings

        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        $usertimes[$attemptid] = ['timeclose' => 1200, 'timelimit' => 600, 'message' => 'Test1A', 'time1000state' => 'finished'];

        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 1800]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        $usertimes[$attemptid] = ['timeclose' => 1200, 'timelimit' => 1800, 'message' => 'Test1B', 'time1000state' => 'inprogress'];

        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        $usertimes[$attemptid] = ['timeclose' => 1200, 'timelimit' => 0, 'message' => 'Test1C', 'time1000state' => 'inprogress'];

        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 0, 'timelimit' => 600]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        $usertimes[$attemptid] = ['timeclose' => 0, 'timelimit' => 600, 'message' => 'Test1D', 'time1000state' => 'finished'];

        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 0, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => null]);
        $usertimes[$attemptid] = ['timeclose' => 1300, 'timelimit' => 0, 'message' => 'Test2A', 'time1000state' => 'inprogress'];

        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 0]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1100, 'timelimit' => null]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => null, 'timelimit' => 700]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 0, 'timelimit' => 600]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => null, 'timelimit' => 500]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 0, 'timelimit' => 600]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => null, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        $usertimes[$attemptid] = ['timeclose' => 1300, 'timelimit' => 500, 'message' => 'Test2F', 'time1000state' => 'finished'];
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 1000, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz), 'attempt' => 1]);
        $usertimes[$attemptid] = ['timeclose' => 1300, 'timelimit' => 500, 'message' => 'Test2G', 'time1000state' => 'inprogress'];

        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group3->id, 'timeclose' => 1300, 'timelimit' => 500]); // User not in group.
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 501]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group2->id, 'timeclose' => 1301, 'timelimit' => 500]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1301, 'timelimit' => 500]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group2->id, 'timeclose' => 1300, 'timelimit' => 501]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group2->id, 'timeclose' => 1300, 'timelimit' => 501]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group3->id, 'timeclose' => 1500, 'timelimit' => 1000]); // User not in group.
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group2->id, 'timeclose' => null, 'timelimit' => 501]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group2->id, 'timeclose' => 1301, 'timelimit' => null]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group2->id, 'timeclose' => 1301, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 500]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group2->id, 'timeclose' => 0, 'timelimit' => 501]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'timeclose' => 1200, 'timelimit' => 600]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'timeclose' => 1201, 'timelimit' => 601]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'timeclose' => 0, 'timelimit' => 601]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'timeclose' => 1201, 'timelimit' => 0]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'timeclose' => null, 'timelimit' => 601]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'timeclose' => 1201, 'timelimit' => null]);
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group1->id, 'timeclose' => 1300, 'timelimit' => 700]);
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'userid' => 0, 'timeclose' => 1201, 'timelimit' => 601]); // Not user.
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'inprogress',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        $attemptid = $DB->insert_record('thquiz_attempts', ['thquiz' => $thquiz->id, 'userid' => $user1->id, 'state' => 'overdue',
                'timestart' => 100, 'timecheckstate' => 0, 'layout' => '', 'uniqueid' => $this->usage_id($thquiz)]);
        ...
        foreach ($usertimes as $attemptid => $times) {
            $attempt = $DB->get_record('thquiz_attempts', ['id' => $attemptid], '*', MUST_EXIST);
        ...
        //
        // Test thquiz_update_open_attempts().
        //

        thquiz_update_open_attempts(['courseid' => $course->id]);
        foreach ($usertimes as $attemptid => $times) {
            $attempt = $DB->get_record('thquiz_attempts', ['id' => $attemptid], '*', MUST_EXIST);

            if ($attempt->state == 'overdue') {
                $graceperiod = $DB->get_field('thquiz', 'graceperiod', ['id' => $attempt->thquiz]);
            } else {
                $graceperiod = 0;
            }
        ...
        $overduehander = new mod_thquiz_overdue_attempt_updater();
        ...
        $attempts->close();
        $this->assertEquals($DB->count_records_select('thquiz_attempts', 'timecheckstate IS NOT NULL'), $count);
        ...
        [$count, $thquizcount] = $overduehander->update_overdue_attempts(1000, 940);

        $attempts = $DB->get_records('thquiz_attempts', null, 'thquiz, userid, attempt',
                'id, thquiz, userid, attempt, state, timestart, timefinish, timecheckstate');
        foreach ($attempts as $attempt) {
            $this->assertTrue(isset($usertimes[$attempt->id]));
            $times = $usertimes[$attempt->id];
        ...
        $this->assertEquals(19, $thquizcount);
    }

    /**
     * Make any old question usage for a thquiz.
     *
     ...
     */
    protected function usage_id(\stdClass $thquiz): int {
        $quba = question_engine::make_questions_usage_by_activity('mod_thquiz',
                \context_module::instance($thquiz->cmid));
    ...
    public function test_group_event_handlers() {
        ...
        thquiz_update_open_attempts(['thquizid' => $thquiz->id]);
        $this->assertEquals(1300, $DB->get_field('thquiz_attempts', 'timecheckstate', ['id' => $attemptid]));

        // remove from group
        $this->assertTrue(groups_remove_member($group1, $user1));
        $this->assertEquals(1200, $DB->get_field('thquiz_attempts', 'timecheckstate', ['id' => $attemptid]));
        ...
        $this->assertEquals(1200, $DB->get_field('thquiz_attempts', 'timecheckstate', ['id' => $attemptid]));
        $this->assertEquals(0, $DB->count_records('thquiz_overrides', ['thquiz' => $thquiz->id]));

        // add a group2 override
        $DB->insert_record('thquiz_overrides',
                ['thquiz' => $thquiz->id, 'groupid' => $group2->id, 'timeclose' => 1400, 'timelimit' => null]);
        thquiz_update_open_attempts(['thquizid' => $thquiz->id]);
        $this->assertEquals(1400, $DB->get_field('thquiz_attempts', 'timecheckstate', ['id' => $attemptid]));
    ...
    /**
     * Test the functions thquiz_create_attempt_handling_errors
     */
    public function test_thquiz_create_attempt_handling_errors() {
        ...
        $thquiz = $thquizgenerator->create_instance(['course' => $course->id, 'questionsperpage' => 0, 'grade' => 100.0,
            'sumgrades' => 2]);
        // Create questions.
        ...
        $thquizobj = thquiz::create($thquiz->id, $user1->id);
        ...
        $timenow = time();
        // Create an attempt.
        $attempt = thquiz_create_attempt($thquizobj, 1, null, $timenow, false, $user1->id);
        thquiz_start_new_attempt($thquizobj, $quba, $attempt, 1, $timenow);
        thquiz_attempt_save_started($thquizobj, $quba, $attempt);
        $result = thquiz_create_attempt_handling_errors($attempt->id, $thquiz->cmid);
        $this->assertEquals($result->get_attemptid(), $attempt->id);
        try {
            $result = thquiz_create_attempt_handling_errors($attempt->id, 9999);
            $this->fail('Exception expected due to invalid course module id.');
        } catch (\moodle_exception $e) {
            $this->assertEquals('invalidcoursemodule', $e->errorcode);
        }
        try {
            thquiz_create_attempt_handling_errors(9999, $result->get_cmid());
            $this->fail('Exception expected due to thquiz content change.');
        } catch (\moodle_exception $e) {
            $this->assertEquals('attempterrorcontentchange', $e->errorcode);
        }
        try {
            thquiz_create_attempt_handling_errors(9999);
            $this->fail('Exception expected due to invalid thquiz attempt id.');
        } catch (\moodle_exception $e) {
        ...
        try {
            thquiz_create_attempt_handling_errors(9999, $result->get_cmid());
            $this->fail('Exception expected due to thquiz content change for user without permission.');
        } catch (\moodle_exception $e) {
            $this->assertEquals('attempterrorcontentchangeforuser', $e->errorcode);
        }
        try {
            thquiz_create_attempt_handling_errors($attempt->id, 9999);
