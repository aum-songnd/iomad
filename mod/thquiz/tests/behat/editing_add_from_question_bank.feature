@mod @mod_thquiz @javascript
Feature: Adding questions to a thquiz from the question bank
  ...
  Scenario: Questions are added in the right place with multiple sections
    ...
    And I log in as "teacher1"
    And I am on the "Thquiz 1" "mod_thquiz > Edit" page
    When I open the "Page 1" add to thquiz menu
    And I follow "from question bank"
    And I set the field with xpath "//tr[contains(normalize-space(.), 'question 03 name')]//input[@type='checkbox']" to "1"
    And I click on "Add selected questions to the thquiz" "button"
    Then I should see "question 03 name" on thquiz page "1"
    And I should see "question 01 name" before "question 03 name" on the edit thquiz page

  Scenario: Add several selected questions from the question bank
    Given I am on the "Thquiz 1" "mod_thquiz > Edit" page logged in as "teacher1"
    When I open the "last" add to thquiz menu
    And I follow "from question bank"
    And I set the field with xpath "//input[@type='checkbox' and @id='qbheadercheckbox']" to "1"
    And I press "Add selected questions to the thquiz"
    Then I should see "question 01 name" on thquiz page "1"
    And I should see "question 02 name" on thquiz page "2"
  ...
  Scenario: Shuffle option could be set before adding any question to the thquiz
    Given the following "questions" exist:
      | questioncategory | qtype | name             | questiontext     |
      | Test questions   | essay | question 03 name | question 03 text |
    And I log in as "teacher1"
