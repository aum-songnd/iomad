@mod @mod_thquiz
Feature: Thquiz user override
  In order to grant a student special access to a thquiz
  As a teacher
  ...
  Scenario: Add, modify then delete a user override
    Given the following "activities" exist:
      ...
      | thquiz       | Test thquiz | C1     | thquiz1    |
    And I am on the "Test thquiz" "mod_thquiz > View" page logged in as "teacher"
  ...
  @javascript
  Scenario: Can add a user override when the thquiz is not available to the student
    Given the following "activities" exist:
      | activity   | name      | course | idnumber | visible |
      | thquiz       | Test thquiz | C1     | thquiz1    | 0       |
    When I am on the "Test thquiz" "mod_thquiz > User overrides" page logged in as "teacher"
    And I press "Add user override"
    And I set the following fields to these values:
      | Override user    | Student One (student1@example.com) |
      | Attempts allowed | 1                                  |
  ...
  Scenario: A teacher without accessallgroups permission should only be able to add user override for users that he/she shares groups with,
    ...
    And the following "activities" exist:
      ...
      | thquiz     | Test thquiz | C1     | thquiz1    | 1         |
    When I am on the "Test thquiz" "mod_thquiz > User overrides" page logged in as "teacher"
    And I press "Add user override"
  ...
  Scenario: A non-editing teacher can see the overrides, but not change them
    Given the following "activities" exist:
      | activity   | name      | course | idnumber |
      | thquiz       | Test thquiz | C1     | thquiz1    |
    And the following "mod_thquiz > user overrides" exist:
      | thquiz      | user     | attempts |
      | Test thquiz | student1 | 2        |
      | Test thquiz | student2 | 2        |
    And I am on the "Test thquiz" "mod_thquiz > View" page logged in as "helper"
    ...
    And I am on the "Test thquiz" "mod_thquiz > View" page
  ...
  Scenario: Add button disabled if no users
    ...
    And the following "activities" exist:
      | activity   | name       | course | idnumber |
      | thquiz       | Other thquiz | C2     | thquiz2    |
    When I am on the "Other thquiz" "mod_thquiz > User overrides" page logged in as "admin"
