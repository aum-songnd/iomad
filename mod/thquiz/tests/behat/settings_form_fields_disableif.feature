...
Feature: Settings form fields disabled if not required
  In to create thquizzes as simply as possible
  As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | thquiz     | C1     | 1       | Test thquiz 1 |

  @javascript
  Scenario: Depending on the number of attempts, different form fields are disabled.
    When I am on the "Test thquiz 1" "thquiz activity editing" page logged in as teacher
  ...
  Scenario: Depending on whether there is a close date, some review options are disabled.
    ...
    And I add a "Thquiz" to section "1"
    And I expand all fieldsets
    And I set the field "Name" to "Test thquiz"
  ...
  @javascript
  Scenario: If there are thquiz attempts, there is not option to repaginate.
    ...
    And thquiz "Thquiz 1" contains the following questions:
      ...
      | TF1      | 1    |         |
    When I am on the "Thquiz 1" "thquiz activity editing" page logged in as teacher
    And I expand all fieldsets
    Then I should see "Repaginate now"
    And user "student1" has attempted "Thquiz 1" with responses:
      | slot | response |
      |   1  | True     |
    And I am on the "Thquiz 1" "thquiz activity editing" page
    And I expand all fieldsets
    And I should not see "Repaginate now"
