...
/**
 * Thquiz attempt walk through using data from csv file.
 *
 * @package    mod_thquiz
 * @category   test
...
class attempt_walkthrough_from_csv_test extends \advanced_testcase {
    ...
    /**
     * @var stdClass the thquiz record we create.
     */
    protected $thquiz;
     ...
     * directory.
     *
     * @param array $thquizsettings of settings read from csv file thquizzes.csv
     * @param array $csvdata of data read from csv file "questionsXX.csv", "stepsXX.csv" and "resultsXX.csv".
     * @dataProvider get_data_for_walkthrough
     */
    public function test_walkthrough_from_csv($thquizsettings, $csvdata) {

        // CSV data files for these tests were generated using :
        // https://github.com/jamiepratt/moodle-thquiz-tools/tree/master/responsegenerator

        $this->create_thquiz_simulate_attempts_and_check_results($thquizsettings, $csvdata);
    }

    public function create_thquiz($thquizsettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $thquizsettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->thquiz = $thquizgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                thquiz_add_thquiz_question($slotquestion['id'], $this->thquiz, 0, $slotquestion['mark']);
            } else {
                thquiz_add_random_questions($this->thquiz, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create thquiz, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $thquizsettings Thquiz overrides for this thquiz.
     * @param array $csvdata Data loaded from csv files for this test.
     */
    protected function create_thquiz_simulate_attempts_and_check_results(array $thquizsettings, array $csvdata) {
        $this->resetAfterTest();

        $this->create_thquiz($thquizsettings, $csvdata['questions']);
    ...
    public function get_data_for_walkthrough(): array {
        $thquizzes = $this->load_csv_data_file('thquizzes')['thquizzes'];
        $datasets = array();
        foreach ($thquizzes as $thquizsettings) {
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $thquizsettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $thquizsettings['testnumber'])[$file];
                }
            }
            $datasets[] = array($thquizsettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the thquiz_attempt as stored in the db.
     */
    protected function walkthrough_attempts(array $steps): array {
        ...
        foreach ($steps as $steprow) {
            ...
            // Find existing user or make a new user to do the thquiz.
            ...
            if (!isset($attemptids[$step['thquizattempt']])) {
                // Start the attempt.
                $thquizobj = thquiz::create($this->thquiz->id, $user->id);
                ...
                $prevattempts = thquiz_get_user_attempts($this->thquiz->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = thquiz_create_attempt($thquizobj, $attemptnumber, null, $timenow, false, $user->id);
                ...
                thquiz_start_new_attempt($thquizobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                thquiz_attempt_save_started($thquizobj, $quba, $attempt);
                $attemptid = $attemptids[$step['thquizattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['thquizattempt']];
            }
     ...
     * @param array $attemptids attempt no as in csv file => the id of the thquiz_attempt as stored in the db.
     */
    protected function check_attempts_results(array $results, array $attemptids) {
        foreach ($results as $resultrow) {
            ...
            $attemptobj = thquiz_attempt::create($attemptids[$result['thquizattempt']]);
     ...
     * @param thquiz_attempt $attemptobj         the attempt object loaded from db.
     */
    protected function check_attempt_results(array $result, thquiz_attempt $attemptobj) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['thquizattempt']}.");
                                    break;
                                default :
                ...
                case 'summarks' :
                    $this->assertEquals((float)$value, $attemptobj->get_sum_marks(),
                        "Sum of marks of attempt {$result['thquizattempt']}.");
                    break;
                case 'thquizgrade' :
                    // Check thquiz grades.
                    $grades = thquiz_get_user_grades($attemptobj->get_thquiz(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "Thquiz grade for attempt {$result['thquizattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'thquiz',
                                                        $attemptobj->get_thquizid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['thquizattempt']}.");
                    break;
                default :
