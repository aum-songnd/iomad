 ...
 * @covers \thquiz_attempt
 */
class attempt_walkthrough_test extends \advanced_testcase {

    /**
     * Create a thquiz with questions and walk through a thquiz attempt.
     */
    public function test_thquiz_attempt_walkthrough() {
        global $SITE;
        ...
        $thquiz = $thquizgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 3));
        ...
        thquiz_add_thquiz_question($matchq->id, $thquiz);
        thquiz_add_thquiz_question($description->id, $thquiz);
        ...
        $timenow = time();
        $attempt = thquiz_create_attempt($thquizobj, 1, false, $timenow, false, $user1->id);
        ...
        // Check thquiz grades.
        $grades = thquiz_get_user_grades($thquiz, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'thquiz', $thquiz->id, $user1->id);
    ...
    /**
     * Create a thquiz containing one question and a close time.
     *
     * The question is the standard shortanswer test question.
     * The thquiz is set to close 1 hour from now.
     * The thquiz is set to use a grade period of 1 hour once time expires.
     *
     * @return \stdClass the thquiz that was created.
     */
    protected function create_thquiz_with_one_question(): \stdClass {
        global $SITE;
        $this->resetAfterTest();
        ...
        // Add them to the thquiz.
        thquiz_add_thquiz_question($saq->id, $thquiz, 0, 1);
        thquiz_update_sumgrades($thquiz);

        return $thquiz;
    }

    public function test_thquiz_attempt_walkthrough_submit_time_recorded_correctly_when_overdue() {

        $thquiz = $this->create_thquiz_with_one_question();
        ...
        $attemptobj = thquiz_attempt::create($attempt->id);
        $this->assertEquals(1, $attemptobj->get_number_of_unanswered_questions());
        $attemptobj->process_submitted_actions($thquiz->timeclose - 30 * MINSECS, false, [1 => ['answer' => 'frog']]);

        // Attempt goes overdue (e.g. if cron ran).
        $attemptobj = thquiz_attempt::create($attempt->id);
        $attemptobj->process_going_overdue($thquiz->timeclose + 2 * get_config('thquiz', 'graceperiodmin'), false);
        ...
        $attemptobj = thquiz_attempt::create($attempt->id);
        $attemptobj->process_attempt($thquiz->timeclose + 30 * MINSECS, true, false, 1);

        // Verify the attempt state.
        $attemptobj = thquiz_attempt::create($attempt->id);
        $this->assertEquals(1, $attemptobj->get_attempt_number());
        $this->assertEquals(true, $attemptobj->is_finished());
        $this->assertEquals($thquiz->timeclose + 30 * MINSECS, $attemptobj->get_submitted_date());
        $this->assertEquals($user->id, $attemptobj->get_userid());
        $this->assertTrue($attemptobj->has_response_to_at_least_one_graded_question());
        $this->assertEquals(0, $attemptobj->get_number_of_unanswered_questions());
    }

    public function test_thquiz_attempt_walkthrough_close_time_extended_at_last_minute() {
        global $DB;

        $thquiz = $this->create_thquiz_with_one_question();
        $originaltimeclose = $thquiz->timeclose;
        ...
        // Teacher edits the thquiz to extend the time-limit by one minute.
        $DB->set_field('thquiz', 'timeclose', $originaltimeclose + MINSECS, ['id' => $thquiz->id]);
        \course_modinfo::clear_instance_cache($thquiz->course);

        // Timer expires in the student browser and thinks it is time to submit the thquiz.
        ...
        $this->assertEquals(thquiz_attempt::IN_PROGRESS, $attemptobj->get_state());
        $this->assertEquals(0, $attemptobj->get_submitted_date());
        $this->assertEquals($user->id, $attemptobj->get_userid());
    }

    /**
     * Create a thquiz with a random as well as other questions and walk through thquiz attempts.
     */
    public function test_thquiz_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $thquiz = $thquizgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the thquiz.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            thquiz_start_new_attempt($thquizobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $thquizwithvariants = null;

    /**
     * Create a thquiz with a single question with variants and walk through thquiz attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_thquiz_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->thquizwithvariants === null) {
            // Make a thquiz.
            $thquizgenerator = $this->getDataGenerator()->get_plugin_generator('mod_thquiz');

            $this->thquizwithvariants = $thquizgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            thquiz_add_thquiz_question($calc->id, $this->thquizwithvariants, 0);
        }


        // Make a new user to do the thquiz.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $thquizobj = thquiz::create($this->thquizwithvariants->id, $user1->id);
        ...
        // Select variant.
        thquiz_start_new_attempt($thquizobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check thquiz grades.
        $grades = thquiz_get_user_grades($this->thquizwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'thquiz', $this->thquizwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
