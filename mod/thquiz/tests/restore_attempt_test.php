...
/**
 * Unit tests restoring thquiz attempts
 *
 * @package     mod_thquiz
...
class restore_attempt_test extends \advanced_testcase {
     ...
     * Test restore dates.
     *
     * @covers \restore_thquiz_activity_structure_step
     */
    public function test_restore_question_attempts_missing_users(): void {
        ...
        // Contains a thquiz with four attempts from different users. The users are as follows (user ID -> user):
        ...
        $restoredthquiz = $DB->get_record('thquiz', []);
        ...
        $this->assertContains("Mapped user ID not found for user 4, thquiz {$restoredthquiz->id}, attempt 1. Skipping thquiz attempt",
            $loginfomessages);
        $this->assertContains("Mapped user ID not found for user 5, thquiz {$restoredthquiz->id}, attempt 1. Skipping thquiz attempt",
            $loginfomessages);

        // User 01 has supplied the wrong answer, assert dates match the backup file too.
        $user01attempt = $DB->get_record('thquiz_attempts', [
            'thquiz' => $restoredthquiz->id,
        ...
        $user04attempt = $DB->get_record('thquiz_attempts', [
            'thquiz' => $restoredthquiz->id,
