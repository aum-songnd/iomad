...
/**
 * Unit tests for the {@link \mod_thquiz\repaginate} class.
 * @package   mod_thquiz
 * @category  test
...
require_once($CFG->dirroot . '/mod/thquiz/classes/repaginate.php');

/**
 * Testable subclass, giving access to the protected methods of {@link \mod_thquiz\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_thquiz_repaginate_testable extends repaginate {

    public function __construct($thquizid = 0, $slots = null) {
        return parent::__construct($thquizid, $slots);
    }
...
class repaginate_test extends \advanced_testcase {
    ...
    private $thquizslots;
    /** @var mod_thquiz_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp(): void {
        $this->set_thquiz_slots($this->get_thquiz_object()->get_slots());
        $this->repaginate = new mod_thquiz_repaginate_testable(0, $this->thquizslots);
    }
    ...
    /**
     * Create a thquiz, add five questions to the thquiz
     * which are all on one page and return the thquiz object.
     */
    private function get_thquiz_object() {
        global $SITE;
        ...
        $thquiz = $thquizgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('thquiz', $thquiz->id, $SITE->id);
        ...
        thquiz_add_thquiz_question($shortanswer->id, $thquiz);
        thquiz_add_thquiz_question($numerical->id, $thquiz);
        thquiz_add_thquiz_question($essay->id, $thquiz);
        thquiz_add_thquiz_question($truefalse->id, $thquiz);
        thquiz_add_thquiz_question($match->id, $thquiz);

        // Return the thquiz object.
        $thquizobj = new thquiz($thquiz, $cm, $SITE);
        return structure::create_for_thquiz($thquizobj);
    }

    /**
     * Set the thquiz slots
     * @param string $slots
     */
    private function set_thquiz_slots($slots = null) {
        if (!$slots) {
            $this->thquizslots = $this->get_thquiz_object()->get_slots();
        } else {
            $this->thquizslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_thquiz_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->thquizslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->thquizslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->thquizslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->thquizslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->thquizslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->thquizslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->thquizslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->thquizslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->thquizslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->thquizslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_thquiz_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->thquizslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->thquizslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->thquizslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
