YUI.add('moodle-mod_thquiz-modform', function (Y, NAME) {

/**
 * The modform class has all the JavaScript specific to mod/thquiz/mod_form.php.
 *
 * @module moodle-mod_thquiz-modform
 */
...
Y.extend(MODFORM, Y.Base, {
    ...
    initializer: function() {
        ...
        if (!this.repaginateCheckbox) {
            // The checkbox only appears when editing an existing thquiz.
            return;
        }
...
// Ensure that M.mod_thquiz exists and that coursebase is initialised correctly
M.mod_thquiz = M.mod_thquiz || {};
M.mod_thquiz.modform = M.mod_thquiz.modform || new MODFORM();
M.mod_thquiz.modform.init = function() {
    return new MODFORM();
};
