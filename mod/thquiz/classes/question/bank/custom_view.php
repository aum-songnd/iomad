...
/**
 * Defines the custom question bank view used on the Edit thquiz page.
 *
 * @package   mod_thquiz
 * @category  question
...
namespace mod_thquiz\question\bank;

use core_question\local\bank\question_version_status;
use mod_thquiz\question\bank\filter\custom_category_condition;

/**
 * Subclass to customise the view of the question bank for the thquiz editing screen.
 *
...
class custom_view extends \core_question\local\bank\view {
    ...
    /** @var bool $thquizhasattempts whether the thquiz this is used by has been attemptd. */
    protected $thquizhasattempts = false;

    /** @var \stdClass $thquiz the thquiz settings. */
    protected $thquiz = false;
     ...
     * @param \stdClass $thquiz thquiz settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $thquiz) {
    ...
    protected function get_question_bank_plugins(): array {
        ...
        foreach ($corequestionbankcolumns as $fullname) {
            $shortname = $fullname;
            if (class_exists('mod_thquiz\\question\\bank\\' . $fullname)) {
                $fullname = 'mod_thquiz\\question\\bank\\' . $fullname;
    ...
    protected function heading_column(): string {
        return 'mod_thquiz\\question\\bank\\question_name_text_column';
    }

    protected function default_sort(): array {
        // Using the extended class for thquiz specific sort.
        return [
            'qbank_viewquestiontype\\question_type_column' => 1,
            'mod_thquiz\\question\\bank\\question_name_text_column' => 1,
        ];
    }

    /**
     * Let the question bank display know whether the thquiz has been attempted,
     * hence whether some bits of UI, like the add this question to the thquiz icon,
     * should be displayed.
     *
     * @param bool $thquizhasattempts whether the thquiz has attempts.
     */
    public function set_thquiz_has_attempts($thquizhasattempts): void {
        $this->thquizhasattempts = $thquizhasattempts;
        if ($thquizhasattempts && isset($this->visiblecolumns['addtothquizaction'])) {
            unset($this->visiblecolumns['addtothquizaction']);
        }
    }
    ...
    public function preview_question_url($question) {
        return thquiz_question_preview_url($this->thquiz, $question);
    }

    /**
     * URL of add to thquiz.
     *
     * @param $questionid
     * @return \moodle_url
     */
    public function add_to_thquiz_url($questionid) {
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/thquiz/edit.php', $params);
    }
    ...
    protected function display_bottom_controls(\context $catcontext): void {
        ...
        $cmoptions->hasattempts = !empty($this->thquizhasattempts);

        $canuseall = has_capability('moodle/question:useall', $catcontext);

        echo \html_writer::start_tag('div', ['class' => 'pt-2']);
        if ($canuseall) {
            // Add selected questions to the thquiz.
            $params = array(
                'type' => 'submit',
                'name' => 'add',
                'class' => 'btn btn-primary',
                'value' => get_string('addselectedquestionstothquiz', 'thquiz'),
                'data-action' => 'toggle',
