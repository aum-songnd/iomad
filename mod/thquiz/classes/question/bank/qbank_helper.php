...
class qbank_helper {
    ...
    /**
     * Get the information about which questions should be used to create a thquiz attempt.
     *
     ...
     *
     * @param int $thquizid the id of the thquiz to load the data for.
     * @param \context_module $thquizcontext the context of this thquiz.
     * @param int|null $slotid optional, if passed only load the data for this one slot (if it is in this thquiz).
     * @return array indexed by slot, with information about the content of each slot.
     */
    public static function get_question_structure(int $thquizid, \context_module $thquizcontext,
            int $slotid = null): array {
        global $DB;

        $params = [
            'draft' => question_version_status::QUESTION_STATUS_DRAFT,
            'thquizcontextid' => $thquizcontext->id,
            'thquizcontextid2' => $thquizcontext->id,
            'thquizcontextid3' => $thquizcontext->id,
            'thquizid' => $thquizid,
            'thquizid2' => $thquizid,
        ];
        $slotidtest = '';
        ...
        $slotdata = $DB->get_records_sql("
             ...
             -- case where a particular question has been added to the thquiz.
             LEFT JOIN {question_references} qr ON qr.usingcontextid = :thquizcontextid AND qr.component = 'mod_thquiz'
             ...
             LEFT JOIN (
                   SELECT lv.questionbankentryid,
                     ...
                     FROM {thquiz_slots} lslot
                     JOIN {question_references} lqr ON lqr.usingcontextid = :thquizcontextid2 AND lqr.component = 'mod_thquiz'
                    ...
                    WHERE lslot.thquizid = :thquizid2
                          $slotidtest2
             ...
             LEFT JOIN {question_set_references} qsr ON qsr.usingcontextid = :thquizcontextid3 AND qsr.component = 'mod_thquiz'
                                        AND qsr.questionarea = 'slot' AND qsr.itemid = slot.id

                 WHERE slot.thquizid = :thquizid
                       $slotidtest
        ...
        foreach ($slotdata as $slot) {
            ...
            if ($slot->filtercondition) {
                ...
                $slot->qtype = 'random';
                $slot->name = get_string('random', 'thquiz');
                $slot->length = 1;
                ...
                $slot->name = get_string('missingquestion', 'thquiz');
                $slot->questiontext = ' ';
     ...
     * @param int $thquizid the id of the thquiz to load the data for.
     * @param \context_module $thquizcontext the context of this thquiz.
     * @param int $slotid optional, if passed only load the data for this one slot (if it is in this thquiz).
     ...
     */
    public static function choose_question_for_redo(int $thquizid, \context_module $thquizcontext,
            int $slotid, qubaid_condition $qubaids): int {
        $slotdata = self::get_question_structure($thquizid, $thquizcontext, $slotid);
