...
namespace mod_thquiz\completion;
...
use grade_item;
use thquiz;
use thquiz_access_manager;

/**
 * Activity custom completion subclass for the thquiz activity.
 *
 * Class for defining mod_thquiz's custom completion rules and fetching the completion statuses
 * of the custom completion rules for a given thquiz instance and a user.
 *
 * @package   mod_thquiz
...
class custom_completion extends activity_custom_completion {
    ...
    protected function check_passing_grade_or_all_attempts(): bool {
        ...
        $attempts = thquiz_get_user_attempts($this->cm->instance, $this->userid, 'finished', true);
        if (!$attempts) {
            return false;
        }
        $lastfinishedattempt = end($attempts);
        $context = context_module::instance($this->cm->id);
        $thquizobj = thquiz::create($this->cm->instance, $this->userid);
        $accessmanager = new thquiz_access_manager(
            $thquizobj,
            time(),
            has_capability('mod/thquiz:ignoretimelimits', $context, $this->userid, false)
        );
    ...
    public function get_custom_rule_descriptions(): array {
        ...
        $description['completionminattempts'] = get_string('completiondetail:minattempts', 'mod_thquiz', $minattempts);

        // Completion pass grade is now part of core. Only show the following if it's combined with min attempts.
        $completionpassorattempts = $this->cm->customdata['customcompletionrules']['completionpassorattemptsexhausted'] ?? [];
        if (!empty($completionpassorattempts['completionattemptsexhausted'])) {
            $description['completionpassorattemptsexhausted'] = get_string('completiondetail:passorexhaust', 'mod_thquiz');
        }
