...
/**
 * Defines the \mod_thquiz\structure class.
 *
 * @package   mod_thquiz
...
/**
 * Thquiz structure class.
 *
 * The structure of the thquiz. That is, which questions it is built up
 * from. This is used on the Edit thquiz page (edit.php) and also when
 * starting an attempt at the thquiz (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \thquiz the thquiz this is the structure of. */
    protected $thquizobj = null;

    /**
     * @var \stdClass[] the questions in this thquiz. Contains the row from the questions
     * table, with the data from the thquiz_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] thquiz_slots.slot => the thquiz_slots rows for this thquiz, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * thquiz_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty thquiz.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given thquiz.
     * @param \thquiz $thquizobj the thquiz.
     * @return structure
     */
    public static function create_for_thquiz($thquizobj) {
        $structure = self::create();
        $structure->thquizobj = $thquizobj;
        $structure->populate_structure();
        return $structure;
    }

    /**
     * Whether there are any questions in the thquiz.
     * @return bool true if there is at least one question in the thquiz.
     */
    ...
    /**
     * Get the number of questions in the thquiz.
     * @return int the number of questions in the thquiz.
     */
     ...
     * question_category.contextid, and the thquiz_slots data for the question in this thquiz.
     */
     ...
     * questions cannot be depended upon because of thquiz options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->thquizobj->get_navigation_method() == THQUIZ_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->thquizobj->get_thquiz()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_thquiz', $this->thquizobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->thquizobj->get_thquiz()->preferredbehaviour);
    ...
    /**
     * Get the course id that the thquiz belongs to.
     * @return int the course.id for the thquiz.
     */
    ...
    /**
     * Get the course module id of the thquiz.
     * @return int the course_modules.id for the thquiz.
     */
    ...
    /**
     * Get id of the thquiz.
     * @return int the thquiz.id for the thquiz.
     */
     ...
     * @return \stdClass the thquiz settings row from the database.
     */
    ...
    /**
     * Thquizzes can only be repaginated if they have not been attempted, the
     * questions are not shuffled, and there are two or more questions.
     * @return bool whether this thquiz can be repaginated.
     */
    ...
    /**
     * Thquizzes can only be edited if they have not been attempted.
     * @return bool whether the thquiz can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !thquiz_has_attempts($this->thquizobj->get_thquizid());
        }
        return $this->canbeedited;
    }

    /**
     * This thquiz can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = thquiz_attempt_summary_link_to_reports($this->get_thquiz(),
                    $this->thquizobj->get_cm(), $this->thquizobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'thquiz',
                    new \moodle_url('/mod/thquiz/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the thquiz.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the thquiz.
     * @return int the number of questions that should be on each page of the
     * thquiz by default.
     */
    public function get_questions_per_page() {
        return $this->thquizobj->get_thquiz()->questionsperpage;
    }

    /**
     * Get thquiz slots.
     * @return \stdClass[] the slots in this thquiz.
     */
    ...
    /**
     * Is this slot the last one in the thquiz?
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the thquiz.
     */
    public function is_last_slot_in_thquiz($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the thquiz?
     * @param \stdClass $section the thquiz_sections row.
     * @return bool whether this is first section in the thquiz.
     */
    ...
    /**
     * Is this the last section in the thquiz?
     * @param \stdClass $section the thquiz_sections row.
     * @return bool whether this is first section in the thquiz.
     */
    ...
    /**
     * Get the final slot in the thquiz.
     * @return \stdClass the thquiz_slots for for the final slot in the thquiz.
     */
     ...
     * @return \stdClass the requested thquiz_slots row.
    ...
    /**
     * Get all the slots in a section of the thquiz.
    ...
    /**
     * Get all the sections of the thquiz.
     * @return \stdClass[] the sections in this thquiz.
     */
    ...
    /**
     * Get the overall thquiz grade formatted for display.
     * @return string the maximum grade for this thquiz.
     */
    public function formatted_thquiz_grade() {
        return thquiz_format_grade($this->get_thquiz(), $this->get_thquiz()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return thquiz_format_question_grade($this->get_thquiz(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displyaing overall thquiz grades or marks.
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_thquiz()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return thquiz_get_grade_format($this->get_thquiz());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (thquiz_has_attempts($this->thquizobj->get_thquizid())) {
            $reviewlink = thquiz_attempt_summary_link_to_reports($this->thquizobj->get_thquiz(),
                    $this->thquizobj->get_cm(), $this->thquizobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'thquiz', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the thquiz.
    ...
    public function get_dates_summary() {
        $timenow = time();
        $thquiz = $this->thquizobj->get_thquiz();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($thquiz->timeopen > 0) {
            if ($timenow > $thquiz->timeopen) {
                $dates[] = get_string('thquizopenedon', 'thquiz', userdate($thquiz->timeopen));
            } else {
                $dates[] = get_string('thquizwillopen', 'thquiz', userdate($thquiz->timeopen));
            }
        }
        if ($thquiz->timeclose > 0) {
            if ($timenow > $thquiz->timeclose) {
                $dates[] = get_string('thquizclosed', 'thquiz', userdate($thquiz->timeclose));
            } else {
                $dates[] = get_string('thquizcloseson', 'thquiz', userdate($thquiz->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'thquiz');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $thquiz->timeopen) {
            $currentstatus = get_string('thquizisclosedwillopen', 'thquiz',
                    userdate($thquiz->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($thquiz->timeclose && $timenow <= $thquiz->timeclose) {
            $currentstatus = get_string('thquizisopenwillclose', 'thquiz',
                    userdate($thquiz->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($thquiz->timeclose && $timenow > $thquiz->timeclose) {
            $currentstatus = get_string('thquizisclosed', 'thquiz');
        } else {
            $currentstatus = get_string('thquizisopen', 'thquiz');
        }
    ...
    /**
     * Set up this class with the structure for a given thquiz.
     */
    protected function populate_structure() {
        global $DB;

        $slots = qbank_helper::get_question_structure($this->thquizobj->get_thquizid(), $this->thquizobj->get_context());
        ...
        foreach ($slots as $slotdata) {
            ...
            $slot = clone($slotdata);
            $slot->thquizid = $this->thquizobj->get_thquizid();
            $this->slotsinorder[$slot->slot] = $slot;
        }

        // Get thquiz sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('thquiz_sections', ['thquizid' => $this->thquizobj->get_thquizid()], 'firstslot');
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slotsinorder as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'thquiz');
            } else {
    ...
    public function get_version_choices_for_slot(int $slotnumber): array {
        ...
        foreach ($versions as $version) {
            ...
            if ($version->version === $latestversion->version) {
                $version->versionvalue = get_string('questionversionlatest', 'thquiz', $version->version);
            } else {
                $version->versionvalue = get_string('questionversion', 'thquiz', $version->version);
            }
        ...
        $alwaysuselatest->versionvalue = get_string('alwayslatest', 'thquiz');
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_thquiz($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_thquiz($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_thquiz($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('thquiz_slots', 'slot', $slotreorder,
                    array('thquizid' => $this->get_thquizid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('thquiz_slots', 'page', $page,
                    array('id' => $movingslot->id));
        }

        // Update section fist slots.
        thquiz_update_section_firstslots($this->get_thquizid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE thquizid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {thquiz_slots} WHERE thquizid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_thquizid(), $this->get_thquizid()));
        ...
        foreach ($emptypages as $emptypage) {
            $DB->execute("
                    UPDATE {thquiz_slots}
                     ...
                     WHERE thquizid = ?
                       AND page > ?
                    ", array($this->get_thquizid(), $emptypage));
        }

        $trans->allow_commit();

        // Log slot moved event.
        $event = \mod_thquiz\event\slot_moved::create([
            'context' => $this->thquizobj->get_context(),
            'objectid' => $idmove,
            'other' => [
                'thquizid' => $this->thquizobj->get_thquizid(),
    ...
    /**
     * Refresh page numbering of thquiz slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('thquiz_slots', array('thquizid' => $this->get_thquizid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of thquiz slots and save to the database.
     * @param \stdClass $thquiz the thquiz object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('thquiz_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('thquiz_slots', array('thquizid' => $this->get_thquizid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {thquiz_slots} WHERE thquizid = ?', array($this->get_thquizid()));

        $trans = $DB->start_delegated_transaction();
        // Delete the reference if its a question.
        $questionreference = $DB->get_record('question_references',
                ['component' => 'mod_thquiz', 'questionarea' => 'slot', 'itemid' => $slot->id]);
        ...
        if ($questionsetreference) {
            $DB->delete_records('question_set_references',
                ['id' => $questionsetreference->id, 'component' => 'mod_thquiz', 'questionarea' => 'slot']);
        }
        $DB->delete_records('thquiz_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('thquiz_slots', 'slot', $i - 1,
                    array('thquizid' => $this->get_thquizid(), 'slot' => $i));
            $this->slotsinorder[$i]->slot = $i - 1;
            $this->slotsinorder[$i - 1] = $this->slotsinorder[$i];
            unset($this->slotsinorder[$i]);
        }

        thquiz_update_section_firstslots($this->get_thquizid(), -1, $slotnumber);
        ...
        $event = \mod_thquiz\event\slot_deleted::create([
            'context' => $this->thquizobj->get_context(),
            'objectid' => $slot->id,
            'other' => [
     ...
     *
     * Saves changes to the question grades in the thquiz_slots table and any
     * corresponding question_attempts.
     * It does not update 'sumgrades' in the thquiz table.
     *
     * @param \stdClass $slot row from the thquiz_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('thquiz_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_thquiz($slot->thquizid),
                $slot->slot, $maxmark);
        $trans->allow_commit();

        // Log slot mark updated event.
        // We use $num + 0 as a trick to remove the useless 0 digits from decimals.
        $event = \mod_thquiz\event\slot_mark_updated::create([
            'context' => $this->thquizobj->get_context(),
            'objectid' => $slot->id,
            'other' => [
                'thquizid' => $this->get_thquizid(),
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('thquiz_slots', 'requireprevious', $requireprevious, array('id' => $slotid));

        // Log slot require previous event.
        $event = \mod_thquiz\event\slot_requireprevious_updated::create([
            'context' => $this->thquizobj->get_context(),
            'objectid' => $slotid,
            'other' => [
                'thquizid' => $this->get_thquizid(),
     ...
     *
     * Saves changes to the slot page relationship in the thquiz_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $thquizslots = $DB->get_records('thquiz_slots', array('thquizid' => $this->get_thquizid()), 'slot');
        $repaginate = new \mod_thquiz\repaginate($this->get_thquizid(), $thquizslots);
        $repaginate->repaginate_slots($thquizslots[$slotid]->slot, $type);
        $slots = $this->refresh_page_numbers_and_update_db();

        if ($type == repaginate::LINK) {
            // Log page break created event.
            $event = \mod_thquiz\event\page_break_deleted::create([
                'context' => $this->thquizobj->get_context(),
                'objectid' => $slotid,
                'other' => [
                    'thquizid' => $this->get_thquizid(),
                    'slotnumber' => $thquizslots[$slotid]->slot
                ]
            ]);
            $event->trigger();
        } else {
            // Log page deleted created event.
            $event = \mod_thquiz\event\page_break_created::create([
                'context' => $this->thquizobj->get_context(),
                'objectid' => $slotid,
                'other' => [
                    'thquizid' => $this->get_thquizid(),
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'thquiz');
        }
        $section->thquizid = $this->get_thquizid();
        $slotsonpage = $DB->get_records('thquiz_slots', array('thquizid' => $this->get_thquizid(), 'page' => $pagenumber), 'slot DESC');
        $firstslot = end($slotsonpage);
        $section->firstslot = $firstslot->slot;
        $section->shufflequestions = 0;
        $sectionid = $DB->insert_record('thquiz_sections', $section);

        // Log section break created event.
        $event = \mod_thquiz\event\section_break_created::create([
            'context' => $this->thquizobj->get_context(),
            'objectid' => $sectionid,
            'other' => [
                'thquizid' => $this->get_thquizid(),
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('thquiz_sections', array('id' => $id), '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('thquiz_sections', $section);

        // Log section title updated event.
        $firstslot = $DB->get_record('thquiz_slots', array('thquizid' => $this->get_thquizid(), 'slot' => $section->firstslot));
        $event = \mod_thquiz\event\section_title_updated::create([
            'context' => $this->thquizobj->get_context(),
            'objectid' => $id,
            'other' => [
                'thquizid' => $this->get_thquizid(),
    ...
    public function set_section_shuffle($id, $shuffle) {
        ...
        $event = \mod_thquiz\event\section_shuffle_updated::create([
            'context' => $this->thquizobj->get_context(),
            'objectid' => $id,
            'other' => [
                'thquizid' => $this->get_thquizid(),
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('thquiz_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a thquiz.');
        }
        $DB->delete_records('thquiz_sections', array('id' => $sectionid));

        // Log page deleted created event.
        $firstslot = $DB->get_record('thquiz_slots', array('thquizid' => $this->get_thquizid(), 'slot' => $section->firstslot));
        $event = \mod_thquiz\event\section_break_deleted::create([
            'context' => $this->thquizobj->get_context(),
            'objectid' => $sectionid,
            'other' => [
                'thquizid' => $this->get_thquizid(),
    ...
    /**
     * Whether the current user can add random questions to the thquiz or not.
    ...
    public function can_add_random_questions() {
        if ($this->canaddrandom === null) {
            $thquizcontext = $this->thquizobj->get_context();
            $relatedcontexts = new \core_question\local\bank\question_edit_contexts($thquizcontext);
