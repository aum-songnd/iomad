...
/**
 * Defines the thquiz repaginate class.
 *
 * @package   mod_thquiz
 ...
 *
 * The thquiz setting allows users to write thquizzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the thquiz being manipulated. */
    private $thquizid;
    /** @var array the thquiz_slots for that thquiz. */
    private $slots;

    /**
     * Constructor.
     * @param int $thquizid the id of the thquiz being manipulated.
     * @param stdClass[] $slots the thquiz_slots for that thquiz.
     */
    public function __construct($thquizid = 0, $slots = null) {
        global $DB;
        $this->thquizid = $thquizid;
        if (!$this->thquizid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('thquiz_slots', array('thquizid' => $this->thquizid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('thquiz_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $thquizslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($thquizslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$thquizslots) {
            return null;
        }
        $newslots = array();
        foreach ($thquizslots as $slot) {
            if ($type == self::LINK) {
