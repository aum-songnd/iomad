...
/**
 * Renderer outputting the thquiz editing UI.
 *
 * @package mod_thquiz
...
class edit_renderer extends \plugin_renderer_base {
    ...
    protected $togglegroup = 'thquiz-questions';

    /**
     * Render the edit page
     *
     * @param \thquiz $thquizobj object containing all the thquiz settings information.
     * @param structure $structure object containing the structure of the thquiz.
     ...
     */
    public function edit_page(\thquiz $thquizobj, structure $structure,
        \core_question\local\bank\question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading(get_string('questions', 'thquiz'));

        // Information at the top.
        $output .= $this->thquiz_state_warnings($structure);

        $output .= html_writer::start_div('mod_thquiz-edit-top-controls');
        ...
        $output .= $this->thquiz_information($structure);
        $output .= html_writer::end_tag('div');
        $output .= $this->maximum_grade_input($structure, $pageurl);
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_div('d-flex justify-content-between flex-wrap mb-1');
        $output .= html_writer::start_div('mod_thquiz-edit-action-buttons btn-group edit-toolbar', ['role' => 'group']);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->selectmultiple_button($structure);
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_div('d-flex flex-column justify-content-around');
        $output .= $this->total_marks($thquizobj->get_thquiz());
        $output .= html_writer::end_tag('div');
        $output .= html_writer::end_tag('div');
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->js_call_amd('mod_thquiz/thquizquestionbank', 'init', [
                $thiscontext->id
            ]);

            $this->page->requires->js_call_amd('mod_thquiz/add_random_question', 'init', [
                $thiscontext->id,
                $pagevars['cat'],
    ...
    /**
     * Render any warnings that might be required about the state of the thquiz,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the thquiz structure.
     * @return string HTML to output.
     */
    public function thquiz_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function thquiz_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'thquiz', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'thquizopeningstatus',
    ...
    /**
     * Render the form for setting a thquiz' overall grade
     *
     * @param structure $structure the thquiz structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::start_tag('form', array('method' => 'post', 'action' => 'edit.php',
                'class' => 'thquizsavegradesform form-inline'));
        ...
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_thquiz_grade(),
                'class' => 'form-control'));
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'class' => 'btn btn-secondary ml-1',
                'name' => 'savechanges', 'value' => get_string('save', 'thquiz')));
     ...
     * @param structure $structure the structure of the thquiz being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {
        $header = html_writer::tag('span', get_string('repaginatecommand', 'thquiz'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'thquiz'),
        ...
        if (!$structure->can_be_repaginated()) {
            ...
            $this->page->requires->js_call_amd('mod_thquiz/repaginate', 'init');
        }
    ...
    protected function selectmultiple_button(structure $structure) {
        $buttonoptions = array(
            ...
            'value' => get_string('selectmultipleitems', 'thquiz'),
        ...
        return html_writer::tag('button', get_string('selectmultipleitems', 'thquiz'), $buttonoptions);
    }
    ...
    protected function selectmultiple_controls(structure $structure) {
        ...
        $buttondeleteoptions = array(
            ...
            'value' => get_string('deleteselected', 'mod_thquiz'),
            'class' => 'btn btn-secondary',
            'data-action' => 'toggle',
        ...
        $output .= html_writer::tag('div',
                        html_writer::tag('button', get_string('deleteselected', 'mod_thquiz'), $buttondeleteoptions) .
                        " " .
        ...
        $toolbaroptions = array(
            ...
            'role' => 'toolbar',
            'aria-label' => get_string('selectmultipletoolbar', 'thquiz'),
        );
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'thquiz');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'thquiz', $select) .
    ...
    /**
     * Render the total marks available for the thquiz.
     *
     * @param \stdClass $thquiz the thquiz settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($thquiz) {
        $totalmark = html_writer::span(thquiz_format_grade($thquiz, $thquiz->sumgrades), 'mod_thquiz_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'thquiz', $totalmark),
     ...
     *
     * @param structure $structure the structure of the thquiz being edited.
     * @param \stdClass $section The thquiz_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if ($section->heading) {
            ...
            $sectionheadingtext = get_string('sectionnoname', 'thquiz');
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'thquiz', $sectionheadingtext),
     ...
     *
     * @param structure $structure object containing the structure of the thquiz.
     * @param \stdClass $section data from the thquiz_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'thquiz');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'thquiz'),
    ...
    /**
     * Render an icon to remove a section from the thquiz.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'thquiz', format_string($section->heading));
        $url = new \moodle_url('/mod/thquiz/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the thquiz.
     *
     * This function calls {@link core_course_renderer::thquiz_section_question()}
     *
     * @param structure $structure object containing the structure of the thquiz.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_thquiz($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-thquiz-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'thquiz'), array('class' => 'add-menu'));
        ...
        // Disable the link if thquiz has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'thquiz');
        }

        // Get section, page, slotnumber and maxmark.
        $actions = array();

        // Add a new question to the thquiz.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'thquiz', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'thquiz');
        }
        ...
        if ($structure->can_add_random_questions()) {
            $returnurl = new \moodle_url('/mod/thquiz/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
            $params = ['returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion'];
            $url = new \moodle_url('/mod/thquiz/addrandom.php', $params);
            ...
            if ($page) {
                $title = get_string('addrandomquestiontopage', 'thquiz', $page);
            } else {
                $title = get_string('addrandomquestionatend', 'thquiz');
            }
    ...
    /**
     * Render the form that contains the data for adding a new question to the thquiz.
     *
     * @param structure $structure object containing the structure of the thquiz.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, int $slot, \moodle_url $pageurl) {
        ...
        if ($structure->get_slot_by_number($slot)->qtype !== 'random') {
            ...
            $this->page->requires->js_call_amd('mod_thquiz/question_slot', 'init', [$slotid]);
        }

        // Render the question slot template.
        $output .= $this->render_from_template('mod_thquiz/question_slot', $data);
    ...
    public function get_checkbox_render(structure $structure, int $slot) : string {
        ...
        $checkbox = new \core\output\checkbox_toggleall($this->togglegroup, false,
            [
                ...
                'label' => get_string('selectquestionslot', 'thquiz', $questionslot),
    ...
    public function get_action_icon(structure $structure, int $slot, \moodle_url $pageurl) : string {
        ...
        $questionicons = '';
        if ($qtype !== 'random') {
            $questionicons .= $this->question_preview_icon($structure->get_thquiz(),
     ...
     * @return string HTML to output.
     */
    public function question_preview_icon($thquiz, $questiondata, $label = null, $variant = null, $restartversion = null) {
        ...
        $url = thquiz_question_preview_url($thquiz, $question, $variant, $restartversion);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'thquiz');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'thquiz');
    ...
    /**
     * Render an icon to remove a question from the thquiz.
     *
     * @param structure $structure object containing the structure of the thquiz.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the thquiz.
     *
     * @param structure $structure object containing the structure of the thquiz.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('thquizid' => $structure->get_thquizid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'thquiz');
            $image = $this->image_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'thquiz');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'thquiz', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'thquiz'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'thquiz', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'thquiz'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a thquiz edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = thquiz_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'thquiz').' '.$title));
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $editurl = new \moodle_url('/mod/thquiz/editrandom.php',
                array('returnurl' => $pageurl->out_as_local_url(), 'slotid' => $slot->id));

        $temp = clone($question);
        $temp->questiontext = '';
        $temp->name = qbank_helper::describe_random_question($slot);
        $instancename = thquiz_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'thquiz');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                        get_string('seequestions', 'thquiz'), array('class' => 'mod_thquiz_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'thquiz')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'thquiz'),
                )
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_thquiz/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_thquiz\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/thquiz/edit_rest.php';
        $config->sectionurl = '/mod/thquiz/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_thquiz-toolboxes',
                'M.mod_thquiz.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'thquizid' => $structure->get_thquizid(),
        ...
        $this->page->requires->strings_for_js(array('areyousureremoveselected'), 'thquiz');
        $this->page->requires->yui_module('moodle-mod_thquiz-toolboxes',
                'M.mod_thquiz.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'thquizid' => $structure->get_thquizid(),
        ...
        $this->page->requires->yui_module('moodle-mod_thquiz-dragdrop', 'M.mod_thquiz.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'thquizid' => $structure->get_thquizid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_thquiz-dragdrop', 'M.mod_thquiz.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'thquizid' => $structure->get_thquizid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'thquiz', '');
     ...
     *
     * @param \mod_thquiz\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_thquiz\question\bank\custom_view $questionbank, array $pagevars) {
