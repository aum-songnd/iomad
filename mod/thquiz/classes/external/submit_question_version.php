...
/**
 * External api for changing the question version in the thquiz.
 *
 * @package    mod_thquiz
...
class submit_question_version extends external_api {
    ...
    public static function execute(int $slotid, int $newversion): array {
        ...
        $referencedata = $DB->get_record('question_references',
            ['itemid' => $params['slotid'], 'component' => 'mod_thquiz', 'questionarea' => 'slot']);
        $slotdata = $DB->get_record('thquiz_slots', ['id' => $slotid]);

        // Capability check.
        list($course, $cm) = get_course_and_cm_from_instance($slotdata->thquizid, 'thquiz');
        $context = \context_module::instance($cm->id);
        self::validate_context($context);
        require_capability('mod/thquiz:manage', $context);
