 ...
 * @since      Moodle 3.1
 */
class mod_thquiz_external extends external_api {

    /**
     * Describes the parameters for get_thquizzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_thquizzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of thquizzes in a provided list of courses,
     * if no list is provided all thquizzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of thquizzes details
     * @since Moodle 3.1
     */
    public static function get_thquizzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedthquizzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_thquizzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the thquizzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $thquizzes = get_all_instances_in_courses("thquiz", $courses);
            foreach ($thquizzes as $thquiz) {
                $context = context_module::instance($thquiz->coursemodule);

                // Update thquiz with override information.
                $thquiz = thquiz_update_effective_access($thquiz, $USER->id);

                // Entry to return.
                $thquizdetails = helper_for_get_mods_by_courses::standard_coursemodule_element_values(
                        $thquiz, 'mod_thquiz', 'mod/thquiz:view', 'mod/thquiz:view');

                if (has_capability('mod/thquiz:view', $context)) {
                    $thquizdetails['introfiles'] = external_util::get_area_files($context->id, 'mod_thquiz', 'intro', false, false);
                    ...
                    $hasfeedback = thquiz_has_feedback($thquiz);
                    $thquizdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $thquizobj = thquiz::create($thquiz->id, $USER->id);
                    $accessmanager = new thquiz_access_manager($thquizobj, $timenow, has_capability('mod/thquiz:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the thquiz.
                    if (!$accessmanager->prevent_access()) {
                        $thquizdetails['hasquestions'] = (int) $thquizobj->has_questions();
                        $thquizdetails['autosaveperiod'] = get_config('thquiz', 'autosaveperiod');
                        ...
                        // Any course module fields that previously existed in thquiz.
                        $thquizdetails['completionpass'] = $thquizobj->get_cm()->completionpassgrade;
                    }
                    ...
                    foreach ($viewablefields as $field) {
                        $thquizdetails[$field] = $thquiz->{$field};
                    }
                }
                $returnedthquizzes[] = $thquizdetails;
            }
        }
        $result = array();
        $result['thquizzes'] = $returnedthquizzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_thquizzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_thquizzes_by_courses_returns() {
        return new external_single_structure(
            array(
                'thquizzes' => new external_multiple_structure(
                    new external_single_structure(array_merge(
                        helper_for_get_mods_by_courses::standard_coursemodule_elements_returns(true),
                        [
                            'timeopen' => new external_value(PARAM_INT, 'The time when this thquiz opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this thquiz closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for thquiz attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a thquiz attempt.', VALUE_OPTIONAL),
                            ...
                            'attemptonlast' => new external_value(PARAM_INT, 'Whether subsequent attempts start from the answer
                                                                    ...
                                                                    VALUE_OPTIONAL),
                            'grademethod' => new external_value(PARAM_INT, 'One of the values THQUIZ_GRADEHIGHEST, THQUIZ_GRADEAVERAGE,
                                                                    THQUIZ_ATTEMPTFIRST or THQUIZ_ATTEMPTLAST.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their thquiz
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_thquiz_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their thquiz
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their thquiz attempts
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the thquiz overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the thquiz was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a thquiz attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this thquiz can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark thquiz complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the thquiz to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the thquiz has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the thquiz has questions', VALUE_OPTIONAL),
                        ]
                    ))
                ),
    ...
    /**
     * Utility function for validating a thquiz.
     *
     * @param int $thquizid thquiz instance id
     * @return array array containing the thquiz, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_thquiz($thquizid) {
        global $DB;

        // Request and permission validation.
        $thquiz = $DB->get_record('thquiz', array('id' => $thquizid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($thquiz, 'thquiz');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($thquiz, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_thquiz.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_thquiz_parameters() {
        return new external_function_parameters (
            array(
                'thquizid' => new external_value(PARAM_INT, 'thquiz instance id'),
            )
        );
    }
     ...
     */
    public static function view_thquiz($thquizid) {
        global $DB;

        $params = self::validate_parameters(self::view_thquiz_parameters(), array('thquizid' => $thquizid));
        $warnings = array();

        list($thquiz, $course, $cm, $context) = self::validate_thquiz($params['thquizid']);
    ...
    /**
     * Describes the view_thquiz return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_thquiz_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'thquiz status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given thquiz and user.
     *
     * @param int $thquizid thquiz instance id
     * @param int $userid user id
     * @param string $status thquiz status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($thquizid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/thquiz:viewreports', $context);
        }

        // Update thquiz with override information.
        $thquiz = thquiz_update_effective_access($thquiz, $params['userid']);
        $attempts = thquiz_get_user_attempts($thquiz->id, $user->id, $params['status'], $params['includepreviews']);
        $attemptresponse = [];
        foreach ($attempts as $attempt) {
            $reviewoptions = thquiz_get_review_options($thquiz, $attempt, $context);
            if (!has_capability('mod/thquiz:viewreports', $context) &&
                    ($reviewoptions->marks < question_display_options::MARK_AND_MAX || $attempt->state != thquiz_attempt::FINISHED)) {
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'thquiz' => new external_value(PARAM_INT, 'Foreign key reference to the thquiz that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this thquiz.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this thquiz
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time thquiz cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a thquiz.
     *
     * @param int $thquizid thquiz instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($thquizid, $userid = 0) {
        global $DB, $USER, $CFG;
        ...
        // This code was mostly copied from mod/thquiz/view.php. We need to make the web service logic consistent.
        // Get this user's attempts.
        $attempts = thquiz_get_user_attempts($thquiz->id, $user->id, 'all');
        $canviewgrade = false;
        if ($attempts) {
            if ($USER->id != $user->id) {
                // No need to check the permission here. We did it at by require_capability('mod/thquiz:viewreports', $context).
                $canviewgrade = true;
            } else {
                // Work out which columns we need, taking account what data is available in each attempt.
                [$notused, $alloptions] = thquiz_get_combined_reviewoptions($thquiz, $attempts);
                $canviewgrade = $alloptions->marks >= question_display_options::MARK_AND_MAX;
            }
        }

        $grade = $canviewgrade ? thquiz_get_best_grade($thquiz, $user->id) : null;
        ...
        $gradinginfo = grade_get_grades($course->id, 'mod', 'thquiz', $thquiz->id, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given thquiz.'),
                'grade' => new external_value(PARAM_FLOAT, 'The grade (only if the user has a grade).', VALUE_OPTIONAL),
                'gradetopass' => new external_value(PARAM_FLOAT, 'The grade to pass the thquiz (only if set).', VALUE_OPTIONAL),
                'warnings' => new external_warnings(),
            )
        );
    }
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($thquizid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = thquiz_get_user_attempts($thquiz->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a thquiz.
     *
     * @param int $thquizid thquiz instance id
     ...
     */
    public static function start_attempt($thquizid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$thquizobj->has_questions()) {
            throw new moodle_thquiz_exception($thquizobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            thquiz_validate_new_attempt($thquizobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$thquizobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_thquiz_exception($thquizobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == thquiz_attempt::OVERDUE) {
                    throw new moodle_thquiz_exception($thquizobj, 'stateoverdue');
                } else {
                    throw new moodle_thquiz_exception($thquizobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = thquiz_prepare_and_start_new_attempt($thquizobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = thquiz_attempt::create($params['attemptid']);
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'notyourattempt');
        }

        // General capabilities check.
        $ispreviewuser = $attemptobj->is_preview_user();
        if (!$ispreviewuser) {
            $attemptobj->require_capability('mod/thquiz:attempt');
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == thquiz_attempt::OVERDUE) {
            throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'stateoverdue');
        }

        // User submitted data (like the thquiz password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'Out of sequence access');
            }
            ...
            if (empty($slots)) {
                throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'noquestionsfound');
            }
        }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the thquiz display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the thquiz this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the thquiz', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the thquiz display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the thquiz display settings.'
        );
    }
     ...
     *
     * @param  thquiz_attempt  $attemptobj  the thquiz attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(thquiz_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_thquizobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_thquiz');
        $contextid = $attemptobj->get_thquizobj()->get_context()->id;
    ...
    /**
     * Returns information for the given attempt page for a thquiz attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_thquiz_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/thquiz:preview capability,
    ...
    /**
     * Returns a summary of a thquiz attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the thquiz. This function is intended for the thquiz auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a thquiz and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    public static function process_attempt($attemptid, $data, $finishattempt = false, $timeup = false, $preflightdata = array()) {
        ...
        $attemptobj = thquiz_attempt::create($params['attemptid']);
        $failifoverdue = !($attemptobj->get_thquizobj()->get_thquiz()->overduehandling == 'graceperiod');
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'attemptclosed');
            } else if (!$displayoptions->attempt) {
                throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'noreview', null, '',
                    $attemptobj->cannot_review_message());
            }
        } else if (!$attemptobj->is_review_allowed()) {
            throw new moodle_thquiz_exception($attemptobj->get_thquizobj(), 'noreviewattempt');
        }
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_thquizobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = thquiz_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_thquiz(), false);

        $feedback = $attemptobj->get_overall_feedback($grade);
        if ($displayoptions->overallfeedback && $feedback) {
            $result['additionaldata'][] = array(
                'id' => 'feedback',
                'title' => get_string('feedback', 'thquiz'),
                'content' => $feedback,
            );
        }
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the thquiz (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_thquiz_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'thquizid' => new external_value(PARAM_INT, 'thquiz instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given thquiz.
     *
     * @param int $thquizid thquiz instance id
     ...
     * @throws moodle_exception
     */
    public static function get_thquiz_feedback_for_grade($thquizid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_thquiz_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = thquiz_feedback_record_for_grade($params['grade'], $thquiz);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_thquiz', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_thquiz', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_thquiz_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_thquiz_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_thquiz_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_thquiz_access_information_parameters() {
        return new external_function_parameters (
            array(
                'thquizid' => new external_value(PARAM_INT, 'thquiz instance id')
            )
    ...
    /**
     * Return access information for a given thquiz.
     *
     * @param int $thquizid thquiz instance id
     ...
     * @throws  moodle_thquiz_exception
     */
    public static function get_thquiz_access_information($thquizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_thquiz_access_information_parameters(), $params);

        list($thquiz, $course, $cm, $context) = self::validate_thquiz($params['thquizid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/thquiz:attempt', $context);;
        $result['canmanage'] = has_capability('mod/thquiz:manage', $context);;
        $result['canpreview'] = has_capability('mod/thquiz:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/thquiz:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/thquiz:viewreports', $context);;

        // Access manager now.
        $thquizobj = thquiz::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/thquiz:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new thquiz_access_manager($thquizobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_thquiz_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_thquiz_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the thquiz or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the thquiz settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the thquiz or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the thquiz reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a thquiz.
     *
     * @param int $thquizid thquiz instance id
     ...
     * @throws  moodle_thquiz_exception
     */
    public static function get_attempt_access_information($thquizid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = thquiz_get_user_attempt_unfinished($thquiz->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != thquiz_attempt::IN_PROGRESS and $unfinishedattempt->state != thquiz_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_thquiz_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_thquiz_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'thquizid' => new external_value(PARAM_INT, 'thquiz instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given thquiz.
     ...
     * @throws  moodle_thquiz_exception
     */
    public static function get_thquiz_required_qtypes($thquizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_thquiz_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $thquizobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_thquiz_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_thquiz_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the thquiz'),
                'warnings' => new external_warnings(),
            )
        );
    }
