...
/**
 * Implementaton of the thquizaccess_ipaddress plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquizaccess_ipaddress extends thquiz_access_rule_base {

    public static function make(thquiz $thquizobj, $timenow, $canignoretimelimits) {
        if (empty($thquizobj->get_thquiz()->subnet)) {
            return null;
        }
    ...
    public function prevent_access() {
        if (address_in_subnet(getremoteaddr(), $this->thquiz->subnet)) {
            return false;
        } else {
            return get_string('subnetwrong', 'thquizaccess_ipaddress');
        }
    }
}
