...
/**
 * Implementation of the thquizaccess_seb plugin.
 *
...
use thquizaccess_seb\access_manager;
use thquizaccess_seb\thquiz_settings;
use thquizaccess_seb\settings_provider;
use \thquizaccess_seb\event\access_prevented;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquizaccess_seb extends thquiz_access_rule_base {

    /** @var access_manager $accessmanager Instance to manage the access to the thquiz for this plugin. */
    private $accessmanager;

    /**
     * Create an instance of this rule for a particular thquiz.
     *
     * @param thquiz $thquizobj information about the thquiz in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param access_manager $accessmanager the thquiz accessmanager.
     */
    public function __construct(thquiz $thquizobj, int $timenow, access_manager $accessmanager) {
        parent::__construct($thquizobj, $timenow);
        $this->accessmanager = $accessmanager;
    }

    /**
     * Return an appropriately configured instance of this rule, if it is applicable
     * to the given thquiz, otherwise return null.
     *
     * @param thquiz $thquizobj information about the thquiz in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/thquiz:ignoretimelimits capability.
     * @return thquiz_access_rule_base|null the rule, if applicable, else null.
     */
    public static function make (thquiz $thquizobj, $timenow, $canignoretimelimits) {
        $accessmanager = new access_manager($thquizobj);
        // If Safe Exam Browser is not required, this access rule is not applicable.
        if (!$accessmanager->seb_required()) {
            return null;
        }

        return new self($thquizobj, $timenow, $accessmanager);
    }

    /**
     * Add any fields that this rule requires to the thquiz settings form. This
     * method is called from {@link mod_thquiz_mod_form::definition()}, while the
     ...
     */
    public static function add_settings_form_fields(mod_thquiz_mod_form $thquizform, MoodleQuickForm $mform) {
        settings_provider::add_seb_settings_fields($thquizform, $mform);
    }
     ...
     * @param mod_thquiz_mod_form $thquizform the thquiz form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
                                                         array $data, $files, mod_thquiz_mod_form $thquizform) : array {

        $thquizid = $data['instance'];
        $cmid = $data['coursemodule'];
        $context = $thquizform->get_context();

        if (!settings_provider::can_configure_seb($context)) {
            return $errors;
        }

        if (settings_provider::is_seb_settings_locked($thquizid)) {
            return $errors;
        }
        ...
        $thquizsettings = (new thquiz_settings())->from_record($settings);
        // Set non-form fields.
        $thquizsettings->set('thquizid', $thquizid);
        $thquizsettings->set('cmid', $cmid);
        $thquizsettings->validate();

        // Add any errors to list.
        foreach ($thquizsettings->get_errors() as $name => $error) {
        ...
        if ($thquizsettings->get('requiresafeexambrowser') == settings_provider::USE_SEB_UPLOAD_CONFIG) {
        ...
        if ($thquizsettings->get('requiresafeexambrowser') == settings_provider::USE_SEB_TEMPLATE) {
            if (empty($data['seb_templateid'])) {
                $errors['seb_templateid'] = get_string('invalidtemplate', 'thquizaccess_seb');
            }
        }

        if ($thquizsettings->get('requiresafeexambrowser') != settings_provider::USE_SEB_NO) {
            // Global settings may be active which require a thquiz password to be set if using SEB.
            if (!empty(get_config('thquizaccess_seb', 'thquizpasswordrequired')) && empty($data['thquizpassword'])) {
                $errors['thquizpassword'] = get_string('passwordnotset', 'thquizaccess_seb');
            }
        }
    ...
    /**
     * Save any submitted settings when the thquiz settings form is submitted. This
     * is called from {@link thquiz_after_add_or_update()} in lib.php.
     *
     * @param object $thquiz the data from the thquiz form, including $thquiz->id
     *      which is the id of the thquiz being saved.
     */
    public static function save_settings($thquiz) {
        ...
        if (settings_provider::is_seb_settings_locked($thquiz->id)) {
            return;
        }
        ...
        $cm = get_coursemodule_from_instance('thquiz', $thquiz->id, $thquiz->course, false, MUST_EXIST);

        $settings = settings_provider::filter_plugin_settings($thquiz);
        $settings->thquizid = $thquiz->id;
        $settings->cmid = $cm->id;

        // Get existing settings or create new settings if none exist.
        $thquizsettings = thquiz_settings::get_by_thquiz_id($thquiz->id);
        if (empty($thquizsettings)) {
            $thquizsettings = new thquiz_settings(0, $settings);
        } else {
            $settings->id = $thquizsettings->get('id');
        ...
        if ($thquizsettings->get('requiresafeexambrowser') != settings_provider::USE_SEB_NO) {
            $thquizsettings->save();
        } else if ($thquizsettings->get('id')) {
            $thquizsettings->delete();
        }
    }

    /**
     * Delete any rule-specific settings when the thquiz is deleted. This is called
     * from {@link thquiz_delete_instance()} in lib.php.
     *
     * @param object $thquiz the data from the database, including $thquiz->id
     *      which is the id of the thquiz being deleted.
     */
    public static function delete_settings($thquiz) {
        $thquizsettings = thquiz_settings::get_by_thquiz_id($thquiz->id);
        // Check that there are existing settings.
        if ($thquizsettings !== false) {
            $thquizsettings->delete();
        }
    }
     ...
     * here is probalby to read the code of {@link thquiz_access_manager::load_settings()}.
     *
     * If you have some settings that cannot be loaded in this way, then you can
     * use the {@link get_extra_settings()} method instead, but that has
     * performance implications.
     *
     * @param int $thquizid the id of the thquiz we are loading settings for. This
     *     can also be accessed as thquiz.id in the SQL. (thquiz is a table alisas for {thquiz}.)
     ...
     */
    public static function get_settings_sql($thquizid) : array {
        return [
                ...
                , 'LEFT JOIN {thquizaccess_seb_settings} seb ON seb.thquizid = thquiz.id '
                . 'LEFT JOIN {thquizaccess_seb_template} sebtemplate ON seb.templateid = sebtemplate.id '
                , []
        ];
    }
    ...
    public function prevent_access() {
        ...
        // If the rule is active, enforce a secure view whilst taking the thquiz.
        $PAGE->set_pagelayout('secure');
        $this->prevent_display_blocks();

        // Access has previously been validated for this session and thquiz.
    ...
    private function get_user_finished_attempts() : array {
        ...
        return thquiz_get_user_attempts(
            $this->thquizobj->get_thquizid(),
            $USER->id,
            thquiz_attempt::FINISHED,
            false
        );
    }
    ...
    private function prevent_display_blocks() {
        ...
        if ($PAGE->has_set_url() && $PAGE->url == $this->thquizobj->view_url()) {
            $attempts = $this->get_user_finished_attempts();

            // Don't display blocks before starting an attempt.
            if (empty($attempts) && !get_config('thquizaccess_seb', 'displayblocksbeforestart')) {
                $PAGE->blocks->show_only_fake_blocks();
            }

            // Don't display blocks after finishing an attempt.
            if (!empty($attempts) && !get_config('thquizaccess_seb', 'displayblockswhenfinished')) {
                $PAGE->blocks->show_only_fake_blocks();
            }
        }
    }
    ...
    private function get_reason_text(string $identifier) : string {
        if (in_array($identifier, ['not_seb', 'invalid_config_key', 'invalid_browser_key'])) {
            return get_string($identifier, 'thquizaccess_seb');
        }

        return get_string('unknown_reason', 'thquizaccess_seb');
    }
    ...
    private function get_invalid_key_error_message() : string {
        ...
        return get_string('invalidkeys', 'thquizaccess_seb')
    ...
    private function get_require_seb_error_message() : string {
        $message = get_string('clientrequiresseb', 'thquizaccess_seb');
    ...
    private function get_quit_button() : string {
        ...
        if (!empty($this->thquiz->seb_linkquitseb)) {
            $quitbutton = html_writer::link(
                $this->thquiz->seb_linkquitseb,
                get_string('exitsebbutton', 'thquizaccess_seb'),
    ...
    /**
     * Information, such as might be shown on the thquiz view page, relating to this restriction.
    ...
    public function description() : array {
        ...
        $messages = [get_string('sebrequired', 'thquizaccess_seb')];
        ...
        if (!$this->prevent_access()) {
            ...
            $PAGE->requires->js_call_amd('thquizaccess_seb/validate_thquiz_access', 'init',
                [$this->thquiz->cmid, (bool)get_config('thquizaccess_seb', 'autoreconfigureseb')]);
        }
    ...
    public function setup_attempt_page($page) {
        $page->set_title($this->thquizobj->get_course()->shortname . ': ' . $page->title);
    ...
    /**
     * This is called when the current attempt at the thquiz is finished.
     */
    ...
    private function get_action_buttons() : string {
        ...
        $linkconfig = explode(',', get_config('thquizaccess_seb', 'showseblinks'));
    ...
    private function get_download_seb_button() : string {
        ...
        if (!empty($this->get_seb_download_url())) {
            $button = $OUTPUT->single_button($this->get_seb_download_url(), get_string('sebdownloadbutton', 'thquizaccess_seb'));
        }
    ...
    private function get_launch_seb_button() : string {
        ...
        $seblink = \thquizaccess_seb\link_generator::get_link($this->thquiz->cmid, true, is_https());

        $buttonlink = html_writer::start_tag('div', array('class' => 'singlebutton'));
        $buttonlink .= html_writer::link($seblink, get_string('seblinkbutton', 'thquizaccess_seb'),
            ['class' => 'btn btn-secondary', 'title' => get_string('seblinkbutton', 'thquizaccess_seb')]);
    ...
    private function get_download_config_button() : string {
        // Rendering as a href and not as button in a form to circumvent browser warnings for sending to URL with unknown protocol.
        $httplink = \thquizaccess_seb\link_generator::get_link($this->thquiz->cmid, false, is_https());

        $buttonlink = html_writer::start_tag('div', array('class' => 'singlebutton'));
        $buttonlink .= html_writer::link($httplink, get_string('httplinkbutton', 'thquizaccess_seb'),
            ['class' => 'btn btn-secondary', 'title' => get_string('httplinkbutton', 'thquizaccess_seb')]);
        $buttonlink .= html_writer::end_tag('div');
    ...
    private function get_seb_download_url() : string {
        return get_config('thquizaccess_seb', 'downloadlink');
    }
    ...
    private function should_display_download_seb_link() : bool {
        return !empty($this->thquiz->seb_showsebdownloadlink);
    }
}