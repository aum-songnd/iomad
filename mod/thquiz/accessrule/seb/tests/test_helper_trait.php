...
global $CFG;
require_once($CFG->dirroot . "/mod/thquiz/accessrule/seb/rule.php"); // Include plugin rule class.
require_once($CFG->dirroot . "/mod/thquiz/mod_form.php"); // Include plugin rule class.
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
trait thquizaccess_seb_test_helper_trait {
     ...
     * @param int $requiresafeexambrowser How to use SEB for this thquiz?
     * @return  array
     */
    protected function create_test_thquiz($course, $requiresafeexambrowser = settings_provider::USE_SEB_NO) {
        ...
        $thquiz->seb_showsebdownloadlink = 1;
        $thquiz->coursemodule = $thquiz->cmid;
     ...
     * @return \thquizaccess_seb\template Just created template.
     */
    public function create_template(string $xml = null) {
        ...
        return $this->getDataGenerator()->get_plugin_generator('thquizaccess_seb')->create_template($data);
    }

    /**
     * Get access manager for testing.
     *
     * @return \thquizaccess_seb\access_manager
     */
    protected function get_access_manager() {
        return new access_manager(new \thquiz($this->thquiz,
            get_coursemodule_from_id('thquiz', $this->thquiz->cmid), $this->course));
    }

    /**
     * A helper method to make the rule form the currently created thquiz and  course.
     *
     * @return \thquiz_access_rule_base|null
     */
    protected function make_rule() {
        return \thquizaccess_seb::make(
            new \thquiz($this->thquiz, get_coursemodule_from_id('thquiz', $this->thquiz->cmid), $this->course),
            0,
            true
        );
    }
    ...
    /**
     * A helper method to set up thquiz view page.
     */
    protected function set_up_thquiz_view_page() {
        global $PAGE;

        $page = new \moodle_page();
        $page->set_context(\context_module::instance($this->thquiz->cmid));
        $page->set_course($this->course);
        $page->set_pagelayout('standard');
        $page->set_pagetype("mod-thquiz-view");
        $page->set_url('/mod/thquiz/view.php?id=' . $this->thquiz->cmid);
