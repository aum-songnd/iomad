...
namespace thquizaccess_seb\event;
...
class events_test extends \advanced_testcase {
     ...
     *
     * @covers \thquizaccess_seb\event\access_prevented
     */
    public function test_event_access_prevented() {
        $this->resetAfterTest();

        $this->setAdminUser();
        $thquiz = $this->create_test_thquiz($this->course, \thquizaccess_seb\settings_provider::USE_SEB_CONFIG_MANUALLY);
        $accessmanager = new \thquizaccess_seb\access_manager(new thquiz($thquiz,
            get_coursemodule_from_id('thquiz', $thquiz->cmid), $this->course));
        ...
        $event = \thquizaccess_seb\event\access_prevented::create_strict($accessmanager, 'Because I said so.');
        ...
        $this->assertEquals('Thquiz access was prevented', $event->get_name());
        $this->assertEquals(
            "The user with id '$user->id' has been prevented from accessing thquiz with id '$thquiz->id' by the "
        ...
        $this->assertEquals($thquiz->id, $event->objectid);
     ...
     *
     * @covers \thquizaccess_seb\event\template_created
     */
    public function test_event_create_template() {
        ...
        $event = \thquizaccess_seb\event\template_created::create_strict(
            $template,
        ...
        // Test that the event data is as expected.
        $this->assertInstanceOf('\thquizaccess_seb\event\template_created', $event);
