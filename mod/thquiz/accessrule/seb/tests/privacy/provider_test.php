...
use thquizaccess_seb\privacy\provider;
...
class provider_test extends provider_testcase {
    ...
    /**
     * Setup the user, the thquiz and ensure that the user is the last user to modify the SEB thquiz settings.
     */
    public function setup_test_data() {
        ...
        $this->thquiz = $this->create_test_thquiz($this->course, \thquizaccess_seb\settings_provider::USE_SEB_CONFIG_MANUALLY);
        ...
        $thquizsettings->set('requiresafeexambrowser', \thquizaccess_seb\settings_provider::USE_SEB_TEMPLATE);
    ...
    public function test_get_contexts_for_userid() {
        ...
        $this->assertEquals(\context_module::instance($this->thquiz->cmid)->id, reset($contextids));
    }

    /**
     * That that no module context is found for a user who has not modified any thquiz settings.
     */
    ...
    public function test_export_user_data() {
        ...
        // the thquiz and not anything with the same instance id.
        ...
        $index = '1'; // Get first data returned from the thquizsettings table metadata.
    ...
    public function test_get_users_in_context() {
        ...
        // Create empty userlist with thquiz module context.
        $userlist = new userlist(\context_module::instance($this->thquiz->cmid), 'thquizaccess_seb');
    ...
    public function test_delete_data_for_users() {
        ...
        $approveduserlist = new approved_userlist(\context_module::instance($this->thquiz->cmid),
                'thquizaccess_seb', [$this->user->id]);

        // Test data exists.
        $this->assertNotEmpty(thquiz_settings::get_record(['thquizid' => $this->thquiz->id]));

        // Test data is deleted.
        provider::delete_data_for_users($approveduserlist);
        $record = thquiz_settings::get_record(['thquizid' => $this->thquiz->id]);
        $this->assertEmpty($record->get('usermodified'));

        $template = \thquizaccess_seb\template::get_record(['id' => $record->get('templateid')]);
