 ...
 * @covers \thquizaccess_seb\access_manager
 */
class access_manager_test extends \advanced_testcase {
    ...
    /**
     * Test access_manager private property thquizsettings is null.
     */
    public function test_access_manager_thquizsettings_null() {
        ...
        $reflection = new \ReflectionClass('\thquizaccess_seb\access_manager');
        $property = $reflection->getProperty('thquizsettings');
    ...
    /**
     * Test that the thquiz Config Key matches the incoming request header.
     */
    public function test_access_keys_validate_with_config_key() {
        ...
        $configkey = thquiz_settings::get_record(['thquizid' => $this->thquiz->id])->get_config_key();
    ...
    /**
     * Test that the thquiz Config Key matches a provided config key with no incoming request header.
     */
    ...
    /**
     * Test that the thquiz Config Key does not match the incoming request header.
     */
    ...
    /**
     * Test that if there are no browser exam keys for thquiz, check is skipped.
     */
    ...
    /**
     * Test can correctly get type of SEB usage for the thquiz.
     */
    ...
    public function test_access_manager_uses_cached_config_key() {
        ...
        $this->assertNotEquals($thquizsettings->get_config_key(), $configkey);
        $this->assertTrue($accessmanager->validate_config_key());
    ...
    /**
     * Check that valid SEB config key is null if thquiz doesn't have SEB settings.
     */
    public function test_valid_config_key_is_null_if_no_settings() {
        ...
        $this->assertEmpty(thquiz_settings::get_record(['thquizid' => $this->thquiz->id]));
    ...
    public function test_set_session_access() {
        ...
        $this->assertTrue($SESSION->thquizaccess_seb_access[$this->thquiz->cmid]);
    }
    ...
    public function test_session_access_set_for_specific_course_module() {
        ...
        $this->thquiz = $this->create_test_thquiz($this->course, settings_provider::USE_SEB_CLIENT_CONFIG);
        $thquiz2 = $this->create_test_thquiz($this->course, settings_provider::USE_SEB_CLIENT_CONFIG);
        $accessmanager = $this->get_access_manager();

        $accessmanager->set_session_access(true);

        $this->assertCount(1, $SESSION->thquizaccess_seb_access);
        $this->assertTrue($SESSION->thquizaccess_seb_access[$this->thquiz->cmid]);
        $this->assertTrue(empty($SESSION->thquizaccess_seb_access[$thquiz2->cmid]));
    }
    ...
    public function test_should_redirect_to_seb_config_link() {
        ...
        set_config('autoreconfigureseb', '1', 'thquizaccess_seb');
        $_SERVER['HTTP_USER_AGENT'] = 'SEB';
