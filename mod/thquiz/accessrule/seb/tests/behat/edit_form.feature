@javascript @mod_thquiz @thquizaccess @thquizaccess_seb
Feature: Safe Exam Browser settings in thquiz edit form
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | thquiz     | C1     | 1       | Thquiz 1 |

  Scenario: Thquiz setting "Require the use of Safe Exam Browser" has all types, except "Use an existing template".
    When I am on the "Thquiz 1" "thquiz activity editing" page logged in as admin
  ...
  Scenario: Thquiz setting "Require the use of Safe Exam Browser" has all types if at least one template has been added.
    Given the following "thquizaccess_seb > seb templates" exist:
      | name       |
      | Template 1 |
    When I am on the "Thquiz 1" "thquiz activity editing" page logged in as admin
    And I expand all fieldsets
  ...
  Scenario: Thquiz can be edited without capability to select SEB template
    Given the following "permission override" exists:
      | role         | editingteacher                       |
      | capability   | thquizaccess/seb:manage_seb_templateid |
      | permission   | Prevent                              |
      | contextlevel | System                               |
      | reference    |                                      |
    ...
    # Create the thquiz.
    When I add a "Thquiz" to section "0" and I fill the form with:
      | Name | My thquiz |
    Then I should not see "Adding a new Thquiz"
    # Edit the thquiz.
    And I open "My thquiz" actions menu
    And I click on "Edit settings" "link" in the "My thquiz" activity
    And I set the field "Name" to "My thquiz edited"
    And I press "Save and return to course"
    And I should not see "Updating: Thquiz"

  Scenario: SEB settings if using No SEB
    Given the following "thquizaccess_seb > seb templates" exist:
      | name       |
      | Template 1 |
    And I am on the "Thquiz 1" "thquiz activity editing" page logged in as admin
    And I expand all fieldsets
