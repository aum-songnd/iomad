...
class backup_restore_test extends \advanced_testcase {
    ...
    /**
     * A helper method to create a thquiz with template usage of SEB.
     *
     * @return thquiz_settings
     */
    protected function create_thquiz_with_template() {
        ...
        $thquizsettings->set('templateid', $this->template->get('id'));
        $thquizsettings->save();

        return $thquizsettings;
    }

    /**
     * A helper method to emulate backup and restore of the thquiz.
     *
     * @return \cm_info|null
     */
    protected function backup_and_restore_thquiz() {
        return duplicate_module($this->course, get_fast_modinfo($this->course)->get_cm($this->thquiz->cmid));
    }
     ...
     */
    protected function backup_thquiz() {
        global $CFG;
        ...
        $bc = new \backup_controller(\backup::TYPE_1ACTIVITY, $this->thquiz->coursemodule, \backup::FORMAT_MOODLE,
     ...
     */
    protected function restore_thquiz($backupid) {
    ...
    protected function validate_backup_restore(\cm_info $newcm) {
        $this->assertEquals(2, thquiz_settings::count_records());
        $actual = thquiz_settings::get_record(['thquizid' => $newcm->instance]);
    ...
    public function test_backup_restore_no_seb() {
        ...
        $this->assertEquals(0, thquiz_settings::count_records());

        $this->backup_and_restore_thquiz();
        $this->assertEquals(0, thquiz_settings::count_records());
    }
    ...
    public function test_backup_restore_manual_config() {
        ...
        $this->assertEquals(1, thquiz_settings::count_records());

        $newcm = $this->backup_and_restore_thquiz();
    ...
    public function test_backup_restore_uploaded_config() {
        ...
        $expectedfile = settings_provider::get_module_context_sebconfig_file($this->thquiz->cmid);
    ...
    public function test_restore_template_to_a_different_site_when_the_same_template_exists() {
        $this->create_thquiz_with_template();
        $backupid = $this->backup_thquiz();

        $this->assertEquals(1, thquiz_settings::count_records());
        $this->assertEquals(1, template::count_records());

        $this->change_site();
        $this->restore_thquiz($backupid);
    ...
    public function test_restore_template_to_a_different_site_when_the_same_name_but_different_content() {
        ...
        $newxml = file_get_contents($CFG->dirroot . '/mod/thquiz/accessrule/seb/tests/fixtures/simpleunencrypted.seb');
