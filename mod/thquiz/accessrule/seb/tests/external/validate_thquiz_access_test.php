...
namespace thquizaccess_seb\external;
 ...
 * @covers \thquizaccess_seb\external\validate_thquiz_access
 */
class validate_thquiz_access_test extends \advanced_testcase {
    ...
    public function test_invalid_parameters($cmid, $url, $configkey, $messageregex) {
        ...
        \external_api::validate_parameters(validate_thquiz_keys::execute_parameters(), $params);
    }
    ...
    public function test_context_is_not_valid_for_user() {
        // Set user as user not enrolled in course and thquiz.
        $this->user = $this->getDataGenerator()->create_user();
        $this->setUser($this->user);

        $this->expectException(\require_login_exception::class);
        $this->expectExceptionMessage('Course or activity not accessible. (Not enrolled)');
        validate_thquiz_keys::execute($this->thquiz->cmid, 'https://www.example.com/moodle', 'configkey');
    }
    ...
    public function test_no_keys_provided() {
        ...
        validate_thquiz_keys::execute($this->thquiz->cmid, 'https://www.example.com/moodle');
    }

    /**
     * Test exception thrown if cmid doesn't match a thquiz.
     */
    public function test_thquiz_does_not_exist() {
        $this->setAdminUser();
        $forum = $this->getDataGenerator()->create_module('forum', ['course' => $this->course->id]);
        $this->expectException(\invalid_parameter_exception::class);
        $this->expectExceptionMessage('Thquiz not found matching course module ID: ' . $forum->cmid);
        validate_thquiz_keys::execute($forum->cmid, 'https://www.example.com/moodle', 'configkey');
    }
    ...
    public function test_config_key_valid() {
        ...
        // Test settings to populate the thquiz.
        ...
        // Create the thquiz settings.
        $thquizsettings = new thquiz_settings(0, $settings);
        $thquizsettings->save();

        $fullconfigkey = hash('sha256', $url . $thquizsettings->get_config_key());
        $result = validate_thquiz_keys::execute($this->thquiz->cmid, $url, $fullconfigkey);
    ...
    public function test_config_key_not_valid() {
        ...
        $result = validate_thquiz_keys::execute($this->thquiz->cmid, 'https://www.example.com/moodle', 'badconfigkey');
    ...
    public function test_browser_exam_key_valid() {
        ...
        $settings = $this->get_test_settings([
            'thquizid' => $this->thquiz->id,
            'cmid' => $this->thquiz->cmid,
            'requiresafeexambrowser' => \thquizaccess_seb\settings_provider::USE_SEB_CLIENT_CONFIG,
        ...
        $result = validate_thquiz_keys::execute($this->thquiz->cmid, $url, null, $fullbrowserexamkey);
        $this->assertTrue($result['configkey']);
        $this->assertTrue($result['browserexamkey']);
        $events = $sink->get_events();
        $this->assertCount(0, $events);
    ...
    public function test_browser_exam_key_not_valid() {
        ...
        $result = validate_thquiz_keys::execute($this->thquiz->cmid, 'https://www.example.com/moodle', null,
