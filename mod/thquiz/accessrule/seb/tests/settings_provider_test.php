...
class settings_provider_test extends \advanced_testcase {
    ...
    /**
     * Mocked thquiz form instance.
     * @var \mod_thquiz_mod_form
     */
    protected $mockedthquizform;
    ...
    protected function set_up_form_mocks() {
        ...
        $this->mockedthquizform = $this->createMock('mod_thquiz_mod_form');
        $this->mockedthquizform->method('get_context')->willReturn($this->context);
        $this->mockedthquizform->method('get_instance')->willReturn($this->thquiz->id);
    ...
    /**
     * Test that settings types to be added to thquiz settings, are part of thquiz_settings persistent class.
     */
    public function test_setting_elements_are_part_of_thquiz_settings_table() {
        $dbsettings = (array) (new thquiz_settings())->to_record();
        ...
        // Get all elements to be added to form, that are not in the persistent thquiz_settings class.
    ...
    public function test_hideifs() {
        $settinghideifs = settings_provider::get_thquiz_hideifs();
    ...
    public function test_build_setting_capability_name_incorrect_setting() {
        ...
        $this->expectExceptionMessage('Incorrect SEB thquiz setting broken');
    ...
    public function test_build_setting_capability_name_correct_setting() {
        foreach (settings_provider::get_seb_config_elements() as $name => $type) {
            $expected = 'thquizaccess/seb:manage_' . $name;
    ...
    public function test_get_requiresafeexambrowser_options($settingcapability) {
        ...
        assign_capability('thquizaccess/seb:manage_seb_templateid', CAP_ALLOW, $this->roleid, $this->context->id);
        $options = settings_provider::get_requiresafeexambrowser_options($this->context);
        $this->assertCount(4, $options);
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_CONFIG_MANUALLY, $options));
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_TEMPLATE, $options));
        ...
        assign_capability('thquizaccess/seb:manage_filemanager_sebconfigfile', CAP_ALLOW, $this->roleid, $this->context->id);
        $options = settings_provider::get_requiresafeexambrowser_options($this->context);
        $this->assertCount(5, $options);
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_CONFIG_MANUALLY, $options));
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_TEMPLATE, $options));
    ...
    public function test_get_requiresafeexambrowser_options_with_conflicting_permissions() {
        ...
        $settings = thquiz_settings::get_record(['thquizid' => $this->thquiz->id]);
    ...
    public function test_form_elements_are_frozen_if_conflicting_permissions() {
        ...
        assign_capability('thquizaccess/seb:manage_seb_requiresafeexambrowser', CAP_ALLOW, $this->roleid, $this->context->id);
        assign_capability('thquizaccess/seb:manage_seb_showsebdownloadlink', CAP_ALLOW, $this->roleid, $this->context->id);
        assign_capability('thquizaccess/seb:manage_seb_allowedbrowserexamkeys', CAP_ALLOW, $this->roleid, $this->context->id);

        $this->set_up_form_mocks();

        settings_provider::add_seb_settings_fields($this->mockedthquizform, $this->mockedform);
    ...
    /**
     * Test that All settings are frozen if thquiz was attempted and use seb with manual settings.
     */
    public function test_form_elements_are_locked_when_thquiz_attempted_manual() {
        $this->resetAfterTest();
        $this->course = $this->getDataGenerator()->create_course();
    ...
    /**
     * Test that All settings are frozen if a thquiz was attempted and use template.
     */
    public function test_form_elements_are_locked_when_thquiz_attempted_template() {
        $this->resetAfterTest();
        $this->setAdminUser();
        $this->course = $this->getDataGenerator()->create_course();
    ...
    public function test_validate_draftarea_configfile_failure() {
        ...
        $itemid = $this->create_test_draftarea_file($xml);
        $errors = settings_provider::validate_draftarea_configfile($itemid);
        $this->assertEquals($errors, new \lang_string('fileparsefailed', 'thquizaccess_seb'));
    }
    ...
    /**
     * Test saving files from the user draft area into the thquiz context area storage.
     */
    public function test_save_filemanager_sebconfigfile_draftarea() {
        ...
        settings_provider::save_filemanager_sebconfigfile_draftarea($draftitemid, $this->thquiz->cmid);

        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'thquizaccess_seb', 'filemanager_sebconfigfile');

        $this->assertCount(2, $files);
    }

    /**
     * Test deleting the $this->thquiz->cmid itemid from the file area.
     */
    public function test_delete_uploaded_config_file() {
        ...
        settings_provider::delete_uploaded_config_file($this->thquiz->cmid);
    ...
    public function test_get_module_context_sebconfig_file() {
        ...
        $file = settings_provider::get_module_context_sebconfig_file($this->thquiz->cmid);
    ...
    public function test_is_seb_settings_locked() {
        ...
        $this->assertFalse(settings_provider::is_seb_settings_locked($this->thquiz->id));

        $this->attempt_thquiz($this->thquiz, $user);
        $this->assertTrue(settings_provider::is_seb_settings_locked($this->thquiz->id));
    }
