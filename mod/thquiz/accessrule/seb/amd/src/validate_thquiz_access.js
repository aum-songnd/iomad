 ...
 *
 * @module     thquizaccess_seb/validate_thquiz_access
...
import * as View from 'thquizaccess_seb/view';
 ...
 *
 * @param {init} cmid Value of course module id of the thquiz.
 * @param {boolean} autoreconfigure Value of Moodle setting: thquizaccess_seb/autoreconfigureseb.
 */
const safeExamBrowserKeysUpdated = (cmid, autoreconfigure = false) => {
    // Action opening up the thquiz.
    isThquizAccessValid(cmid).then((response) => {
        ...
        if (response.configkey && response.browserexamkey) {
            ...
            // If autoreconfigureseb is enabled, attempt to reconfigure page with thquiz settings.
 ...
 * @return {Promise}
 */
const isThquizAccessValid = (cmid) => {
    const request = {
        methodname: 'thquizaccess_seb_validate_thquiz_keys',
        args: {
            cmid: cmid,
...
/**
 * Reload Safe Exam Browser with current thquiz configuration.
 *
 * @param {init} cmid Value of course module id of the thquiz.
 */
const reconfigureSafeExamBrowser = (cmid) => {
    const domain = Config.wwwroot.replace(/^http/i, 'seb');
    const redirecturl = domain + '/mod/thquiz/accessrule/seb/config.php?cmid=' + cmid;
