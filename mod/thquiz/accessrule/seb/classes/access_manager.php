...
/**
 * Manage the access to the thquiz.
 *
...
class access_manager {
    ...
    /** @var thquiz $thquiz A thquiz object containing all information pertaining to current thquiz. */
    private $thquiz;

    /** @var thquiz_settings $thquizsettings A thquiz settings persistent object containing plugin settings */
    private $thquizsettings;

    /** @var context_module $context Context of this thquiz activity. */
    private $context;
     ...
     *
     * @param thquiz $thquiz The details of the thquiz.
     */
    public function __construct(thquiz $thquiz) {
        $this->thquiz = $thquiz;
        $this->context = context_module::instance($thquiz->get_cmid());
        $this->thquizsettings = thquiz_settings::get_by_thquiz_id($thquiz->get_thquizid());
        $this->validconfigkey = thquiz_settings::get_config_key_by_thquiz_id($thquiz->get_thquizid());
    }
    ...
    public function validate_browser_exam_key(?string $browserexamkey = null, ?string $url = null): bool {
        ...
        $validbrowserexamkeys = $this->thquizsettings->get('allowedbrowserexamkeys');
    ...
    /**
     * Check if Safe Exam Browser is required to access thquiz.
     * If thquizsettings do not exist, then there is no requirement for using SEB.
     *
     * @return bool If required.
     */
    public function seb_required() : bool {
        if (!$this->thquizsettings) {
            return false;
        } else {
    ...
    /**
     * This is the basic check for the Safe Exam Browser previously used in the thquizaccess_safebrowser plugin that
    ...
    public function can_bypass_seb(): bool {
        return has_capability('thquizaccess/seb:bypassseb', $this->context);
    }
    ...
    /**
     * Getter for the thquiz object.
     *
     * @return thquiz
     */
    public function get_thquiz() : thquiz {
        return $this->thquiz;
    }

    /**
     * Check that at least one browser exam key exists in the thquiz settings.
     *
     * @return bool True if one or more keys are set in thquiz settings.
     */
    private function is_allowed_browser_examkeys_configured(): bool {
        return !empty($this->thquizsettings->get('allowedbrowserexamkeys'));
    }
    ...
    /**
     * Get type of SEB usage for the thquiz.
     *
     * @return int
     */
    public function get_seb_use_type(): int {
        if (empty($this->thquizsettings)) {
            return settings_provider::USE_SEB_NO;
        } else {
            return $this->thquizsettings->get('requiresafeexambrowser');
        }
    }
    ...
    /**
     * Set session access for thquiz.
     *
     * @param bool $accessallowed
     */
    public function set_session_access(bool $accessallowed): void {
        global $SESSION;
        if (!isset($SESSION->thquizaccess_seb_access)) {
            $SESSION->thquizaccess_seb_access = [];
        }
        $SESSION->thquizaccess_seb_access[$this->thquiz->get_cmid()] = $accessallowed;
    }

    /**
     * Check session access for thquiz if already set.
     *
     * @return bool
     */
    public function validate_session_access(): bool {
        global $SESSION;
        return !empty($SESSION->thquizaccess_seb_access[$this->thquiz->get_cmid()]);
    }

    /**
     * Unset the global session access variable for this thquiz.
     */
    public function clear_session_access(): void {
        global $SESSION;
        unset($SESSION->thquizaccess_seb_access[$this->thquiz->get_cmid()]);
    }
    ...
    public function redirect_to_seb_config_link(): void {
        ...
        $seblink = \thquizaccess_seb\link_generator::get_link($this->thquiz->get_cmid(), true, is_https());
    ...
    public function should_redirect_to_seb_config_link(): bool {
        ...
        return $this->is_using_seb()
                && get_config('thquizaccess_seb', 'autoreconfigureseb')
                && $haskeyinheader;
    }
}
