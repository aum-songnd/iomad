...
use thquiz;
use thquizaccess_seb\event\access_prevented;
 ...
 */
class validate_thquiz_keys extends external_api {
    ...
    public static function execute(string $cmid, string $url, ?string $configkey = null, ?string $browserexamkey = null): array {
        ...
        if (empty($configkey) && empty($browserexamkey)) {
            throw new invalid_parameter_exception(get_string('error:ws:nokeyprovided', 'thquizaccess_seb'));
        }

        // Check thquiz exists corresponding to cmid.
        if (($thquizid = self::get_thquiz_id($cmid)) === 0) {
            throw new invalid_parameter_exception(get_string('error:ws:thquiznotexists', 'thquizaccess_seb', $cmid));
        }

        $result = ['configkey' => true, 'browserexamkey' => true];

        $accessmanager = new access_manager(thquiz::create($thquizid));

        // Check if there is a valid config key.
        if (!$accessmanager->validate_config_key($configkey, $url)) {
            access_prevented::create_strict($accessmanager, get_string('invalid_config_key', 'thquizaccess_seb'),
        ...
        if (!$accessmanager->validate_browser_exam_key($browserexamkey, $url)) {
            access_prevented::create_strict($accessmanager, get_string('invalid_browser_key', 'thquizaccess_seb'),
                    $configkey, $browserexamkey)->trigger();
    ...
    /**
     * Check if there is a valid thquiz corresponding to a course module it.
     *
     * @param string $cmid Course module ID.
     * @return int Returns thquiz id if cmid matches valid thquiz, or 0 if there is no match.
     */
    private static function get_thquiz_id(string $cmid): int {
        $thquizid = 0;

        $coursemodule = get_coursemodule_from_id('thquiz', $cmid);
        if (!empty($coursemodule)) {
            $thquizid = $coursemodule->instance;
        }
