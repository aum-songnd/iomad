...
class helper {
    ...
    /**
     * Get seb config content for a particular thquiz. This method checks caps.
     *
     * @param string $cmid The course module ID for a thquiz with config.
    ...
    public static function get_seb_config_content(string $cmid) : string {
        ...
        // Retrieve the config for thquiz.
        $config = thquiz_settings::get_config_by_thquiz_id($cm->instance);
        if (empty($config)) {
            throw new \moodle_exception('noconfigfound', 'thquizaccess_seb', '', $cm->id);
        }
        return $config;
    }
