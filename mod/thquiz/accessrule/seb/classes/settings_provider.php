...
/**
 * Class for providing thquiz settings, to make setting up thquiz form manageable.
 *
...
/**
 * Helper class for providing thquiz settings, to make setting up thquiz form manageable.
 *
...
class settings_provider {
     ...
     * Insert form element.
     *
     * @param \mod_thquiz_mod_form $thquizform the thquiz settings form that is being built.
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     * @param \HTML_QuickForm_element $element Element to insert.
     * @param string $before Insert element before.
     */
    protected static function insert_element(\mod_thquiz_mod_form $thquizform,
     ...
     */
    protected static function remove_element(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform, string  $elementname) {
     ...
     * @param string $elementname Element name.
     */
    protected static function add_help_button(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform, string $elementname) {
        if ($mform->elementExists($elementname)) {
            $mform->addHelpButton($elementname, $elementname, 'thquizaccess_seb');
        }
    }
     ...
     */
    protected static function set_default(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform, string  $elementname, $value) {
     ...
     */
    protected static function set_type(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform, string $elementname, string $type) {
     ...
     * @param string $elementname Element name.
     */
    protected static function freeze_element(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform, string $elementname) {
        if ($mform->elementExists($elementname)) {
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_header_element(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        global  $OUTPUT;

        $element = $mform->createElement('header', 'seb', get_string('seb', 'thquizaccess_seb'));
        self::insert_element($thquizform, $mform, $element);

        // Display notification about locked settings.
        if (self::is_seb_settings_locked($thquizform->get_instance())) {
            $notify = new \core\output\notification(
                get_string('settingsfrozen', 'thquizaccess_seb'),
                \core\output\notification::NOTIFY_WARNING
            );

            $notifyelement = $mform->createElement('html', $OUTPUT->render($notify));
            self::insert_element($thquizform, $mform, $notifyelement);
        }

        if (self::is_conflicting_permissions($thquizform->get_context())) {
            $notify = new \core\output\notification(
                get_string('conflictingsettings', 'thquizaccess_seb'),
                \core\output\notification::NOTIFY_WARNING
            );
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_usage_options(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        $element = $mform->createElement(
            'select',
            'seb_requiresafeexambrowser',
            get_string('seb_requiresafeexambrowser', 'thquizaccess_seb'),
            self::get_requiresafeexambrowser_options($thquizform->get_context())
        );

        self::insert_element($thquizform, $mform, $element);
        self::set_type($thquizform, $mform, 'seb_requiresafeexambrowser', PARAM_INT);
        self::set_default($thquizform, $mform, 'seb_requiresafeexambrowser', self::USE_SEB_NO);
        self::add_help_button($thquizform, $mform, 'seb_requiresafeexambrowser');

        if (self::is_conflicting_permissions($thquizform->get_context())) {
            self::freeze_element($thquizform, $mform, 'seb_requiresafeexambrowser');
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_templates(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        if (self::can_use_seb_template($thquizform->get_context()) || self::is_conflicting_permissions($thquizform->get_context())) {
            $element = $mform->createElement(
                'select',
                'seb_templateid',
                get_string('seb_templateid', 'thquizaccess_seb'),
        ...
        self::insert_element($thquizform, $mform, $element);
        self::set_type($thquizform, $mform, 'seb_templateid', PARAM_INT);
        self::set_default($thquizform, $mform, 'seb_templateid', 0);
        self::add_help_button($thquizform, $mform, 'seb_templateid');

        // In case if the user can't use templates, but the thquiz is configured to use them,
        // we'd like to display template, but freeze it.
        if (self::is_conflicting_permissions($thquizform->get_context())) {
            self::freeze_element($thquizform, $mform, 'seb_templateid');
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_config_file(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        $itemid = 0;

        $draftitemid = 0;
        file_prepare_draft_area(
            $draftitemid,
            $thquizform->get_context()->id,
            'thquizaccess_seb',
            'filemanager_sebconfigfile',
            $itemid
        );

        if (self::can_upload_seb_file($thquizform->get_context())) {
            $element = $mform->createElement(
                'filemanager',
                'filemanager_sebconfigfile',
                get_string('filemanager_sebconfigfile', 'thquizaccess_seb'),
                null,
        ...
        self::insert_element($thquizform, $mform, $element);
        self::set_type($thquizform, $mform, 'filemanager_sebconfigfile', PARAM_RAW);
        self::set_default($thquizform, $mform, 'filemanager_sebconfigfile', $draftitemid);
        self::add_help_button($thquizform, $mform, 'filemanager_sebconfigfile');
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_show_download_link(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        if (self::can_change_seb_showsebdownloadlink($thquizform->get_context())) {
            $element = $mform->createElement('selectyesno',
                'seb_showsebdownloadlink',
                get_string('seb_showsebdownloadlink', 'thquizaccess_seb')
            );
            self::insert_element($thquizform, $mform, $element);
            self::set_type($thquizform, $mform, 'seb_showsebdownloadlink', PARAM_BOOL);
            self::set_default($thquizform, $mform, 'seb_showsebdownloadlink', 1);
            self::add_help_button($thquizform, $mform, 'seb_showsebdownloadlink');
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_allowedbrowserexamkeys(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        if (self::can_change_seb_allowedbrowserexamkeys($thquizform->get_context())) {
            $element = $mform->createElement('textarea',
                'seb_allowedbrowserexamkeys',
                get_string('seb_allowedbrowserexamkeys', 'thquizaccess_seb')
            );
            self::insert_element($thquizform, $mform, $element);
            self::set_type($thquizform, $mform, 'seb_allowedbrowserexamkeys', PARAM_RAW);
            self::set_default($thquizform, $mform, 'seb_allowedbrowserexamkeys', '');
            self::add_help_button($thquizform, $mform, 'seb_allowedbrowserexamkeys');
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function add_seb_config_elements(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        ...
        foreach (self::get_seb_config_elements() as $name => $type) {
            if (!self::can_manage_seb_config_setting($name, $thquizform->get_context())) {
                $type = 'hidden';
            }

            $element = $mform->createElement($type, $name, get_string($name, 'thquizaccess_seb'));
            self::insert_element($thquizform, $mform, $element);
            unset($element); // We need to make sure each &element only references the current element in loop.

            self::add_help_button($thquizform, $mform, $name);

            if (isset($defaults[$name])) {
                self::set_default($thquizform, $mform, $name, $defaults[$name]);
            }

            if (isset($types[$name])) {
                self::set_type($thquizform, $mform, $name, $types[$name]);
            }
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_seb_settings_fields(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        if (self::can_configure_seb($thquizform->get_context())) {
            self::add_seb_header_element($thquizform, $mform);
            self::add_seb_usage_options($thquizform, $mform);
            self::add_seb_templates($thquizform, $mform);
            self::add_seb_config_file($thquizform, $mform);
            self::add_seb_show_download_link($thquizform, $mform);
            self::add_seb_config_elements($thquizform, $mform);
            self::add_seb_allowedbrowserexamkeys($thquizform, $mform);
            self::hide_seb_elements($thquizform, $mform);
            self::lock_seb_elements($thquizform, $mform);
        }
    }
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function hide_seb_elements(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        foreach (self::get_thquiz_hideifs() as $elname => $rules) {
     ...
     * @param \MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    protected static function lock_seb_elements(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) {
        if (self::is_seb_settings_locked($thquizform->get_instance()) || self::is_conflicting_permissions($thquizform->get_context())) {
            // Freeze common thquiz settings.
            self::freeze_element($thquizform, $mform, 'seb_requiresafeexambrowser');
            self::freeze_element($thquizform, $mform, 'seb_templateid');
            self::freeze_element($thquizform, $mform, 'seb_showsebdownloadlink');
            self::freeze_element($thquizform, $mform, 'seb_allowedbrowserexamkeys');

            $thquizsettings = thquiz_settings::get_by_thquiz_id((int) $thquizform->get_instance());

            // If the file has been uploaded, then replace it with the link to download the file.
            if (!empty($thquizsettings) && $thquizsettings->get('requiresafeexambrowser') == self::USE_SEB_UPLOAD_CONFIG) {
                self::remove_element($thquizform, $mform, 'filemanager_sebconfigfile');
                if ($link = self::get_uploaded_seb_file_download_link($thquizform, $mform)) {
                    $element = $mform->createElement(
                        'static',
                        'filemanager_sebconfigfile',
                        get_string('filemanager_sebconfigfile', 'thquizaccess_seb'),
                    ...
                    self::insert_element($thquizform, $mform, $element, 'seb_showsebdownloadlink');
                }
            }

            // Remove template ID if not using template for this thquiz.
            if (empty($thquizsettings) || $thquizsettings->get('requiresafeexambrowser') != self::USE_SEB_TEMPLATE) {
                $mform->removeElement('seb_templateid');
            }

            // Freeze all SEB specific settings.
            foreach (self::get_seb_config_elements() as $element => $type) {
                self::freeze_element($thquizform, $mform, $element);
            }
        }
    }
     ...
     * @return string
     */
    protected static function get_uploaded_seb_file_download_link(\mod_thquiz_mod_form $thquizform, \MoodleQuickForm $mform) : string {
        $link = '';
        $file = self::get_module_context_sebconfig_file($thquizform->get_coursemodule()->id);
        ...
        if ($file) {
            ...
            $link = \html_writer::link($url, get_string('downloadsebconfig', 'thquizaccess_seb'));
        }
    ...
    /**
     * Get the type of element for each of the form elements in thquiz settings.
     *
     * Contains all setting elements. Array key is name of 'form element'/'database column (excluding prefix)'.
     *
     * @return array All thquiz form elements to be added and their types.
     */
    ...
    /**
     * Get the types of the thquiz settings elements.
     ...
     * use template (but still can update other settings) edit the same thquiz. This is
    ...
    public static function is_conflicting_permissions(\context $context) {
        ...
        $settings = thquiz_settings::get_record(['cmid' => (int) $context->instanceid]);
    ...
    public static function get_requiresafeexambrowser_options(\context $context) : array {
        ...
        if (self::can_configure_manually($context) || self::is_conflicting_permissions($context)) {
            $options[self::USE_SEB_CONFIG_MANUALLY] = get_string('seb_use_manually', 'thquizaccess_seb');
        }

        if (self::can_use_seb_template($context) || self::is_conflicting_permissions($context)) {
            if (!empty(self::get_template_options())) {
                $options[self::USE_SEB_TEMPLATE] = get_string('seb_use_template', 'thquizaccess_seb');
            }
        }

        if (self::can_upload_seb_file($context) || self::is_conflicting_permissions($context)) {
            $options[self::USE_SEB_UPLOAD_CONFIG] = get_string('seb_use_upload', 'thquizaccess_seb');
        }

        $options[self::USE_SEB_CLIENT_CONFIG] = get_string('seb_use_client', 'thquizaccess_seb');
    ...
    /**
     * Get the default values of the thquiz settings.
     *
    ...
    public static function validate_draftarea_configfile($itemid) {
        ...
        if ($file) {
            if (!helper::is_valid_seb_config($file->get_content())) {
                return new lang_string('fileparsefailed', 'thquizaccess_seb');
            }
        }

        // If we require an SEB config uploaded, and the file does not exist, error.
        if (!$file) {
            return new lang_string('filenotpresent', 'thquizaccess_seb');
        }
    }
    ...
    public static function get_module_context_sebconfig_file(string $cmid) : ?stored_file {
        ...
        if (!$files = $fs->get_area_files($context->id, 'thquizaccess_seb', 'filemanager_sebconfigfile', 0,
            'id DESC', false)) {
            return null;
        }
     ...
     * @param string $cmid The cmid of for the thquiz.
     * @return bool Always true
     */
    public static function save_filemanager_sebconfigfile_draftarea(string $draftitemid, string $cmid) : bool {
        if ($draftitemid) {
            $context = context_module::instance($cmid);
            file_save_draft_area_files($draftitemid, $context->id, 'thquizaccess_seb', 'filemanager_sebconfigfile',
                0, []);
        }
    ...
    public static function can_configure_seb(\context $context) : bool {
        return has_capability('thquizaccess/seb:manage_seb_requiresafeexambrowser', $context);
    }
    ...
    public static function can_use_seb_template(\context $context) : bool {
        return has_capability('thquizaccess/seb:manage_seb_templateid', $context);
    }
    ...
    public static function can_upload_seb_file(\context $context) : bool {
        return has_capability('thquizaccess/seb:manage_filemanager_sebconfigfile', $context);
    }
    ...
    public static function can_change_seb_showsebdownloadlink(\context $context) : bool {
        return has_capability('thquizaccess/seb:manage_seb_showsebdownloadlink', $context);
    }
    ...
    public static function can_change_seb_allowedbrowserexamkeys(\context $context) : bool {
        return has_capability('thquizaccess/seb:manage_seb_allowedbrowserexamkeys', $context);
    }
     ...
     */
    public static function get_thquiz_hideifs() : array {
        $hideifs = [];
    ...
    public static function build_setting_capability_name(string $settingname) : string {
        if (!key_exists($settingname, self::get_seb_config_elements())) {
            throw new \coding_exception('Incorrect SEB thquiz setting ' . $settingname);
        }

        return 'thquizaccess/seb:manage_' . $settingname;
    }

    /**
     * Check if settings is locked.
     *
     * @param int $thquizid Thquiz ID.
     * @return bool
     */
    public static function is_seb_settings_locked($thquizid) : bool {
        if (empty($thquizid)) {
            return false;
        }

        return thquiz_has_attempts($thquizid);
    }

    /**
     * Filter a standard class by prefix.
     *
     * @param stdClass $settings Thquiz settings object.
    ...
    /**
     * Filter thquiz settings for this plugin only.
     *
     * @param stdClass $settings Thquiz settings.
