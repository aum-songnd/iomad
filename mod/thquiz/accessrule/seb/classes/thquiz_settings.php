...
/**
 * Entity model representing thquiz settings for the seb plugin.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_settings extends persistent {

    /** Table name for the persistent. */
    const TABLE = 'thquizaccess_seb_settings';
    ...
    /**
     * Return an instance by thquiz id.
     *
     * This method gets data from cache before doing any DB calls.
     *
     * @param int $thquizid Thquiz id.
     * @return false|\thquizaccess_seb\thquiz_settings
     */
    public static function get_by_thquiz_id(int $thquizid) {
        if ($data = self::get_thquiz_settings_cache()->get($thquizid)) {
            return new static(0, $data);
        }

        return self::get_record(['thquizid' => $thquizid]);
    }

    /**
     * Return cached SEB config represented as a string by thquiz ID.
     *
     * @param int $thquizid Thquiz id.
     * @return string|null
     */
    public static function get_config_by_thquiz_id(int $thquizid) : ?string {
        $config = self::get_config_cache()->get($thquizid);
        ...
        $config = null;
        if ($settings = self::get_by_thquiz_id($thquizid)) {
            $config = $settings->get_config();
            self::get_config_cache()->set($thquizid, $config);
        }
    ...
    /**
     * Return cached SEB config key by thquiz ID.
     *
     * @param int $thquizid Thquiz id.
     * @return string|null
     */
    public static function get_config_key_by_thquiz_id(int $thquizid) : ?string {
        $configkey = self::get_config_key_cache()->get($thquizid);
        ...
        if ($settings = self::get_by_thquiz_id($thquizid)) {
            ...
            self::get_config_key_cache()->set($thquizid, $configkey);
        }
    ...
    private static function get_config_key_cache() : \cache_application {
        return \cache::make('thquizaccess_seb', 'configkey');
    }
    ...
    private static function get_config_cache() : \cache_application {
        return \cache::make('thquizaccess_seb', 'config');
    }

    /**
     * Return thquiz settings cache object,
     *
     * @return \cache_application
     */
    private static function get_thquiz_settings_cache() : \cache_application {
        return \cache::make('thquizaccess_seb', 'thquizsettings');
    }
    ...
    private function after_save() {
        self::get_thquiz_settings_cache()->set($this->get('thquizid'), $this->to_record());
        self::get_config_cache()->set($this->get('thquizid'), $this->config);
        self::get_config_key_cache()->set($this->get('thquizid'), $this->configkey);
    }
    ...
    protected function before_delete() {
        $key = $this->get('thquizid');
        self::get_thquiz_settings_cache()->delete($key);
    ...
    protected function validate_allowedbrowserexamkeys($keys) {
        ...
        foreach ($keys as $i => $key) {
            if (!preg_match('~^[a-f0-9]{64}$~', $key)) {
                return new lang_string('allowedbrowserkeyssyntax', 'thquizaccess_seb');
            }
        }
        if (count($keys) != count(array_unique($keys))) {
            return new lang_string('allowedbrowserkeysdistinct', 'thquizaccess_seb');
        }
        return true;
    }
    ...
    /**
     * Create or update the config string based on the current thquiz settings.
     */
    ...
    /**
     * Case for USE_SEB_UPLOAD_CONFIG. This creates a plist from an uploaded configuration file, then applies the thquiz
    ...
    private function process_seb_upload_config() {
        ...
        if (empty($file)) {
            throw new moodle_exception('noconfigfilefound', 'thquizaccess_seb', '', $this->get('cmid'));
        } else {
    ...
    private function process_required_enforced_settings() {
        ...
        $thquizurl = new moodle_url($CFG->wwwroot . "/mod/thquiz/view.php", ['id' => $this->get('cmid')]);
        $this->plist->set_or_update_value('startURL', new CFString($thquizurl->out(true)));
    ...
    /**
     * Sets the quitURL if found in the thquiz_settings.
     */
    ...
    /**
     * Sets the thquiz_setting's linkquitseb if a quitURL value was found in a template or uploaded config.
     */
