...
/**
 * Implementaton of the thquizaccess_timelimit plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquizaccess_timelimit extends thquiz_access_rule_base {

    public static function make(thquiz $thquizobj, $timenow, $canignoretimelimits) {

        if (empty($thquizobj->get_thquiz()->timelimit) || $canignoretimelimits) {
            return null;
        }
    ...
    public function description() {
        return get_string('thquiztimelimit', 'thquizaccess_timelimit',
                format_time($this->thquiz->timelimit));
    }

    public function end_time($attempt) {
        $timedue = $attempt->timestart + $this->thquiz->timelimit;
        if ($this->thquiz->timeclose) {
            $timedue = min($timedue, $this->thquiz->timeclose);
        }
        return $timedue;
    }
    ...
    public function add_preflight_check_form_fields(mod_thquiz_preflight_check_form $thquizform,
        ...
        $mform->addElement('header', 'honestycheckheader',
                get_string('confirmstartheader', 'thquizaccess_timelimit'));
        $mform->addElement('static', 'honestycheckmessage', '',
                get_string('confirmstart', 'thquizaccess_timelimit', format_time($this->thquiz->timelimit)));
    }
}
