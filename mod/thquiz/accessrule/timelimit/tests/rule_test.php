...
namespace thquizaccess_timelimit;

use thquiz;
use thquizaccess_timelimit;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/thquiz/accessrule/timelimit/rule.php');


/**
 * Unit tests for the thquizaccess_timelimit plugin.
 *
 * @package    thquizaccess_timelimit
...
class rule_test extends \basic_testcase {
    public function test_time_limit_access_rule() {
        $thquiz = new \stdClass();
        $thquiz->timeclose = 0;
        $thquiz->timelimit = 3600;
        $cm = new \stdClass();
        $cm->id = 0;
        $thquizobj = new thquiz($thquiz, $cm, null);
        $rule = new thquizaccess_timelimit($thquizobj, 10000);
        $attempt = new \stdClass();

        $this->assertEquals($rule->description(),
            get_string('thquiztimelimit', 'thquizaccess_timelimit', format_time(3600)));
    ...
    /**
     * Test the time_left_display method of the thquizaccess_timelimit class.
     *
     * @param int $timetoclose  The number of seconds that is left to the thquiz' closing time
     * @param int $timelimit    Time limit of the thquiz
     * @param int $displaylimit The limit that is displayed on the thquiz page
    ...
    public function test_time_limit_access_rule_with_time_close($timetoclose, $timelimit, $displaylimit, $actuallimit) {
        ...
        $thquiz = new \stdClass();
        $thquiz->timeclose = $timenow + $timetoclose;
        $thquiz->timelimit = $timelimit;
        $cm = new \stdClass();
        $cm->id = 0;
        $thquizobj = new thquiz($thquiz, $cm, null);
        $rule = new thquizaccess_timelimit($thquizobj, $timenow);
        $attempt = new \stdClass();

        $this->assertEquals($rule->description(),
            get_string('thquiztimelimit', 'thquizaccess_timelimit', format_time($displaylimit)));
