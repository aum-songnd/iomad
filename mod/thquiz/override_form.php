...
/**
 * Settings form for overrides in the thquiz module.
 *
 * @package    mod_thquiz
...
require_once($CFG->dirroot . '/mod/thquiz/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class thquiz_override_form extends moodleform {

    /** @var cm_info course module object. */
    protected $cm;

    /** @var stdClass the thquiz settings object. */
    protected $thquiz;
     ...
     * @param object the thquiz settings object.
     * @param context the thquiz context.
     ...
     */
    public function __construct($submiturl, $cm, $thquiz, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'thquiz'));

        $thquizgroupmode = groups_get_activity_groupmode($cm);
        $accessallgroups = ($thquizgroupmode == NOGROUPS) || has_capability('moodle/site:accessallgroups', $this->context);
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'thquiz'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/thquiz/overrides.php', array('cmid'=>$cm->id));
                    throw new \moodle_exception('groupsnone', 'thquiz', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'thquiz'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $capabilityjoin = get_with_capability_join($this->context, 'mod/thquiz:attempt', 'u.id');
                ...
                $users = $DB->get_records_sql("
                        SELECT $userfieldsql->selects
                          FROM {user} u
                          LEFT JOIN {thquiz_overrides} existingoverride ON
                                      existingoverride.userid = u.id AND existingoverride.thquiz = :thquizid
                         ...
                         WHERE existingoverride.id IS NULL
                           $capabilitywhere
                      ORDER BY $sort
                        ", array_merge(['thquizid' => $this->thquiz->id], $capabilityjoin->params,
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/thquiz/overrides.php', array('cmid'=>$cm->id));
                    throw new \moodle_exception('usersnone', 'thquiz', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'thquiz'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'thquiz');
        $mform->setDefault('password', $this->thquiz->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('thquizopen', 'thquiz'), mod_thquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->thquiz->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('thquizclose', 'thquiz'), mod_thquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->thquiz->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'thquiz'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'thquiz');
        $mform->setDefault('timelimit', $this->thquiz->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'thquiz'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'thquiz');
        $mform->setDefault('attempts', $this->thquiz->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'thquiz'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'thquiz'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'thquiz'));
    ...
    public function validation($data, $files): array {
        ...
        // Ensure that at least one thquiz setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $thquiz->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'thquiz');
        }
