...
/**
 * Upgrade script for the thquiz module.
 *
 * @package    mod_thquiz
...
/**
 * Thquiz module upgrade function.
 * @param string $oldversion the version we are upgrading from.
 */
function xmldb_thquiz_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2020061501) {
        ...
        // Define field completionminattempts to be added to thquiz.
        $table = new xmldb_table('thquiz');
        ...
        // Thquiz savepoint reached.
        upgrade_mod_savepoint(true, 2020061501, 'thquiz');
    }
    ...
    if ($oldversion < 2021052503) {
        ...
        if ($dbman->field_exists($table, $field)) {
            $sql = "SELECT q.id, m.id as thquizid " .
                "FROM {thquiz} q " .
            ...
            $records = $DB->get_recordset_sql($sql, ['name' => 'thquiz', 'completionpass' => 1], 0, 1000);
            while ($records->valid()) {
                $thquizmodule = null;
                foreach ($records as $record) {
                    $ids[] = $record->id;
                    $thquizmodule = $record->thquizid;
                }

                if ($ids) {
                    list($insql, $params) = $DB->get_in_or_equal($ids, SQL_PARAMS_NAMED);
                    $DB->set_field_select('course_modules', 'completionpassgrade', 1,
                        "module = :thquiz AND instance $insql", $params + ['thquiz' => $thquizmodule]);

                    // Reset the value so it doesn't get picked on the next run. The field will be dropped later.
                    $DB->set_field_select('thquiz', 'completionpass', 0, "id $insql", $params);
        ...
        upgrade_mod_savepoint(true, 2021052503, 'thquiz');
    }

    if ($oldversion < 2021101900) {

        // Define field gradednotificationsenttime to be added to thquiz_attempts.
        $table = new xmldb_table('thquiz_attempts');
        ...
        if (!$dbman->field_exists($table, $field)) {
            ...
            $DB->execute('UPDATE {thquiz_attempts} SET gradednotificationsenttime = timefinish');
        }

        // Thquiz savepoint reached.
        upgrade_mod_savepoint(true, 2021101900, 'thquiz');
    }

    if ($oldversion < 2022020300) {
        // Define table thquiz_slot_tags to be dropped.
        $table = new xmldb_table('thquiz_slot_tags');

        // Conditionally launch drop table for thquiz_slot_tags.
        if ($dbman->table_exists($table)) {
            $dbman->drop_table($table);
        }

        // Define fields to be dropped from thquiz_slots.
        $table = new xmldb_table('thquiz_slots');

        // Define key questionid (foreign) to be dropped form thquiz_slots.
        $key = new xmldb_key('questionid', XMLDB_KEY_FOREIGN, ['questionid'], 'question', ['id']);

        // Launch drop key questionid.
        $dbman->drop_key($table, $key);

        // Define key questioncategoryid (foreign) to be dropped form thquiz_slots.
        ...
        // Thquiz savepoint reached.
        upgrade_mod_savepoint(true, 2022020300, 'thquiz');
    }
