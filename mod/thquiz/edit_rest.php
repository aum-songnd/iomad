...
/**
 * Rest endpoint for ajax editing of thquiz structure.
 *
 * @package   mod_thquiz
...
$thquizid     = required_param('thquizid', PARAM_INT);
...
$PAGE->set_url('/mod/thquiz/edit-rest.php',
        array('thquizid' => $thquizid, 'class' => $class));
...
switch($requestmethod) {
    ...
    case 'GET': // For debugging.
        switch ($class) {
            case 'section':
                $table = 'thquiz_sections';
                $section = $structure->get_section_by_id($id);
                switch ($field) {
                    case 'getsectiontitle':
                        require_capability('mod/thquiz:manage', $modcontext);
            ...
            case 'resource':
                switch ($field) {
                    ...
                    case 'getmaxmark':
                        require_capability('mod/thquiz:manage', $modcontext);
                        $slot = $DB->get_record('thquiz_slots', array('id' => $id), '*', MUST_EXIST);
                        $result = array('instancemaxmark' => thquiz_format_question_grade($thquiz, $slot->maxmark));
                        break;
                    ...
                    case 'updatemaxmark':
                        ...
                        $result = array('instancemaxmark' => thquiz_format_question_grade($thquiz, $maxmark),
                                'newsummarks' => thquiz_format_grade($thquiz, $thquiz->sumgrades));
                        break;
                    ...
                    case 'deletemultiple':
                        ...
                        $ids = explode(',', $ids);
                        foreach ($ids as $id) {
                            $slot = $DB->get_record('thquiz_slots', array('thquizid' => $thquiz->id, 'id' => $id),
                                    '*', MUST_EXIST);
                        ...
                        $result = array('newsummarks' => thquiz_format_grade($thquiz, $thquiz->sumgrades),
    ...
    case 'DELETE':
        switch ($class) {
            ...
            case 'resource':
                require_capability('mod/thquiz:manage', $modcontext);
                if (!$slot = $DB->get_record('thquiz_slots', array('thquizid' => $thquiz->id, 'id' => $id))) {
