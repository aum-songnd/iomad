 ...
 * @return array|bool
 */
function question_thpluginfile_record($course, $context, $component, $filearea, $args, $forcedownload, $options = []) {
    global $DB, $CFG;
    ...
    if ($module === 'core_question_preview') {
        ...
        if (strpos($module, 'mod_') === 0) {
            ...
            if (function_exists($filefunctionold)) {
                if($module == 'mod_quiz' && get_capability_info('block/th_scoring_collaborators:review')){                           
                    include_once($CFG->dirroot . "/blocks/th_scoring_collaborators/recordlib.php");
                    $filefunctionold = 'thquiz_question_pluginfile_record';              
                }
