...
require_once($CFG->dirroot . '/blocks/th_scoring_collaborators/locallib.php');
...
$url = new moodle_url('/blocks/th_scoring_collaborators/review.php', array('attempt'=>$attemptid));
if ($page !== 0) {
    $url->param('page', $page);
} else if ($showall) {
...
$attemptobj = quiz_create_attempt_handling_errors1($attemptid, $cmid);
...
// $is_assigned_grader = $DB->record_exists('block_th_assign_grading', [
//     'attemptid' => $attemptid,
//     'ctvid' => $USER->id,
// ]);
// if (!$is_assigned_grader) {
//     $attemptobj->check_review_capability();
// }
...
} else if (!$attemptobj->is_review_allowed() /**&& !$is_assigned_grader**/) {
...
if ($attempt->state == quiz_attempt1::FINISHED) {
...
$summarydata['state'] = array(
    ...
    'content' => quiz_attempt1::state_name($attempt->state),
);
...
ob_start(function ($buffer) {
    ...
    $replace = $CFG->wwwroot . '/blocks/th_scoring_collaborators/thpluginfile.php';
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var element = document.querySelector('.submitbtns');
        var wwwroot = '<?php echo $CFG->wwwroot; ?>';
        var urlParams = new URLSearchParams(window.location.search);
        var confirm = urlParams.get('confirm');
        ...
        if (element) {
            if (confirm) {
                if (status !== null) {
                    ...
                    element.innerHTML = "<a class='th_link_attempt' href='" + wwwroot + "/blocks/th_assign_grading/action.php?confirm=" + id + "'>Phê duyệt</a>";
                } 
                else {
                    console.log('Status is not present');
                    element.innerHTML = "<a class='th_link_attempt' href='" + wwwroot + "/blocks/th_assign_grading/view.php'>Quay lại</a>";
                }
            } else {
                element.innerHTML += "<a class='th_wait' href='" + wwwroot + "/blocks/th_scoring_collaborators/edit.php?id=" + id + "'>Hoàn thành bài chấm</a>";
            }
        }
    });
</script>
