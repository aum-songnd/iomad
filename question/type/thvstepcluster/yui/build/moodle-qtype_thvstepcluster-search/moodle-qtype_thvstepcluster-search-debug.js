YUI.add('moodle-qtype_thvstepcluster-search', function (Y, NAME) {
 ...
 *
 * @namespace M.qtype_thvstepcluster
 * @class Search
 * @constructor
 * @extends Base
 */
...
SEARCH.prototype = {
    ...
    initializer: function() {
        this.form = Y.one('#qtype_thvstepcluster-overview-form');
        this.select = this.form.one('select[data-search=question_entry_items]');
        ...
        this.select.get('options').each(function(option) {
            ...
            var capability = option.get('value') + option.get('text').toLowerCase();
     ...
     */
    typed: function() {
        var search = this.input.get('value').toLowerCase(),
            matching = 0,
            last = null,
            capability;
        if (this.lastsearch) {
...
M.qtype_thvstepcluster = M.qtype_thvstepcluster || {};
 ...
 */
M.qtype_thvstepcluster.init_capability_search = function(options) {
    new SEARCH(options);
};
