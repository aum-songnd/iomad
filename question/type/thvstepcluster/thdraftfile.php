...
if (0 == strpos($relativepath, '/token/')) {
    $relativepath2 = ltrim($relativepath, '/');
    $pathparts = explode('/', $relativepath2, 2);
    $token = $pathparts[0];
    $token = clean_param($token, PARAM_ALPHANUM);
    $relativepath2 = "/{$pathparts[1]}";
}



// print_object($relativepath2);
...
// print_object('relative path');
// print_object($relativepath);
