...
// question/type/recordrtc/uploadrecording.php
...
foreach ($files as $file) {
    ...
    $storaged_file = $fs->get_file($file_record->contextid, $file_record->component, $file_record->filearea,
    $file_record->itemid, $file_record->filepath, $file_record->filename);

    if($storaged_file)
    {
        $storaged_file->delete();
        $existingfile = false;
    }
    ...
    if ($existingfile) {
        ...
        $results["file_record"] = $file_record;
        $results["url"] = moodle_url::make_draftfile_url($file_record->itemid, $file_record->filepath, $file_record->filename)->out(false);
    }
}
