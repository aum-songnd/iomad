...
if ($sidebar) {
	...
	$lambda_body_attributes .= ' sidebar-enabled ' . $sidebar_stat;}
...
if ($hasfrontpageblocks == 1) {
	if ($standardlayout) {echo $OUTPUT->blocks('side-pre', 'span4 desktop-first-column pull-left');} else if (!$sidebar) {echo $OUTPUT->blocks('side-pre', 'span4 desktop-first-column pull-right');}
}?>
            </div>
        	</div>
...
echo $OUTPUT->blocks('hidden-dock');
    ...
    <?php
        if (core_plugin_manager::instance()->get_plugin_info('local_th_feedback_report')) {
            include($CFG->dirroot . '/theme/th_lambda_st/layout/feedback_report_form.php');
        } else {
            echo '<style>
                    .back-to-top {
