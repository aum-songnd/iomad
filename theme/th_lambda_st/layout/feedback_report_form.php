<div class="th_lambda_feedback_report">
    <div id="errorFormContainer" style="display:none;">
        <form id="errorForm" action="<?php echo $CFG->wwwroot ?>/local/th_feedback_report/contact.php" method="post">
            ...
            <h2><?php echo get_string('title_log_feedback_report', 'theme_th_lambda_st'); ?></h2>
            <div class="error_img">
            ...
            <div class="feedback-category">
                <label><?php echo get_string('feedback_type', 'theme_th_lambda_st'); ?></label>
                <select id="supportType" name="supportType" onchange="showSubcategories_Error(this.value)" required>
                    <option value=""><?php echo get_string('select_feedback_type', 'theme_th_lambda_st'); ?></option>
                    <?php
                    global $DB;
                ...
                foreach ($categories as $category) {
                    if ($category->parentid == null) {
                        ...
                        if (!empty($subcategories)) {
                            ...
                            echo '<option value="">'. get_string('select_feedback_type', 'theme_th_lambda_st') .'</option>';
            ...
            <div style="width: 100%; display: flex;">
                <textarea id="issueDescription" name="issueDescription" rows="5" placeholder="<?php echo get_string('issue_description_placeholder', 'theme_th_lambda_st'); ?>" required minlength="60"></textarea>
            </div>
            <input type="file" accept="image/*" id="uploadScreenshot" style="display:none;">
            <div class="feedback_buttons">
                <input type="submit" id="submit_feedback" value="<?php echo get_string('btn_feedback', 'theme_th_lambda_st'); ?>">
            </div>
        </form>
    </div>
    <div class="error-button" >
        <img src="<?php echo $CFG->wwwroot ?>/theme/th_lambda_st/img/feedback.png" alt="error-button" width="36" height="36">
        <div class="feedback_text"><?php echo get_string('feedback_text', 'theme_th_lambda_st') ?></div>
    </div>
</div>
<script>
