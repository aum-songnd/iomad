...
// Use the same method as theme Boost for IOMAD compatibility
$haslogo = false;
$logourl = '';

if (method_exists($OUTPUT, 'should_display_navbar_logo') && $OUTPUT->should_display_navbar_logo()) {
    if (method_exists($OUTPUT, 'get_compact_logo_url')) {
        $logourl = $OUTPUT->get_compact_logo_url();
        // Only set haslogo to true if we have a valid logo URL
        // Check if URL is not empty and doesn't end with /. or just /
        if (!empty($logourl) && !preg_match('#/\.?$#', $logourl)) {
            $haslogo = true;
        }
    }
}

// Get company name for display when no logo
$sitename = $SITE->fullname; // Default to site fullname
$brandshortname = format_string($SITE->shortname, true, ['context' => context_course::instance(SITEID), 'escape' => false]);
if (class_exists('iomad') && class_exists('company')) {
    try {
        $companyid = iomad::get_my_companyid(context_system::instance());
        if ($companyid) {
            $company = new company($companyid);
            $companyshortname = trim((string)$company->get_shortname());
            if ($companyshortname !== '') {
                $sitename = $companyshortname;
                $brandshortname = format_string($companyshortname, true,
                    ['context' => context_system::instance(), 'escape' => false]);
            }
        }
    } catch (Exception $e) {
        // If IOMAD fails, keep site fullname
    }
}
        ...
        <div class="lambda-custom-menu container-fluid">
            <?php
                if ($home_button == 'shortname') {
                    $home_button_string = '<a class="brand" href="'.$CFG->wwwroot.'">'.$brandshortname.'</a>'; 
                }
                    ...
                    if ($fanew) {
                        $home_button_string = '<a class="brand" href="'.$CFG->wwwroot.'"><i class="fa-solid fa-house-chimney"></i></a>'; 
                    } else {
                        $home_button_string = '<a class="brand" href="'.$CFG->wwwroot.'"><i class="fa fa-home"></i></a>'; 
                    }                    
                }
